/**
 * Comprehensive test of 4-step confidence-scored RAG methodology across multiple models
 * Tests each model's performance on complex queries and grades their effectiveness
 */

import { ConfidenceMasterOrchestrator } from './src/core/master-orchestrator/ConfidenceMasterOrchestrator.js';
...
class ConfidenceModelTester {
  ...
  async testModel(modelName: string): Promise<ModelTestResult> {
    ...
    try {
      ...
      const agentRegistry = new AgentRegistry(llmProvider);
      ...
      try {
        ...
        console.log('RAG initialization failed, proceeding without RAG');
      }
      ...
      const orchestrator = new ConfidenceMasterOrchestrator(
        ...
        agentRegistry,
        toolRegistry,
        undefined, // No database for this test
        ragSystem
      );

      // Create a query object
      const query = {
        id: `test-${modelName}-${Date.now()}`,
        text: COMPLEX_QUERY,
        conversationId: `conv-${modelName}`,
        timestamp: new Date(),
        metadata: {
          testModel: modelName,
          testType: '4-step-confidence'
        }
      };
      ...
      try {
        ...
        queryComplexity = await orchestrator['confidenceRAG'].analyzer.assessComplexity(query.text);
        console.log(`Query complexity: ${queryComplexity.score}/10`);
        
        // Perform retrieval if RAG is available
        if (orchestrator['confidenceRAG'].retriever) {
          retrievalResult = await orchestrator['confidenceRAG'].retriever.retrieve(
            query.text,
            { maxDocuments: 10, minScore: 0.6 }
          );
          console.log(`Retrieved ${retrievalResult.documents.length} documents`);
        }
        
        steps.push({
          stepName: 'Query Processing & RAG Retrieval',
          startTime: step1Start,
          endTime: Date.now(),
          duration: Date.now() - step1Start,
          response: { queryComplexity, documentsRetrieved: retrievalResult?.documents.length || 0 },
          confidence: retrievalResult?.retrievalConfidence || 0.5
        });
        stepsCompleted++;
      } catch (error) {
        steps.push({
      ...
      try {
        const context = {
          ...
          retrievalConfidence: retrievalResult?.retrievalConfidence || 0.5
        };
        
        generationResult = await orchestrator['confidenceRAG'].responseGenerator.generateWithConfidence(
          context,
      ...
      try {
        evaluationResult = await orchestrator['multiModalEvaluator'].evaluate(
          query.text,
      ...
      try {
        deliveryResult = await orchestrator['adaptiveDelivery'].deliver(
          evaluationResult || {
  ...
  async runAllTests(): Promise<void> {
    console.log('\nStarting comprehensive model testing for 4-step confidence-scored RAG');
...
// Run the tests
async function main() {
  const tester = new ConfidenceModelTester();
