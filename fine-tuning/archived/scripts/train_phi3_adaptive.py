#!/usr/bin/env python3
"""
Adaptive Training Pipeline for Microsoft Phi-3-mini-4k-instruct
...
@dataclass
class Phi3Config:
    """Configuration optimized for Phi-3-mini on CPU"""
    ...
    # Paths
    dataset_dir: str = "./datasets/gemma_formatted"  # Using same formatted data
    output_dir: str = "./phi3-mini-finetuned"
    checkpoint_dir: str = "./checkpoints-phi3"
    cache_dir: str = "./model_cache"
    
    # Training hyperparameters (CPU-optimized for 3.8B model)
    learning_rate: float = 3e-5  # Slightly lower for larger model
    num_train_epochs: int = 1  # Start with 1 epoch for testing
    per_device_train_batch_size: int = 1  # Very conservative for 3.8B model
    ...
    lora_r: int = 16  # Slightly higher rank for larger model
    lora_alpha: int = 32
    ...
    max_seq_length: int = 4096  # Utilize Phi-3.5's enhanced context (128k capable, start conservative)
    ...
    # Training control
    save_steps: int = 200  # Less frequent saves
    eval_steps: int = 50  # Regular evals
    logging_steps: int = 5  # Frequent logging to see progress
...
class AdaptivePhi3Trainer:
    """Main trainer class for Phi-3-mini with adaptive curriculum"""
    ...
    def load_model_and_tokenizer(self):
        """Load Phi-3 model and tokenizer with CPU optimizations"""
    ...
    def format_for_phi3(self, instruction: str, response: str) -> str:
        """Format data for Phi-3 instruction template"""
        # Phi-3 uses a specific format
        return f"<|user|>\n{instruction}<|end|>\n<|assistant|>\n{response}<|end|>"
        
    def load_datasets(self) -> tuple:
        """Load and prepare datasets with Phi-3 formatting"""
        
        logger.info("Loading datasets for Phi-3...")
        
        # Load pre-formatted datasets (we'll adapt them for Phi-3)
        train_path = Path(self.config.dataset_dir) / "gemma_train.json"
        val_path = Path(self.config.dataset_dir) / "gemma_val.json"
        
        if not train_path.exists() or not val_path.exists():
            raise FileNotFoundError(f"Dataset files not found in {self.config.dataset_dir}")
        ...
        def convert_to_phi3(examples):
            """Convert from Gemma format to Phi-3 format"""
            phi3_texts = []
            for ex in examples:
                # Extract instruction and response from the formatted text
                text = ex.get('text', '')
                if '<start_of_turn>user' in text and '<start_of_turn>model' in text:
                    # Parse Gemma format
                    parts = text.split('<start_of_turn>model')
                    if len(parts) == 2:
                        instruction = parts[0].replace('<start_of_turn>user', '').replace('<end_of_turn>', '').strip()
                        response = parts[1].replace('<end_of_turn>', '').strip()
                        phi3_text = self.format_for_phi3(instruction, response)
                    else:
                        phi3_text = text  # Fallback
                else:
                    # Use original instruction/output if available
                    instruction = ex.get('instruction', '')
                    output = ex.get('output', '')
                    phi3_text = self.format_for_phi3(instruction, output)
                
                phi3_texts.append(phi3_text)
            
            return phi3_texts
        
        # Convert datasets
        train_texts = convert_to_phi3(train_data)
        val_texts = convert_to_phi3(val_data)
    ...
    def train(self):
        ...
        logger.info("="*80)
        logger.info("PHI-3-MINI ADAPTIVE TRAINING PIPELINE")
    ...
    def test_inference(self, prompt: str, max_new_tokens: int = 512):
        ...
        # Format prompt with Phi-3 template
...
def main():
    ...
    # Create trainer
    trainer = AdaptivePhi3Trainer(config)
