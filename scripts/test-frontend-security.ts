...
async function testCORS(): Promise<void> {
  ...
  try {
    ...
    log(`❌ CORS test failed: ${error instanceof Error ? error.message : String(error)}`, 'red');
  }
}
...
async function testCSP(): Promise<void> {
  ...
  try {
    ...
    if (cspHeader) {
      ...
      directives.forEach(directive => {
        if (directive) {
          ...
          if (name === 'script-src' && process.env.NODE_ENV === 'development' && values.includes("'unsafe-inline'")) {
    ...
    log(`❌ CSP test failed: ${error instanceof Error ? error.message : String(error)}`, 'red');
  }
}
...
async function testWebSocket(): Promise<void> {
  ...
  return new Promise<void>((resolve) => {
    ...
    ws.on('unexpected-response', (request, response) => {
      log(`❌ WebSocket unexpected response: ${response.statusCode} ${response.statusMessage}`, 'red');
      resolve();
    });
...
async function testSecurityHeaders(): Promise<void> {
  ...
  try {
    ...
    log(`❌ Security headers test failed: ${error instanceof Error ? error.message : String(error)}`, 'red');
  }
}
...
async function testCSRFIntegration(): Promise<void> {
  ...
  try {
    ...
    log(`❌ CSRF test failed: ${error instanceof Error ? error.message : String(error)}`, 'red');
  }
}
