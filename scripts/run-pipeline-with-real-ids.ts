...
for (const batchNum of batches) {
  const batchFile = path.join(__dirname, `../test_batches/emails_batch_${batchNum}.json`);
  ...
  for (const email of emails) {
    const dbEmail = db.prepare('SELECT id FROM emails WHERE id = ? OR graph_id = ?').get(email.id, email.graph_id);
    if (dbEmail) {
