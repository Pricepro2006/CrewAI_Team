...
/**
 * Continuous Email Processing Script
 * Processes ALL remaining emails with adaptive three-phase analysis
 */
...
const logger = new Logger("ContinuousEmailProcessor");

const ENHANCED_DB_PATH = "./data/crewai_enhanced.db";
const BATCH_SIZE = 1000; // Process 1000 conversations at a time for efficiency
...
class ContinuousEmailProcessor {
  ...
  async processAllEmails(): Promise<void> {
    console.log(
      chalk.cyan("\n🚀 Continuous Adaptive Three-Phase Email Processing\n"),
    );
    console.log(
      chalk.yellow(
        "Processing ALL remaining emails with chain-aware analysis\n",
      ),
    );

    // Get total count of unprocessed conversations
    ...
    this.stats.totalConversations = totalCount.count;

    console.log(
      chalk.bold(
        `📊 Total unprocessed conversations: ${this.stats.totalConversations}\n`,
      ),
    );
    ...
    while (true) {
      ...
      console.log(
        chalk.cyan(
          `\n📦 Processing batch ${processedBatches} (${conversations.length} conversations)\n`,
        ),
      );
      ...
      for (const conv of conversations) {
        ...
        // Display progress every 50 conversations
        if (this.stats.processedConversations % 50 === 0) {
  ...
  private async processConversation(conv: any): Promise<void> {
    try {
      const convNumber = this.stats.processedConversations + 1;
      console.log(
        chalk.gray(
          `\n[${convNumber}/${this.stats.totalConversations}] ${conv.conversation_id}`,
        ),
      );
      console.log(
...
Provide strategic insights:
{
  ...
  private displayProgress(): void {
    ...
    console.log(
      `  Rate: ${rate.toFixed(1)} conv/min | ETA: ${eta.toFixed(0)} minutes`,
    );
  }
  ...
  private displayFinalStats(): void {
    ...
    console.log(`  Emails processed: ${this.stats.processedEmails}`);
    console.log(
    ...
    console.log(
      chalk.yellow(
        "\n🎉 All emails processed with adaptive three-phase analysis! 🌪️",
      ),
    );
  }
}
...
// Run the processor
async function main() {
  const processor = new ContinuousEmailProcessor();
