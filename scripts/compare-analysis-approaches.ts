...
// LLM Helper Function
// ============================================
async function callLLM(prompt: string, model: string = 'llama3.2:3b'): Promise<string> {
  try {
...
function phase1RuleBasedAnalysis(email: any): any {
  ...
  // Extract PO Numbers
  const poMatches = emailContent.match(/\b(?:po|p\.o\.|purchase order)[\s#:-]*(\d{8,12})\b/gi) || [];
  result.entities.po_numbers = poMatches.map(m => m.replace(/[^\d]/g, ''));
  
  // Extract Quote Numbers
  const quoteMatches = emailContent.match(/\b(?:quote|qt)[\s#:-]*([A-Z0-9]{6,10})\b/gi) || [];
  result.entities.quote_numbers = quoteMatches.map(m => m.replace(/^(quote|qt)[\s#:-]*/i, ''));
  
  // Extract Order Numbers
  const orderMatches = emailContent.match(/\b(?:order|ord)[\s#:-]*([A-Z0-9]{8,12})\b/gi) || [];
  result.entities.order_numbers = orderMatches.map(m => m.replace(/^(order|ord)[\s#:-]*/i, ''));
  
  // Extract Tracking Numbers
  const trackingMatches = emailContent.match(/\b(?:1Z[A-Z0-9]{16}|[\d]{20,22})\b/g) || [];
  result.entities.tracking_numbers = trackingMatches;
  
  // Extract Dollar Values
  const dollarMatches = emailContent.match(/\$[\d,]+\.?\d{0,2}/g) || [];
  result.entities.dollar_values = dollarMatches.map(m => parseFloat(m.replace(/[$,]/g, '')));
  
  // Pattern Detection
  result.patterns.is_order_related = /\b(order|purchase|po|p\.o\.)\b/i.test(emailContent);
  result.patterns.is_quote_related = /\b(quote|pricing|proposal|rfq)\b/i.test(emailContent);
  result.patterns.is_shipping_related = /\b(ship|delivery|tracking|fedex|ups)\b/i.test(emailContent);
  result.patterns.contains_pricing = /\b(\$|price|cost|amount|total)\b/i.test(emailContent);
  result.patterns.is_return_rma = /\b(return|rma|refund|damaged|defective)\b/i.test(emailContent);
  result.patterns.is_account_change = /\b(account|update|change|modify|address)\b/i.test(emailContent);
  
  // Calculate confidence based on entities found
  const entityCount = Object.values(result.entities).reduce((sum, arr) => sum + arr.length, 0);
  const patternCount = Object.values(result.patterns).filter(v => v).length;
  result.confidence_score = Math.min((entityCount * 10 + patternCount * 15) / 100, 1);
...
async function threePhaseAnalysis(email: any): Promise<AnalysisResult> {
  ...
  const result: AnalysisResult = {
    ...
    confidence: phase2.enhancements?.confidence || phase1.confidence_score || 0.5,
...
function compareResults(results: AnalysisResult[]): void {
  ...
  ['single-phase', 'three-phase', 'phase-3-only'].forEach(approach => {
    ...
    console.log(`│ Avg Entities    │ ${(s.entities / emailCount).toFixed(1).padEnd(12)} │`);
  });
  ...
  ['single-phase', 'three-phase', 'phase-3-only'].forEach(approach => {
    const s = stats[approach];
    console.log(`│ Category Found  │ ${((s.correct / emailCount) * 100).toFixed(0) + '%'.padEnd(12)} │`);
  });
  ...
  ['single-phase', 'three-phase', 'phase-3-only'].forEach(approach => {
    const s = stats[approach];
    console.log(`│ Avg Time (ms)   │ ${(s.time / emailCount).toFixed(0).padEnd(12)} │`);
  });
  ...
  ['single-phase', 'three-phase', 'phase-3-only'].forEach(approach => {
    const s = stats[approach];
    console.log(`│ Avg Tokens      │ ${(s.tokens / emailCount).toFixed(0).padEnd(12)} │`);
  });
  ...
  const threePhaseImprovement = ((stats['three-phase'].entities - stats['single-phase'].entities) / stats['single-phase'].entities * 100);
  console.log(`1. Entity Extraction: Three-phase found ${threePhaseImprovement.toFixed(1)}% more entities than single-phase`);
  
  const timeOverhead = ((stats['three-phase'].time - stats['single-phase'].time) / stats['single-phase'].time * 100);
  console.log(`2. Processing Time: Three-phase took ${timeOverhead.toFixed(1)}% more time`);
  
  const tokenOverhead = ((stats['three-phase'].tokens - stats['single-phase'].tokens) / stats['single-phase'].tokens * 100);
  console.log(`3. Token Usage: Three-phase used ${tokenOverhead.toFixed(1)}% more tokens`);
  
  console.log(`4. Phase-3-Only: Fastest but extracted ${((stats['phase-3-only'].entities / stats['three-phase'].entities) * 100).toFixed(1)}% of entities compared to three-phase`);
}
...
async function runComparisonTest(): Promise<void> {
  ...
  for (const [index, email] of emails.entries()) {
    ...
    try {
      ...
      console.error(chalk.red(`Failed to process email ${email.id}:`, error.message));
    }
  }
  ...
  const resultsPath = path.join(__dirname, `../test_results/comparison_${timestamp}.json`);
  ...
  fs.writeFileSync(resultsPath, JSON.stringify({
    ...
    results: results,
    summary: {
      threePhaseAdvantage: 'Better entity extraction and context understanding',
      singlePhaseAdvantage: 'Faster processing and fewer tokens',
      phase3OnlyAdvantage: 'Fastest but limited entity extraction',
      recommendation: 'Use three-phase for critical emails, single-phase for routine processing'
    }
  }, null, 2));
