...
const testEndpoints: TestEndpoint[] = [
  ...
  { path: '/api/test', method: 'OPTIONS', headers: { 
  ...
  return new Promise<void>((resolve, reject) => {
    const req = http.request(options, (res) => {
      ...
      securityHeaders.forEach((header: string) => {
        ...
        } else if (header === 'strict-transport-security' && process.env.NODE_ENV !== 'production') {
          console.log(`- ${header}: [Not set in development]`);
        } else if (header.startsWith('access-control-') && endpoint.method !== 'OPTIONS' && !endpoint.headers?.Origin) {
    ...
    req.on('error', (e) => {
      console.error(`Problem with request: ${e instanceof Error ? e.message : String(e)}`);
      reject(e);
    });
