...
function extractEntities(text: string): any {
  const patterns = {
    ...
    customer_names:
      /(?:customer|Customer|CLIENT|Client)[\s:-]+([A-Z][A-Za-z\s&.,'-]+(?:Inc|LLC|Corp|Corporation|Ltd)?)/gi,
    dollar_values:
      /\$[\d,]+\.?\d{0,2}|\b\d+(?:,\d{3})*(?:\.\d{2})?\s*(?:USD|dollars?)\b/gi,
    dates:
      /\b(?:\d{1,2}[-/]\d{1,2}[-/]\d{2,4}|\d{4}[-/]\d{1,2}[-/]\d{1,2}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{1,2},? \d{2,4})\b/gi,
  };
...
Provide a comprehensive JSON response with all findings.`;
  ...
  try {
    ...
    const categoryMatch = response.match(
      new RegExp(`(${WORKFLOW_CATEGORIES.join("|")})`, "i"),
    );
...
Provide actionable intelligence in JSON format.`;
  ...
  let finalResults = {
    ...
    workflow_category:
      phase2Results.workflow_category || phase2Results.category,
    priority: "MEDIUM",
    confidence: 0,
    actions: [],
  ...
  console.log(
    chalk.gray(`    Completed in ${finalResults.phase_times.phase3}ms`),
  );
...
function compareResults(results: any[]): void {
  ...
  Object.entries(byEmail).forEach(([emailId, approaches]: [string, any]) => {
    ...
    if (threeEntities > singleEntities && threeEntities > phase3Entities) {
      ...
      singleEntities > threeEntities &&
      singleEntities > phase3Entities
    ) {
      betterSinglePhase++;
    } else if (
      phase3Entities > singleEntities &&
      phase3Entities > threeEntities
    ) {
      betterPhase3Only++;
    }
  });
  ...
  console.log(
    `  Three-phase won: ${betterThreePhase}/${totalComparisons} (${((betterThreePhase / totalComparisons) * 100).toFixed(1)}%)`,
  );
  console.log(
    `  Single-phase won: ${betterSinglePhase}/${totalComparisons} (${((betterSinglePhase / totalComparisons) * 100).toFixed(1)}%)`,
  );
  console.log(
    `  Phase-3-only won: ${betterPhase3Only}/${totalComparisons} (${((betterPhase3Only / totalComparisons) * 100).toFixed(1)}%)`,
  );
  ...
  console.log(
    `  Single-phase: ${Math.round(avgTimes["single-phase"] / emailCount)}ms`,
  );
  console.log(
    `  Three-phase: ${Math.round(avgTimes["three-phase"] / emailCount)}ms`,
  );
  console.log(
    `  Phase-3-only: ${Math.round(avgTimes["phase3-only"] / emailCount)}ms`,
  );
  ...
  results.forEach((r) => {
    if (
      r.workflow_category &&
      WORKFLOW_CATEGORIES.includes(r.workflow_category)
    ) {
  ...
  console.log(
    `  Single-phase: ${categoryAccuracy["single-phase"]}/${emailCount} (${((categoryAccuracy["single-phase"] / emailCount) * 100).toFixed(1)}%)`,
  );
  console.log(
    `  Three-phase: ${categoryAccuracy["three-phase"]}/${emailCount} (${((categoryAccuracy["three-phase"] / emailCount) * 100).toFixed(1)}%)`,
  );
  console.log(
    `  Phase-3-only: ${categoryAccuracy["phase3-only"]}/${emailCount} (${((categoryAccuracy["phase3-only"] / emailCount) * 100).toFixed(1)}%)`,
  );
  ...
  console.log(
    `  Three-phase: ${results.filter((r) => r.approach === "three-phase")[0]?.llm_calls || 2} calls per email`,
  );
...
async function runComparisonTest(): Promise<void> {
  ...
  const resultsPath = path.join(
    __dirname,
    `../test_results/phase_comparison_${timestamp}.json`,
  );
