...
const testEmails = [
  {
    ...
    body: "Order PO #45791234 containing HP EliteBook 840 G9 (SKU: 5D5V8UT#ABA) has been delayed. New delivery: July 25, 2025. Tracking: 1Z999AA10123456784. Please contact account manager.",
  ...
  },
  {
    ...
    from: {
      emailAddress: { name: "TD SYNNEX Sales", address: "sales@tdsynnex.com" },
    },
  ...
  try {
    ...
    for (const email of testEmails) {
      ...
      try {
        ...
        const entityCount =
          entities.poNumbers.length +
          entities.quoteNumbers.length +
          entities.orderNumbers.length +
          entities.trackingNumbers.length +
          entities.caseNumbers.length +
          entities.customers.length +
          entities.products.length +
          entities.amounts.length;
        totalEntities += entityCount;

        console.log(
          `  ✓ ${email.id}: ${(endTime - startTime).toFixed(0)}ms, confidence: ${(analysis.confidence * 100).toFixed(1)}%, entities: ${entityCount}`,
        );
      } catch (error) {
        const endTime = performance.now();
        times.push(endTime - startTime);
...
async function runQuickEmailModelTest() {
  ...
  console.log(
    `Testing ${keyModels.length} models on ${testEmails.length} representative emails...\n`,
  );
  ...
  console.log("-".repeat(80));
  console.log(
    "Rank | Model         | Quality | Success | Avg Time | Confidence | Entities | Errors",
  );
  console.log("-".repeat(80));
  ...
  results.forEach((result, index) => {
    ...
    const confidence = `${(result.avgConfidence * 100).toFixed(1)}%`.padStart(
      10,
    );
    ...
    console.log(
      `${rank} | ${model} | ${quality} | ${success} | ${avgTime} | ${confidence} | ${entities} | ${errors}`,
    );
  });
  ...
  const mostAccurate = results.sort(
    (a, b) => b.avgConfidence - a.avgConfidence,
  )[0];

  console.log(
    `🏆 Best Overall: ${best.model} (Quality: ${(best.qualityScore * 100).toFixed(1)}%)`,
  );
  console.log(
    `⚡ Fastest: ${fastest.model} (${fastest.avgTime.toFixed(0)}ms average)`,
  );
  console.log(
    `🎯 Most Confident: ${mostAccurate.model} (${(mostAccurate.avgConfidence * 100).toFixed(1)}% confidence)`,
  );

  console.log("\n💡 USAGE GUIDELINES:");
  console.log(
    "• For real-time analysis: Use fastest model with acceptable accuracy",
  );
