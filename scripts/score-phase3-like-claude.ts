...
const CLAUDE_WEIGHTS = {
  contextUnderstanding: 0.20,    // 20%
  entityExtraction: 0.25,        // 25%
  businessProcessing: 0.20,      // 20%
  actionableInsights: 0.20,      // 20%
...
const CLAUDE_BENCHMARKS = {
  contextUnderstanding: {
    ...
    priorityAccuracy: 0.90,        // 90% correct priority
  ...
  entityExtraction: {
    ...
    partNumbers: 0.90,             // 90% part/SKU extraction
  ...
  actionableInsights: {
    actionIdentification: 0.90,    // 90% action detection
    ownerAssignment: 0.85,        // 85% correct owner
    deadlineAccuracy: 0.80        // 80% deadline appropriateness
  },
  responseQuality: {
    toneAppropriate: 0.95,        // 95% tone match
    contentRelevance: 0.90,       // 90% relevant content
...
function scoreContextUnderstanding(analysis: any, emailData: any): number {
  ...
  if (analysis.deep_workflow_primary) {
    ...
    const workflowMatch = analysis.deep_workflow_primary.includes(expectedWorkflow) ||
                         (expectedWorkflow === 'START_POINT' && analysis.deep_workflow_primary.includes('New')) ||
                         (expectedWorkflow === 'IN_PROGRESS' && analysis.deep_workflow_primary.includes('Processing'));
  ...
  if (analysis.quick_priority) {
    const subject = emailData?.Subject?.toLowerCase() || '';
    const expectedPriority = subject.includes('urgent') || subject.includes('asap') ? 'high' : 'medium';
    const priorityMatch = analysis.quick_priority.toLowerCase() === expectedPriority;
...
function scoreEntityExtraction(analysis: any, emailData: any): number {
  ...
  const companyPattern = /\b(?:TD SYNNEX|INSIGHT|Corporation|Inc\.|LLC|Corp\.)/gi;
  const expectedCompanies = (content.match(companyPattern) || []).length > 0 ? 1 : 0;
...
function scoreBusinessProcessing(analysis: any): number {
  ...
  if (analysis.deep_workflow_primary) {
    const validProcesses = ['Order Management', 'Quote Processing', 'Issue Resolution', 'General Support'];
    const hasValidProcess = validProcesses.some(p => analysis.deep_workflow_primary.includes(p));
...
async function analyzePhase3Quality() {
  ...
  console.log('Using the exact same scoring criteria that gave Claude Opus-4 its 8.5/10 score\n');
  ...
  console.log(`Found ${analyses.length} Phase 3 analyses from ${testEmailIds.length} test emails\n`);
  ...
  if (overallScore >= 7.75) {
    ...
    console.log(`❌ Below target by ${(7.75 - overallScore).toFixed(1)} points`);
  }
  ...
  if (avgProcessingTime < 5000) {
    console.log('❌ WARNING: Processing times suggest LLM was NOT actually called!');
    console.log('   Real doomgrave/phi-4 calls should take 50-180 seconds per email.');
    console.log('   These results appear to be from mock/simulated analysis.');
  }
  
  // Save results
  const resultsPath = path.join(__dirname, '../test-results', `claude_methodology_scoring_${Date.now()}.json`);
