...
function detectWorkflowCategory(email: any): string {
  ...
  if (
    /\b(po#?|purchase order|p\.o\.)\s*\d+/i.test(content) ||
  ...
  if (
    ...
    content.includes("quotation") ||
    /\bRFQ\b/i.test(content) ||
  ...
  if (
    ...
    content.includes("contract extension") ||
...
function detectWorkflowState(
  ...
  if (
    STATUS_KEYWORDS.YELLOW.some((kw) => content.includes(kw)) ||
...
function detectTaskStatus(
  ...
  if (
    STATUS_KEYWORDS.RED.some((kw) => content.includes(kw)) ||
    importance === "high"
  ) {
    return "RED";
  }
...
function extractContacts(email: any): {
  internal: string[];
  external: string[];
} {
...
function calculatePriority(
  ...
  const maxAmount = Math.max(
    ...dollarAmounts.map((amt) => parseFloat(amt.replace(/[$,]/g, "")) || 0),
    0,
  );
...
function detectSenderCategory(
  senderEmail?: string,
): "key_customer" | "internal" | "vendor" | "standard" {
...
function detectConversationPosition(
  subject: string,
): "thread_start" | "reply" | "forward" {
...
function extractPartNumbers(text: string): string[] {
  ...
  return matches
    .filter(
      (m) =>
        !m.match(
          /^(THE|AND|FOR|WITH|FROM|THIS|THAT|HAVE|WILL|BEEN|EMAIL|ORDER|QUOTE)$/,
        ) && m.length >= 6,
    )
    .slice(0, 20);
}
...
async function phase2WorkflowAnalysis(
  email: any,
  phase1Results: Phase1WorkflowResults,
): Promise<Phase2WorkflowResults> {
  const startTime = Date.now();

  try {
    const prompt = PHASE2_WORKFLOW_PROMPT.replace(
      "{PHASE1_RESULTS}",
...
async function phase3ExecutiveAnalysis(
  ...
  try {
    const prompt = PHASE3_EXECUTIVE_PROMPT.replace(
      "{PHASE1_RESULTS}",
      JSON.stringify(phase1Results, null, 2),
    )
...
function createWorkflowTask(
  ...
  return {
    ...
    // Task Details
    title:
      analysis.task_details?.task_title ||
      `${analysis.workflow_category}: ${email.subject?.substring(0, 100)}`,
    description:
      analysis.task_details?.task_description ||
      email.body_preview ||
    ...
    updated_at: now,
    sla_deadline:
      analysis.sla_assessment?.sla_deadline ||
...
function determineWorkflowPhases(
  email: any,
  phase1: Phase1WorkflowResults,
): {
  phases: number[];
  reason: string;
  ...
  const needsPhase3 =
    ...
    (phase1.sender_category === "key_customer" &&
      phase1.urgency_indicators.length > 0) ||
...
async function processEmailWorkflow(email: any, db: Database.Database) {
  ...
  console.log(
    `   ✓ Status: ${phase1Results.task_status} (${phase1Results.processing_time}ms)`,
  );
  ...
  if (phaseDecision.phases.includes(2)) {
    ...
    console.log(
      `   ✓ Owner: ${phase2Results.ownership_analysis.current_owner}`,
    );
    console.log(
      `   ✓ SLA: ${phase2Results.sla_assessment.sla_status} (${phase2Results.phase2_processing_time}ms)`,
    );
  }
  ...
  if (phaseDecision.phases.includes(3)) {
    ...
    const phase3Results = await phase3ExecutiveAnalysis(
      email,
      phase1Results,
      finalResults,
    );
    ...
    console.log(
      `   ✓ Revenue: ${phase3Results.business_impact.immediate_revenue} (${phase3Results.phase3_processing_time}ms)`,
    );
  }
...
async function runWorkflowIntelligence() {
  ...
  console.log(
    `   Average per Task: $${(stats.totalRevenue / stats.total).toFixed(2)}`,
  );
  console.log(`\n⏱️  Performance:`);
  console.log(
    `   Average Processing Time: ${(stats.avgProcessingTime / 1000).toFixed(1)}s`,
  );
