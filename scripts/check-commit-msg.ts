...
const conventionalCommitRegex =
  /^(feat|fix|docs|style|refactor|perf|test|build|ci|chore|revert|security)(\(.+\))?: .{1,100}$/;
const typeRegex = /^(\w+)(\(.+\))?:/;
...
if (!conventionalCommitRegex.test(firstLine)) {
  ...
  if (firstLine.length > 100) {
    console.log(
      `❌ First line too long (${firstLine.length} chars). Maximum: 100 chars\n`,
    );
  }
...
if (firstLine.startsWith("security")) {
  if (
    !commitMsg.toLowerCase().includes("cve") &&
    !commitMsg.toLowerCase().includes("vulnerability") &&
    !commitMsg.toLowerCase().includes("security")
  ) {
    issues.push(
      "Security commits should reference the vulnerability or security issue",
    );
  }
}
