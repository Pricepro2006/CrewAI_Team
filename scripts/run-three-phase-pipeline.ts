...
class ThreePhasePipeline {
  ...
  private setupEventHandlers(): void {
    ...
    this.batchProcessor.on('email:phase:complete', ({ phase, results }) => {
      logger.debug(`Phase ${phase} complete in ${results.processing_time || results.phase2_processing_time || results.phase3_processing_time}ms`);
    });
  ...
  private generateReport(
    ...
    // Financial impact
    const totalFinancialImpact = successful.reduce((sum, r) => 
      sum + (r.analysis?.financial_impact || 0), 0
    );
    ...
    console.log(`Average time per email: ${Math.round(totalTime / results.length)}ms`);
    console.log(`Throughput: ${(results.length / (totalTime / 1000)).toFixed(1)} emails/second`);
  ...
  async analyzeSpecificEmails(emailIds: string[]): Promise<void> {
    ...
    if (emails.length !== emailIds.length) {
      logger.warn(`Only found ${emails.length} of ${emailIds.length} requested emails`);
    }
    ...
    results.forEach(r => {
      ...
      if (r.analysis) {
        ...
        if (r.analysis.strategic_insights) {
          ...
          console.log(`  Opportunity: ${r.analysis.strategic_insights.opportunity}`);
...
async function main() {
  ...
  try {
    ...
    const specificIds = args.find(arg => arg.startsWith('--ids='))?.split('=')[1]?.split(',');
