...
const complexTestEmail = {
  ...
  subject:
    "URGENT: Multiple Issues - Order PO #45791234, Quote CAS892456, and Case #INC789012 Need Immediate Attention",
  body: `Dear Partner,
...
async function evaluateAnalysisQuality(analysis: any): Promise<any> {
  ...
  if (analysis?.entities) {
    ...
    let entityCount = 0;
    const expectedCount = 20; // Approximate expected entities
  ...
  if (analysis?.summary) {
    ...
    if (summary.includes("urgent") || summary.includes("critical"))
      evaluation.summaryQuality += 0.33;
    if (summary.includes("multiple") || summary.includes("several"))
      evaluation.summaryQuality += 0.33;
  ...
  if (analysis?.suggestedActions?.length > 0) {
    evaluation.actionsQuality = Math.min(
      1,
      analysis.suggestedActions.length / 4,
    );
  }
...
async function testModelDetailed(
  modelName: string,
  ...
  try {
    ...
    console.log("\n📂 Categories:");
    console.log(
      `  • Workflow: ${analysis.categories.workflow.join(", ") || "None"}`,
    );
    ...
    console.log(
      `  • Tracking: ${analysis.entities.trackingNumbers.join(", ") || "None"}`,
    );
    console.log(
      `  • Case Numbers: ${analysis.entities.caseNumbers.join(", ") || "None"}`,
    );
    console.log(
    ...
    console.log(
      `  • Amounts: ${analysis.entities.amounts.map((a) => `${a.currency} ${a.value}`).join(", ") || "None"}`,
    );
    ...
    console.log("\n📈 Quality Metrics:");
    console.log(
      `  • Categories Score: ${(result.evaluation.categoriesScore * 100).toFixed(0)}%`,
    );
    console.log(
      `  • Entity Extraction: ${(result.evaluation.entitiesScore * 100).toFixed(0)}%`,
    );
    console.log(`  • Confidence: ${(analysis.confidence * 100).toFixed(0)}%`);
    console.log(
      `  • Summary Quality: ${(result.evaluation.summaryQuality * 100).toFixed(0)}%`,
    );
    console.log(
      `  • Actions Quality: ${(result.evaluation.actionsQuality * 100).toFixed(0)}%`,
    );
    console.log(
      `  • Overall Score: ${(result.evaluation.overallScore * 100).toFixed(0)}%`,
    );
...
async function runDetailedEmailAnalysisTest() {
  ...
  console.log(
    `Models Tested: ${results.filter((r) => !r.error).length} of ${allModels.length}`,
  );
  console.log(`Test Email: Complex multi-issue email with 4 critical items`);

  // Filter out unavailable models
  const validResults = results.filter(
    (r) => !r.error || r.error !== "Model not available",
  );
  ...
  console.log("-".repeat(100));
  console.log(
    "Rank | Model         | Overall | Categories | Entities | Confidence | Summary | Actions | Time",
  );
  console.log("-".repeat(100));
  ...
  validResults.forEach((result, index) => {
    if (result.success) {
      ...
      const overall =
        `${(result.evaluation.overallScore * 100).toFixed(0)}%`.padStart(7);
      const categories =
        `${(result.evaluation.categoriesScore * 100).toFixed(0)}%`.padStart(10);
      const entities =
        `${(result.evaluation.entitiesScore * 100).toFixed(0)}%`.padStart(8);
      const confidence =
        `${(result.evaluation.confidenceScore * 100).toFixed(0)}%`.padStart(10);
      const summary =
        `${(result.evaluation.summaryQuality * 100).toFixed(0)}%`.padStart(7);
      const actions =
        `${(result.evaluation.actionsQuality * 100).toFixed(0)}%`.padStart(7);
  ...
  topModels.forEach((result, index) => {
    ...
    if (result.analysis) {
      ...
      console.log(
        `  Entities Found: ${Object.values(result.analysis.entities).flat().length} total`,
      );
      console.log(
        `  Actions Generated: ${result.analysis.suggestedActions.length}`,
      );
    }
  });
  ...
  const bestCategories = validResults.sort(
    (a, b) => b.evaluation.categoriesScore - a.evaluation.categoriesScore,
  )[0];
  const bestEntities = validResults.sort(
    (a, b) => b.evaluation.entitiesScore - a.evaluation.entitiesScore,
  )[0];
  const fastest = validResults
    .filter((r) => r.success)
    .sort((a, b) => a.processingTime - b.processingTime)[0];

  console.log(
    `🏆 Best Overall: ${bestOverall.model} (${(bestOverall.evaluation.overallScore * 100).toFixed(0)}% quality)`,
  );
  console.log(
    `📂 Best Categorization: ${bestCategories.model} (${(bestCategories.evaluation.categoriesScore * 100).toFixed(0)}%)`,
  );
  console.log(
    `🔍 Best Entity Extraction: ${bestEntities.model} (${(bestEntities.evaluation.entitiesScore * 100).toFixed(0)}%)`,
  );
  console.log(
    `⚡ Fastest Processing: ${fastest.model} (${(fastest.processingTime / 1000).toFixed(1)}s)`,
  );

  console.log("\n💡 RECOMMENDATIONS:");
  console.log("-".repeat(50));
  console.log(
    "• For production email analysis: Use the model with best overall score",
  );
  console.log(
    "• For real-time processing: Balance speed vs quality based on requirements",
  );
  console.log(
    "• For critical emails: Prioritize categorization and entity extraction accuracy",
  );
  console.log(
    "• Consider model ensemble for highest accuracy on complex emails",
  );
