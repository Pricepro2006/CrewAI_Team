...
class JsonParsingQualityValidator {
  ...
  async validateQualityRisk(
    ...
    for (const email of emails) {
      ...
      const fallbackQuality = this.scoreResponseQuality(
        fallbackResponse,
        email,
      );
      ...
      const qualityDifference =
        fallbackQuality.overallScore - llmQuality.overallScore;
      ...
      if (qualityDifference > 0.5) {
        ...
        console.log(
          `   Fallback Quality: ${fallbackQuality.overallScore.toFixed(2)}`,
        );
  ...
  private scoreResponseQuality(response: any, email: any): QualityMetrics {
    ...
    const sentimentAccuracy = this.scoreSentimentAnalysis(
      response.sentiment,
      email,
    );

    // Score context understanding
    const contextUnderstanding = this.scoreContextUnderstanding(
      response,
      email,
    );
    ...
    const overallScore =
      (entityAccuracy + sentimentAccuracy + contextUnderstanding) / 3;
  ...
  private async generateQualityReport(
    ...
    const reportPath = path.join(
      process.cwd(),
      "JSON_PARSING_QUALITY_VALIDATION_REPORT.md",
    );

    // Calculate statistics
    const totalTests = results.length;
    const criticalRisk = results.filter(
    ...
    const avgFallbackQuality =
      results.reduce((sum, r) => sum + r.fallbackQuality.overallScore, 0) /
      totalTests;
    const avgLlmQuality =
      results.reduce((sum, r) => sum + r.llmQuality.overallScore, 0) /
      totalTests;
    const avgQualityLoss =
      results.reduce((sum, r) => sum + r.qualityDifference, 0) / totalTests;
...
`;
  ...
  private analyzeSentimentRuleBased(content: string): string {
    ...
    if (content.includes("great") || content.includes("excellent"))
      return "positive";
...
async function main() {
  ...
  console.log(
    "This script validates the critical finding about JSON parsing quality risk.",
  );
  console.log("");
  ...
  try {
    ...
    console.log(
      "📄 Report location: ./JSON_PARSING_QUALITY_VALIDATION_REPORT.md",
    );
  } catch (error) {
