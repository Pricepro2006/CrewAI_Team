...
async function processEmails() {
  console.log(
    "🚀 Running ONLY Phase 3 (doomgrave/phi-4) Analysis on 20 Test Emails\n",
  );
  ...
  for (const [index, email] of testEmails.entries()) {
    ...
    try {
      ...
      const prompt = PHI4_PROMPT.replace(
        "Email to analyze:",
      ...
      const entityCount = Object.values(analysis.entities || {}).reduce(
      ...
      const existing = db
        .prepare("SELECT id, created_at FROM email_analysis WHERE email_id = ?")
        .get(emailId) as any;
  ...
  console.log(
    `Success rate: ${((successCount / testEmails.length) * 100).toFixed(1)}%`,
  );
  console.log(
    `Average processing time: ${(totalProcessingTime / successCount / 1000).toFixed(1)}s per email`,
  );
  console.log(
    `Total processing time: ${(totalProcessingTime / 1000).toFixed(1)}s`,
  );
