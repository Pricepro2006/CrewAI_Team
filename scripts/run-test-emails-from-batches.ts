...
const LLAMA_PROMPT = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, "../prompts/optimized/llama32_3b_prompt.json"),
    "utf-8",
  ),
).prompt;
...
async function analyzeEmail(email: any, phase: number) {
  ...
  try {
    ...
    const processingTime = Date.now() - startTime;
    console.error(
      `    ❌ Analysis failed after ${(processingTime / 1000).toFixed(1)}s`,
    );
    throw error;
  }
}
...
async function processEmails() {
  console.log(
    "🚀 Running Test Emails from Batch Files with Optimized Prompts\n",
  );
  ...
  for (const [index, email] of testEmails.entries()) {
    ...
    try {
      ...
      const quickAnalysis = {
        quick_workflow: email.Subject?.toLowerCase().includes("re:")
          ? "IN_PROGRESS"
          : "START_POINT",
        quick_priority: email.Subject?.toLowerCase().includes("urgent")
          ? "HIGH"
          : "MEDIUM",
        quick_intent: "REQUEST",
        quick_urgency: email.Subject?.toLowerCase().includes("asap")
          ? "HIGH"
          : "MEDIUM",
      ...
      const needsDeepAnalysis =
        quickAnalysis.quick_priority === "HIGH" ||
        quickAnalysis.quick_workflow === "IN_PROGRESS";

      let deepAnalysis = null;
      let finalAnalysis = null;

      if (needsDeepAnalysis || index < 10) {
        // Force deep analysis for first 10 emails
        // Phase 2: Llama 3.2:3b
        try {
          ...
          if (
            deepAnalysis.priority === "CRITICAL" ||
            deepAnalysis.priority === "HIGH" ||
            index < 5
          ) {
      ...
      const existing = db
        .prepare("SELECT id FROM email_analysis WHERE email_id = ?")
        .get(emailId) as any;
      ...
      stmt.run(
        ...
        finalAnalysis
          ? finalAnalysis.executive_summary || finalAnalysis.contextual_summary
          : null,
      ...
      if (deepAnalysis) {
        ...
        const entityCount = Object.values(deepAnalysis.entities || {}).reduce(
          (sum: number, arr: any) =>
      ...
      if (finalAnalysis) {
        ...
        console.log(
          `      - Business Impact: ${finalAnalysis.business_impact ? "Identified" : "None"}`,
        );
  ...
  samples.forEach((s: any) => {
    console.log(
      `- ${s.email_id}: Quick=${s.quick_priority}, Deep=${s.deep_priority}, Model=${s.final_model || "Phase2"}`,
    );
  });
