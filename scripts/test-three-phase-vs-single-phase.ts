...
class AnalysisComparator {
  ...
  constructor() {
    this.db = // Use connection pool instead: getDatabaseConnection().getDatabase() or executeQuery((db) => ...)'./data/crewai.db');
  ...
  async runSinglePhaseAnalysis(email: any): Promise<AnalysisResult> {
    ...
    try {
      console.log(
        chalk.yellow(
          `  Running single-phase analysis for email ${email.id}...`,
        ),
      );
...
Provide a comprehensive JSON response with all extracted information.`;
      ...
      logger.error("Single-phase analysis failed", "COMPARATOR", {
        error,
        emailId: email.id,
      });
      throw error;
  ...
  async runThreePhaseAnalysis(email: any): Promise<AnalysisResult> {
    ...
    try {
      console.log(
        chalk.cyan(`  Running three-phase analysis for email ${email.id}...`),
      );
      ...
      totalTokens += this.estimateTokens(
        phase1Prompt + JSON.stringify(phase1Results),
      );
      console.log(
        chalk.gray(`    Phase 1 completed in ${Date.now() - phase1Start}ms`),
      );
...
Provide enhanced understanding in JSON format.`;
      ...
      console.log(
        chalk.gray(`    Phase 2 completed in ${Date.now() - phase2Start}ms`),
      );
...
Provide actionable intelligence in JSON format.`;
      ...
      console.log(
        chalk.gray(`    Phase 3 completed in ${Date.now() - phase3Start}ms`),
      );

      // Combine all phase results
      const combinedResults = this.combinePhaseResults(
        phase1Results,
        phase2Results,
        phase3Results,
      );
      ...
      logger.error("Three-phase analysis failed", "COMPARATOR", {
        error,
        emailId: email.id,
      });
      throw error;
  ...
  async runPhase3Only(email: any): Promise<AnalysisResult> {
    ...
    try {
      console.log(
        chalk.magenta(
          `  Running Phase 3 only analysis for email ${email.id}...`,
        ),
      );
...
Provide actionable intelligence in JSON format.`;
      ...
      logger.error("Phase 3 only analysis failed", "COMPARATOR", {
        error,
        emailId: email.id,
      });
      throw error;
  ...
  compareResults(results: AnalysisResult[]): ComparisonMetrics {
    ...
    ["single-phase", "three-phase", "phase-3-only"].forEach((approach) => {
      ...
      metrics[approach] = {
        ...
        categoryAccuracy: this.calculateCategoryAccuracy(
          approachResults,
          results,
        ),
        actionItemsCompleteness:
          this.calculateActionCompleteness(approachResults),
  ...
  async runComparisonTest(limit: number = 20): Promise<void> {
    ...
    for (const [index, email] of emails.entries()) {
      ...
      try {
        ...
        console.error(
          chalk.red(`  Failed to analyze email ${email.id}:`, error),
        );
      }
    }
   ...
   */
  private printEmailComparison(
    emailId: string,
    single: AnalysisResult,
    three: AnalysisResult,
    ...
    console.log(
      chalk.gray(
        `  │ Entities Found  │ ${String(singleEntities).padEnd(12)} │ ${String(threeEntities).padEnd(12)} │ ${String(phase3Entities).padEnd(12)} │`,
      ),
    );
    ...
    console.log(
      chalk.gray(
        `  │ Process Time    │ ${String(single.processingTimeMs + "ms").padEnd(12)} │ ${String(three.processingTimeMs + "ms").padEnd(12)} │ ${String(phase3.processingTimeMs + "ms").padEnd(12)} │`,
      ),
    );
    ...
    console.log(
      chalk.gray(
        `  │ LLM Calls       │ ${String(single.llmCalls).padEnd(12)} │ ${String(three.llmCalls).padEnd(12)} │ ${String(phase3.llmCalls).padEnd(12)} │`,
      ),
    );
    ...
    console.log(
      chalk.gray(
        `  │ Action Items    │ ${String(single.actionItems.length).padEnd(12)} │ ${String(three.actionItems.length).padEnd(12)} │ ${String(phase3.actionItems.length).padEnd(12)} │`,
      ),
    );
  ...
  private generateComparisonReport(): void {
    ...
    console.log(
      `│ Entity Extraction       │ ${this.formatScore(metrics["single-phase"]?.entityExtractionAccuracy)} │ ${this.formatScore(metrics["three-phase"]?.entityExtractionAccuracy)} │ ${this.formatScore(metrics["phase-3-only"]?.entityExtractionAccuracy)} │`,
    );

    // Category Accuracy
    console.log(
      `│ Category Accuracy       │ ${this.formatScore(metrics["single-phase"]?.categoryAccuracy)} │ ${this.formatScore(metrics["three-phase"]?.categoryAccuracy)} │ ${this.formatScore(metrics["phase-3-only"]?.categoryAccuracy)} │`,
    );

    // Action Items
    console.log(
      `│ Action Completeness     │ ${this.formatScore(metrics["single-phase"]?.actionItemsCompleteness)} │ ${this.formatScore(metrics["three-phase"]?.actionItemsCompleteness)} │ ${this.formatScore(metrics["phase-3-only"]?.actionItemsCompleteness)} │`,
    );

    // Processing Time
    console.log(
      `│ Avg Processing Time     │ ${this.formatTime(metrics["single-phase"]?.processingTime)} │ ${this.formatTime(metrics["three-phase"]?.processingTime)} │ ${this.formatTime(metrics["phase-3-only"]?.processingTime)} │`,
    );

    // Resource Usage
    console.log(
      `│ Resource Usage          │ ${this.formatResource(metrics["single-phase"]?.resourceUsage)} │ ${this.formatResource(metrics["three-phase"]?.resourceUsage)} │ ${this.formatResource(metrics["phase-3-only"]?.resourceUsage)} │`,
    );
    ...
    // Overall Score
    console.log(
      `│ OVERALL SCORE           │ ${chalk.yellow(this.formatScore(metrics["single-phase"]?.overallScore))} │ ${chalk.green(this.formatScore(metrics["three-phase"]?.overallScore))} │ ${chalk.cyan(this.formatScore(metrics["phase-3-only"]?.overallScore))} │`,
    );
  ...
  private printKeyFindings(): void {
    ...
    const singlePhaseResults = this.results.filter(
      (r) => r.approach === "single-phase",
    );
    const threePhaseResults = this.results.filter(
      (r) => r.approach === "three-phase",
    );
    const phase3OnlyResults = this.results.filter(
      (r) => r.approach === "phase-3-only",
    );
    ...
    console.log(
      `   • Single-phase extracted an average of ${avgEntitiesSingle.toFixed(1)} entities per email`,
    );
    console.log(
      `   • Three-phase extracted an average of ${avgEntitiesThree.toFixed(1)} entities per email`,
    );
    console.log(
      `   • Phase-3-only extracted an average of ${avgEntitiesPhase3.toFixed(1)} entities per email`,
    );
    console.log(
      `   • Three-phase showed ${(((avgEntitiesThree - avgEntitiesSingle) / avgEntitiesSingle) * 100).toFixed(1)}% improvement over single-phase`,
    );

    // 2. Processing Efficiency
    const avgTimeSingle =
      singlePhaseResults.reduce((acc, r) => acc + r.processingTimeMs, 0) /
      singlePhaseResults.length;
    const avgTimeThree =
      threePhaseResults.reduce((acc, r) => acc + r.processingTimeMs, 0) /
      threePhaseResults.length;
    const avgTimePhase3 =
      phase3OnlyResults.reduce((acc, r) => acc + r.processingTimeMs, 0) /
      phase3OnlyResults.length;

    console.log(chalk.cyan("\n2. Processing Efficiency:"));
    console.log(`   • Single-phase: ${avgTimeSingle.toFixed(0)}ms average`);
    console.log(
      `   • Three-phase: ${avgTimeThree.toFixed(0)}ms average (${(avgTimeThree / avgTimeSingle).toFixed(1)}x slower)`,
    );
    console.log(
      `   • Phase-3-only: ${avgTimePhase3.toFixed(0)}ms average (fastest)`,
    );

    // 3. Action Item Quality
    const avgActionsSingle =
      singlePhaseResults.reduce((acc, r) => acc + r.actionItems.length, 0) /
      singlePhaseResults.length;
    const avgActionsThree =
      threePhaseResults.reduce((acc, r) => acc + r.actionItems.length, 0) /
      threePhaseResults.length;
    const avgActionsPhase3 =
      phase3OnlyResults.reduce((acc, r) => acc + r.actionItems.length, 0) /
      phase3OnlyResults.length;

    console.log(chalk.cyan("\n3. Action Item Generation:"));
    console.log(
      `   • Single-phase: ${avgActionsSingle.toFixed(1)} action items per email`,
    );
    console.log(
      `   • Three-phase: ${avgActionsThree.toFixed(1)} action items per email`,
    );
    console.log(
      `   • Phase-3-only: ${avgActionsPhase3.toFixed(1)} action items per email`,
    );
    ...
    if (avgEntitiesThree > avgEntitiesSingle * 1.2) {
      console.log(
        chalk.green(
          "   ✓ Three-phase approach significantly improves entity extraction",
        ),
      );
    }
    if (avgActionsThree > avgActionsSingle * 1.1) {
      console.log(
        chalk.green(
          "   ✓ Three-phase approach generates more comprehensive action items",
        ),
      );
    }
    if (avgTimeThree < avgTimeSingle * 2) {
      console.log(
        chalk.yellow(
          "   ⚠ Three-phase processing time is acceptable for the quality improvement",
        ),
      );
    }
    console.log(
      chalk.blue(
        "   → Use three-phase for critical emails requiring high accuracy",
      ),
    );
    console.log(
      chalk.blue(
        "   → Use phase-3-only for quick triage when speed is priority",
      ),
    );
  }
  ...
  private calculateAvgEntities(results: AnalysisResult[]): number {
    ...
    const total = results.reduce(
      (acc, r) => acc + this.countTotalEntities(r.extractedEntities),
      0,
    );
    return total / results.length;
  }

  private groupResultsByEmail(results: AnalysisResult[]): {
    [emailId: string]: AnalysisResult[];
  } {
  ...
  private calculateEntityAccuracy(
    ...
    approachResults.forEach((result) => {
      const baseline = threePhaseResults.find(
        (r) => r.emailId === result.emailId,
      );
      if (baseline) {
        const resultCount = this.countTotalEntities(result.extractedEntities);
        const baselineCount = this.countTotalEntities(
  ...
  private calculateCategoryAccuracy(
    ...
    const correctCategories = approachResults.filter(
      (r) => r.workflowCategory !== null,
    ).length;
  ...
  private calculateActionCompleteness(results: AnalysisResult[]): number {
    ...
    const avgActions =
      results.reduce((acc, r) => acc + r.actionItems.length, 0) /
      results.length;
  ...
  private calculateAvgProcessingTime(results: AnalysisResult[]): number {
    return (
      results.reduce((acc, r) => acc + r.processingTimeMs, 0) / results.length
    );
  }
  ...
  private calculateResourceUsage(results: AnalysisResult[]): number {
    ...
    const avgTokens =
      results.reduce((acc, r) => acc + r.tokensUsed, 0) / results.length;
    const avgCalls =
      results.reduce((acc, r) => acc + r.llmCalls, 0) / results.length;
  ...
  private parseComprehensiveResponse(response: string): any {
    try {
      ...
      if (jsonMatch) {
        ...
        return {
          entities: {
            ...
            productNumbers:
              parsed.productNumbers || parsed.product_numbers || [],
          },
      ...
      logger.error("Failed to parse comprehensive response", "COMPARATOR", {
        error,
      });
    }
  ...
  private combinePhaseResults(phase1: any, phase2: any, phase3: any): any {
    return {
      entities: {
        poNumbers: [
          ...(phase1.poNumbers || []),
          ...(phase2.additionalPoNumbers || []),
        ],
        quoteNumbers: [
          ...(phase1.quoteNumbers || []),
          ...(phase2.additionalQuoteNumbers || []),
        ],
        customers: [
          ...(phase1.customers || []),
          ...(phase2.additionalCustomers || []),
        ],
      ...
      category:
        phase3.workflowCategory || phase2.category || phase1.category || null,
      confidence: Math.max(
        phase1.confidence || 0,
        phase2.confidence || 0,
        phase3.confidence || 0,
      ...
      dollarValue:
        phase3.dollarValue || phase2.dollarValue || phase1.dollarValue || 0,
  ...
  private saveDetailedResults(): void {
    ...
    const filename = path.join(
      resultsDir,
      `phase_comparison_${timestamp}.json`,
    );
