...
function detectUrgency(text: string): any {
  ...
  return {
    level:
      found.length >= 2 ? "CRITICAL" : found.length === 1 ? "HIGH" : "NORMAL",
    indicators: found,
  };
}
...
Respond in JSON format with these fields:
{
  ...
  try {
    ...
    return {
      ...
      combined_confidence:
        (phase1Results.confidence + enhancement.confidence_score) / 2,
    };
  } catch (error) {
    ...
    return {
      ...
      enhancements: {
        ...
        task_status:
          phase1Results.urgency.level === "CRITICAL" ? "RED" : "YELLOW",
...
function createWorkflowTask(email: any, analysis: any): any {
  ...
  const task = {
    ...
    description: phase2Data.phase1_data.key_phrases
      .join(" | ")
      .substring(0, 500),
    ...
    sla_deadline: new Date(
      Date.now() + phase2Data.enhancements.sla_hours * 60 * 60 * 1000,
    ).toISOString(),
    ...
    confidence_score: phase3Data
      ? phase3Data.final_confidence
      : phase2Data.combined_confidence,

    // Strategic insights (if available)
    executive_summary:
      phase3Data?.strategic_insights?.executive_summary || null,
    revenue_impact:
      phase3Data?.strategic_insights?.revenue_impact?.immediate || 0,
    escalation_required:
      phase3Data?.strategic_insights?.escalation_required || false,
  };
...
async function runWorkflowTest() {
  ...
  for (let i = 0; i < emails.length; i++) {
    ...
    try {
      ...
      console.log(
        chalk.green(`    ✓ Complete (${phase1Results.processing_time}ms)`),
      );
      ...
      console.log(
        chalk.green(`    ✓ Complete (${phase2Results.processing_time}ms)`),
      );
      ...
      if (
        dollarValue > 50000 ||
        phase2Results.enhancements.task_status === "RED"
      ) {
        console.log(chalk.gray("  → Phase 3: Strategic analysis..."));
        const phase3Results = await phase3StrategicAnalysis(
          email,
          phase2Results,
        );
        if (!phase3Results.skipped) {
          ...
          stats.phase3Run++;
          console.log(
            chalk.green(`    ✓ Complete (${phase3Results.processing_time}ms)`),
          );
        } else {
      ...
      // Update stats
      stats.categoryBreakdown[task.workflow_category] =
        (stats.categoryBreakdown[task.workflow_category] || 0) + 1;
      ...
      console.log(
        chalk.gray(
          `     Status: ${chalk[task.task_status === "RED" ? "red" : task.task_status === "YELLOW" ? "yellow" : "green"](task.task_status)}`,
        ),
      );
      console.log(chalk.gray(`     Owner: ${task.current_owner}`));
      console.log(
        chalk.gray(`     Value: $${task.dollar_value.toLocaleString()}`),
      );
  ...
  console.log(
    chalk.gray(
      `  Phase 1 avg: ${(stats.phase1Time / stats.total).toFixed(0)}ms`,
    ),
  );
  console.log(
    chalk.gray(
      `  Phase 2 avg: ${(stats.phase2Time / stats.total).toFixed(0)}ms`,
    ),
  );
  console.log(
    chalk.gray(
      `  Phase 3 avg: ${stats.phase3Run > 0 ? (stats.phase3Time / stats.phase3Run).toFixed(0) : 0}ms`,
    ),
  );
  console.log(
    chalk.gray(
      `  Phase 3 run: ${stats.phase3Run}/${stats.total} emails (${((stats.phase3Run / stats.total) * 100).toFixed(0)}%)\n`,
    ),
  );
  ...
  for (const [category, count] of Object.entries(stats.categoryBreakdown)) {
    console.log(
      chalk.gray(
        `  ${category}: ${count} (${((count / stats.total) * 100).toFixed(0)}%)`,
      ),
    );
  }
  ...
  console.log(
    chalk.red(
      `  RED (Critical): ${stats.statusBreakdown.RED} (${((stats.statusBreakdown.RED / stats.total) * 100).toFixed(0)}%)`,
    ),
  );
  console.log(
    chalk.yellow(
      `  YELLOW (In Progress): ${stats.statusBreakdown.YELLOW} (${((stats.statusBreakdown.YELLOW / stats.total) * 100).toFixed(0)}%)`,
    ),
  );
  console.log(
    chalk.green(
      `  GREEN (On Track): ${stats.statusBreakdown.GREEN} (${((stats.statusBreakdown.GREEN / stats.total) * 100).toFixed(0)}%)`,
    ),
  );

  console.log(chalk.yellow("\nBusiness Impact:"));
  console.log(
    chalk.gray(`  Total Value Tracked: $${stats.totalValue.toLocaleString()}`),
  );
  console.log(
    chalk.gray(
      `  Average Value per Task: $${(stats.totalValue / stats.total).toFixed(0).toLocaleString()}`,
    ),
  );
  ...
  const outputPath = path.join(
    __dirname,
    "../test-results/workflow-test-20-emails.json",
  );
  ...
  console.log(
    chalk.yellow("\nPhase 1 extracted:"),
    Object.keys(
      exampleResult.analysis.phase1_data ||
        exampleResult.analysis.phase2_data.phase1_data,
    ).length,
    "data points",
  );
  console.log(
    chalk.yellow("Phase 2 received:"),
    "All Phase 1 data + original email",
  );
  console.log(
    chalk.yellow("Phase 2 added:"),
    Object.keys(
      exampleResult.analysis.phase2_data?.enhancements ||
        exampleResult.analysis.enhancements,
    ).length,
    "workflow insights",
  );
  if (exampleResult.analysis.phase === 3 && !exampleResult.analysis.skipped) {
    console.log(
      chalk.yellow("Phase 3 received:"),
      "All previous data + original email",
    );
