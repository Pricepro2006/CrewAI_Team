...
const distributionLists = JSON.parse(
  fs.readFileSync('/home/pricepro2006/iems_project/distribution_list.json', 'utf-8')
);
...
function analyzeEmailCoverage() {
  ...
  console.log(`   Emails TO Nick Paul's mailboxes: ${uniqueToAddresses} (${coverage.toFixed(1)}%)`);
  console.log(`   Emails FROM Nick Paul: ${fromEmails.count}`);
  console.log(`   Other emails in database: ${totalEmails.count - uniqueToAddresses - fromEmails.count}`);
  
  if (coverage < 50) {
    console.log('\n❌ WARNING: Low coverage detected!');
    console.log('   Less than 50% of emails are TO Nick Paul\'s managed mailboxes.');
    console.log('   This suggests:');
    console.log('   1. The extraction may have included general TD SYNNEX emails');
