...
function shouldUsePhi4(email: any, index: number): boolean {
  ...
  if (email.importance === 'high' || email.importance === 'critical') return true;
...
async function callLLM(model: string, prompt: string): Promise<any> {
  ...
  try {
    ...
    const elapsed = (Date.now() - startTime) / 1000;
    console.error(`❌ LLM error after ${elapsed.toFixed(1)}s: ${error.message}`);
...
async function analyzeBatch(batchNumber: number, startIndex: number = 0) {
  const batchFile = path.join(__dirname, `../data/email-batches/may-july-2025/batch_${batchNumber}.json`);
  ...
  for (let i = startIndex; i < emails.length; i++) {
    ...
    try {
      ...
      const prompt = usePhi4 
        ? PHI4_PROMPT.replace('Email to analyze:', `Email to analyze:${emailContent}`)
      ...
      if (analysis.risks?.revenue_at_risk) {
        const amount = parseFloat(analysis.risks.revenue_at_risk.replace(/[\$,]/g, ''));
      ...
      stmt.run(
        ...
        analysis.action_items?.map((a: any) => `${a.task} (${a.revenue_impact || 'N/A'})`).join('; ') || null,
        JSON.stringify(analysis.action_items || []),
        analysis.sla_status || 'ON_TRACK',
        // Business impact (enhanced)
        analysis.risks?.revenue_at_risk || analysis.opportunities?.upsell_potential || null,
    ...
    if (i % 10 === 0) {
      ...
      fs.writeFileSync(
        path.join(__dirname, `../data/email-batches/may-july-2025/enhanced_progress_batch_${batchNumber}.json`),
  ...
  return { successCount, failureCount, phi4Count, llamaCount, criticalCount, totalRevenue };
}
...
async function updateDashboard(batchResults: any) {
  ...
  console.log(`   Revenue impact: $${(insights.total_revenue_impact || 0).toLocaleString()}`);
...
async function analyzeAllBatches() {
  ...
  const summaryPath = path.join(__dirname, '../data/email-batches/may-july-2025/pull_summary.json');
  ...
  console.log(`Emails analyzed: ${totalResults.success + totalResults.failure}`);
  console.log(`Model split: ${totalResults.llama} llama (${(totalResults.llama/(totalResults.llama+totalResults.phi4)*100).toFixed(1)}%), ${totalResults.phi4} phi-4 (${(totalResults.phi4/(totalResults.llama+totalResults.phi4)*100).toFixed(1)}%)`);
  console.log(`Critical issues found: ${totalResults.critical}`);
  console.log(`Total revenue at risk: $${totalResults.revenue.toLocaleString()}`);
  console.log(`Processing time: ${(totalTime / 60).toFixed(1)} minutes`);
  console.log(`Average per email: ${(totalTime / totalResults.success).toFixed(1)}s`);
}
