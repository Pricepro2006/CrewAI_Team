...
async function pullMayJulyEmails() {
  ...
  console.log(
    `Creating ${batches.length} batches of ~${batchSize} emails each\n`,
  );
  ...
  const llamaCount = analysisMap.filter((a) =>
    a.model.includes("llama"),
  ).length;
  const phiCount = analysisMap.filter((a) => a.model.includes("phi")).length;

  console.log("\n📊 Analysis Distribution:");
  console.log(
    `   Llama 3.2:3b: ${llamaCount} emails (${((llamaCount / emails.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `   Phi-4:        ${phiCount} emails (${((phiCount / emails.length) * 100).toFixed(1)}%)`,
  );
  ...
  const summary = {
    ...
    importance_breakdown: emails.reduce(
      (acc, email) => {
