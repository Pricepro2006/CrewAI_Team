...
function analyzeQuality() {
  ...
  // Get test email IDs
  const testEmailIds = fs.readFileSync('/tmp/test_email_ids.txt', 'utf-8')
    .split('\n')
    .filter(id => id.trim());
  ...
  const workflowStates = analyses.map(a => a.deep_workflow_primary).filter(Boolean);
  const uniqueWorkflows = [...new Set(workflowStates)];
  console.log(`   - ${workflowStates.length}/${analyses.length} emails have workflow states`);
  ...
  const hasStartPoint = workflowStates.some(w => w?.includes('START') || w?.includes('New'));
  const hasInProgress = workflowStates.some(w => w?.includes('PROGRESS') || w?.includes('Processing'));
  const hasCompletion = workflowStates.some(w => w?.includes('COMPLET') || w?.includes('Resolved'));
  
  metrics.workflowAccuracy = (workflowStates.length / analyses.length) * 
    (hasStartPoint ? 1.2 : 1) * 
    (hasInProgress ? 1.2 : 1) * 
    (hasCompletion ? 1.1 : 1) * 8;
  ...
  analyses.forEach(analysis => {
    ...
    entityTypes.forEach(type => {
      if (analysis[type]) {
        const count = analysis[type].split(',').filter((e: string) => e.trim()).length;
        if (count > 0) {
          totalEntities += count;
          hasEntities = true;
        }
  ...
  console.log(`   - ${emailsWithEntities}/${analyses.length} emails have entities`);
  console.log(`   - Average: ${(totalEntities / analyses.length).toFixed(1)} entities per email`);
  
  metrics.entityExtraction = Math.min(10, (emailsWithEntities / analyses.length) * 10 * 
  ...
  const hasSatisfaction = analyses.filter(a => a.business_impact_satisfaction).length;
  const hasUrgency = analyses.filter(a => a.business_impact_urgency_reason).length;
  const hasSLA = analyses.filter(a => a.action_sla_status).length;
  
  console.log(`   - Revenue impact: ${hasRevenue}/${analyses.length}`);
  console.log(`   - Customer satisfaction: ${hasSatisfaction}/${analyses.length}`);
  console.log(`   - Urgency reasons: ${hasUrgency}/${analyses.length}`);
  console.log(`   - SLA status: ${hasSLA}/${analyses.length}`);
  
  metrics.businessContext = ((hasRevenue + hasSatisfaction + hasUrgency + hasSLA) / 
    (analyses.length * 4)) * 10;
  ...
  const avgConfidence = analyses.reduce((sum, a) => sum + (a.deep_confidence || 0), 0) / analyses.length;
  ...
  metrics.actionableInsights = ((hasActionSummary + hasActionDetails) / (analyses.length * 2)) * 
    avgConfidence * 10;
  ...
  const hasResponse = analyses.filter(a => a.suggested_response && a.suggested_response.length > 20).length;
  const hasContext = analyses.filter(a => a.contextual_summary && a.contextual_summary.length > 50).length;
  ...
  const sampleResponse = analyses.find(a => a.suggested_response)?.suggested_response;
  if (sampleResponse) {
    console.log(`   - Sample: "${sampleResponse.substring(0, 100)}..."`);
  }
  
  metrics.responseQuality = ((hasResponse + hasContext) / (analyses.length * 2)) * 10;
  
  console.log(`   ✅ Score: ${metrics.responseQuality.toFixed(1)}/10\n`);
  
  // Calculate overall score (weighted average)
  metrics.overall = (
    metrics.workflowAccuracy * 0.20 +
    metrics.entityExtraction * 0.25 +
    metrics.businessContext * 0.20 +
    metrics.actionableInsights * 0.20 +
    metrics.responseQuality * 0.15
  );
  
  // Performance metrics
  console.log('⚡ Performance Metrics:');
  const avgProcessingTime = analyses.reduce((sum, a) => sum + (a.deep_processing_time || 0), 0) / 
    analyses.length / 1000;
  console.log(`   - Average processing time: ${avgProcessingTime.toFixed(1)}s per email`);
  console.log(`   - Success rate: ${(analyses.length / testEmailIds.length * 100).toFixed(1)}%`);
  
  // Final summary
  console.log('\n' + '='.repeat(60));
  console.log('QUALITY ASSESSMENT SUMMARY');
  console.log('='.repeat(60));
  console.log(`Workflow Accuracy:    ${metrics.workflowAccuracy.toFixed(1)}/10 (20% weight)`);
  console.log(`Entity Extraction:    ${metrics.entityExtraction.toFixed(1)}/10 (25% weight)`);
  console.log(`Business Context:     ${metrics.businessContext.toFixed(1)}/10 (20% weight)`);
  console.log(`Actionable Insights:  ${metrics.actionableInsights.toFixed(1)}/10 (20% weight)`);
  console.log(`Response Quality:     ${metrics.responseQuality.toFixed(1)}/10 (15% weight)`);
  console.log('─'.repeat(60));
  console.log(`OVERALL SCORE:        ${metrics.overall.toFixed(1)}/10`);
  console.log(`Target Score:         7.75/10`);
  console.log(`Achievement:          ${(metrics.overall / 7.75 * 100).toFixed(1)}% of target`);
  console.log('='.repeat(60));
  ...
  const resultsPath = path.join(__dirname, '../test-results', `phase3_quality_analysis_${Date.now()}.json`);
