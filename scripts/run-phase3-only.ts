...
async function runPhase3Only() {
  ...
  try {
    ...
    stage3.setProgressCallback(async (count: number) => {
      ...
      console.log(`Progress: ${count}/20 emails processed (${(count/20*100).toFixed(0)}%)`);
    });
    ...
    console.log(`⏱️  Total time: ${totalTime.toFixed(1)}s (${(totalTime/20).toFixed(1)}s per email)`);
    console.log(`📊 Results Summary:`);
    console.log(`   - Emails processed: ${results.length}`);
    console.log(`   - Average quality score: ${(results.reduce((sum, r) => sum + r.qualityScore, 0) / results.length).toFixed(1)}/10`);
    console.log(`   - Primary model used: ${results.filter(r => !r.fallbackUsed).length} emails`);
    console.log(`   - Fallback model used: ${results.filter(r => r.fallbackUsed).length} emails`);
