...
const CLAUDE_WEIGHTS = {
  ...
  businessProcessing: 0.2,
  actionableInsights: 0.2,
  ...
  if (analysis.quick_priority) {
    ...
    const expectedPriority =
      subject.includes("urgent") || subject.includes("asap")
        ? "critical"
        : "medium";
    const priorityMatch = analysis.quick_priority
      .toLowerCase()
      .includes(expectedPriority);
...
async function scorePhase2Quality() {
  ...
  console.log(
    `Found ${analyses.length} Phase 2 analyses from ${testEmailIds.length} test emails\n`,
  );
  ...
  if (overallScore >= 6.56) {
    ...
    console.log(
      `❌ Below target by ${(6.56 - overallScore).toFixed(1)} points`,
    );
  }
  ...
  const resultsPath = path.join(
    __dirname,
    "../test-results",
    `phase2_claude_scoring_${Date.now()}.json`,
  );
  ...
  console.log(
    `llama3.2:3b         ${overallScore.toFixed(1)}/10   6.56/10  ${overallScore >= 6.56 ? "✅" : "❌"} ${overallScore >= 6.56 ? "Achieved" : `(-${(6.56 - overallScore).toFixed(1)})`}`,
  );
  console.log("─".repeat(60));
