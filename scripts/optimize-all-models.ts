...
async function loadBaselineResults(): Promise<any[]> {
  ...
  const baselinePath = path.join(__dirname, '../data/claude-baseline-results.json');
...
async function optimizeModel(
  ...
  try {
    ...
    logger.info(`Improvement: +${(result.score - modelConfig.currentScore).toFixed(2)} points`);
...
async function main() {
  ...
  try {
    ...
    if (testEmails.length < TEST_EMAIL_IDS.length) {
      logger.warn(`Only found ${testEmails.length}/${TEST_EMAIL_IDS.length} test emails`);
    }

    // Run optimization for each model
    const results = [];
    for (const modelConfig of MODEL_CONFIGS) {
      const result = await optimizeModel(modelConfig, testEmails, baselineResults);
      if (result) {
        results.push({
          model: modelConfig.name,
          originalScore: modelConfig.currentScore,
    ...
    // Save summary
    const summaryPath = path.join(__dirname, '../prompts/optimization-summary.json');
