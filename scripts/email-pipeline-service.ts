...
class EmailPipelineService extends EventEmitter {
  ...
  async start() {
    ...
    console.log(`   - Email polling: Every ${CONFIG.EMAIL_POLL_INTERVAL / 60000} minutes`);
    console.log(`   - Metric aggregation: Every ${CONFIG.METRIC_AGGREGATION_INTERVAL / 60000} minutes`);
    console.log(`   - SLA monitoring: Every ${CONFIG.SLA_CHECK_INTERVAL / 60000} minutes`);
  ...
  private startEmailIngestion() {
    ...
    cron.schedule('*/5 * * * *', async () => {
      ...
      try {
        ...
        console.log(`   ✓ Email check complete: ${totalNewEmails} new emails (${elapsed}s)`);
  ...
  private async fetchNewEmails(mailbox: string): Promise<any[]> {
    ...
    const newEmails = this.db.prepare(`
      SELECT * FROM emails 
  ...
  private async processBatch(emails: any[]) {
    ...
    for (const email of emails) {
      ...
      if (result.task.task_status === 'RED' || result.task.priority === 'CRITICAL') {
    ...
    const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
    console.log(`   ✓ Batch processed: ${tasks.length} tasks created (${elapsed}s)`);
  ...
  private async aggregateMetrics() {
    ...
    try {
      // Executive metrics
      const executiveMetrics = this.db.prepare(`
        SELECT 
      ...
      // Owner workload
      const ownerWorkload = this.db.prepare(`
        SELECT 
          current_owner,
      ...
      const trendData = this.db.prepare(`
        SELECT 
  ...
  private async checkSLADeadlines() {
    ...
    try {
      ...
      const approachingTasks = this.db.prepare(`
        SELECT 
          task_id,
          title,
          current_owner,
      ...
      // Find violated SLAs
      const violatedTasks = this.db.prepare(`
        SELECT 
          task_id,
          title,
          current_owner,
      ...
      if (approachingTasks.length > 0 || violatedTasks.length > 0) {
        console.log(`   ⚠️  ${approachingTasks.length} approaching, ${violatedTasks.length} violated`);
      } else {
  ...
  private async updateMetrics() {
    ...
    const quickMetrics = this.db.prepare(`
      SELECT 
