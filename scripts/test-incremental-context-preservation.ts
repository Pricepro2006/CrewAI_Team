...
const TEST_EMAIL = {
  id: "test_001",
  subject:
    "URGENT: Quote Request - ABC Corp - Server Upgrade Project - Need by EOD",
  body: `Hi Team,
...
async function demonstrateIncrementalAnalysis() {
  ...
  // =========================================
  console.log(
    chalk.yellow("📋 PHASE 1: Rule-Based Extraction (Pattern Matching)"),
  );
  console.log(chalk.gray("Input: Original email only"));
  console.log(
    chalk.gray(
      "Focus: Extract entities, detect urgency, basic categorization\n",
    ),
  );
  ...
  console.log(
    chalk.gray(
      "Extracted:",
      JSON.stringify(phase1Results, null, 2).substring(0, 200) + "...",
    ),
  );
  ...
  console.log(
    chalk.gray(
      "Focus: Validate extractions, add workflow intelligence, assess risk\n",
    ),
  );
...
Do NOT re-extract what Phase 1 already found. Focus on adding intelligence.`;
  ...
  console.log(
    chalk.gray(
      "Added insights:",
      Object.keys(phase2Results)
        .filter((k) => !phase1Results[k])
        .join(", "),
    ),
  );
  ...
  console.log(
    chalk.gray("Input: Original email + Phase 1 results + Phase 2 results"),
  );
  console.log(
    chalk.gray(
      "Focus: Executive insights, strategic recommendations, action planning\n",
    ),
  );
...
Build upon previous analyses, don't repeat them.`;
  ...
  const phase3Results = {
    ...
    executive_summary: {
      ...
      recommendation:
        "Approve 3% discount to win deal and secure Q2 opportunity",
      business_impact:
        "Winning could lead to $900K total revenue, losing risks account",
    },
  ...
  console.log(
    chalk.gray(
      "Added strategies:",
      Object.keys(phase3Results)
        .filter((k) => !phase2Results[k])
        .join(", "),
    ),
  );
  ...
  console.log(
    chalk.gray(`   - Body length: ${TEST_EMAIL.body.length} characters\n`),
  );

  console.log(chalk.cyan("2. Cumulative Information Growth:"));
  console.log(
    chalk.gray(
      `   - Phase 1 extracted: ${Object.keys(phase1Results).length} data points`,
    ),
  );
  console.log(
    chalk.gray(
      `   - Phase 2 total: ${Object.keys(phase2Results).length} data points (includes all Phase 1)`,
    ),
  );
  console.log(
    chalk.gray(
      `   - Phase 3 total: ${Object.keys(phase3Results).length} data points (includes all previous)\n`,
    ),
  );

  console.log(chalk.cyan("3. Workload Efficiency:"));
  console.log(
    chalk.gray(`   - Phase 1: Focused only on extraction (${phase1Time}ms)`),
  );
  console.log(
    chalk.gray(
      `   - Phase 2: Focused only on workflow intelligence (${phase2Time}ms)`,
    ),
  );
  console.log(
    chalk.gray(`   - Phase 3: Focused only on strategy (${phase3Time}ms)`),
  );
  console.log(
    chalk.gray(`   - Total time: ${phase1Time + phase2Time + phase3Time}ms\n`),
  );
  ...
  console.log(
    chalk.gray(
      `   - All Phase 1 data in Phase 2: ${phase1Keys.every((k) => phase2Keys.includes(k)) ? "✓" : "✗"}`,
    ),
  );
  console.log(
    chalk.gray(
      `   - All Phase 2 data in Phase 3: ${phase2Keys.every((k) => phase3Keys.includes(k)) ? "✓" : "✗"}`,
    ),
  );
  ...
  console.log(
    chalk.gray("   - NO INFORMATION LOST - each phase adds to previous\n"),
  );
  ...
  console.log(
    chalk.gray("   - High value, competitive, urgent → All 3 phases\n"),
  );

  console.log(chalk.yellow("This email qualified for all phases because:"));
  console.log(
    chalk.gray(
      `   - High value: $${phase1Results.entities.dollar_values[0].toLocaleString()}`,
    ),
  );
  console.log(chalk.gray(`   - Urgency: ${phase1Results.urgency.level}`));
  console.log(
    chalk.gray(
      `   - Competitive situation: ${phase1Results.competitors_mentioned.join(", ")}`,
    ),
  );
...
demonstrateIncrementalAnalysis()
  .then((finalResults) => {
    ...
    console.log(
      chalk.blue(
        "KEY INSIGHT: The three-phase approach preserves ALL information",
      ),
    );
    console.log(
      chalk.blue(
        "while reducing each phase's workload through focused analysis.\n",
      ),
    );
  })
  .catch(console.error);
