...
async function debugPipelineMetrics() {
  ...
  try {
    ...
    const emailsEnhanced = db.prepare("SELECT COUNT(*) as count FROM emails_enhanced").get() as { count: number };
    console.log(`emails_enhanced count: ${emailsEnhanced.count}`);
    
    // Check status distribution
    const statusDist = db.prepare("SELECT status, COUNT(*) as count FROM emails_enhanced GROUP BY status").all();
    console.log("\nStatus distribution:");
    ...
    try {
      const analysisCount = db.prepare("SELECT COUNT(*) as count FROM email_analysis").get() as { count: number };
      console.log(`✅ email_analysis count: ${analysisCount.count}`);
      
      // Check for the columns used in stage calculations
      const analysisColumns = db.prepare("PRAGMA table_info(email_analysis)").all();
      const analysisColumnNames = analysisColumns.map((col: any) => col.name);
      console.log(`  Has pipeline_stage: ${analysisColumnNames.includes('pipeline_stage')}`);
      console.log(`  Has analysis_timestamp: ${analysisColumnNames.includes('analysis_timestamp')}`);
      console.log(`  Has created_at: ${analysisColumnNames.includes('created_at')}`);
    } catch (e) {
    ...
    const sampleEnhanced = db.prepare("SELECT * FROM emails_enhanced LIMIT 1").get();
    console.log("\nSample from emails_enhanced table:");
    console.log(JSON.stringify(sampleEnhanced, null, 2));
    
    const sampleAnalysis = db.prepare("SELECT * FROM email_analysis LIMIT 1").get();
