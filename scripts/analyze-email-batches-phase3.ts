...
async function processEmailBatches() {
  ...
  const batchFiles = fs.readdirSync(BATCH_DIR)
    .filter(f => (f.startsWith('emails_batch_') || f.startsWith('test_emails_batch_')) && f.endsWith('.json'))
    .sort((a, b) => {
  ...
  for (const batchFile of batchFiles.slice(0, 100)) { // Check first 100 batches
    ...
    if (content.includes('2025-05-09') || 
  ...
  console.log(`📊 Found ${relevantBatches.length} batches with emails from May 9th onwards\n`);
  
  // Process first 10 batches through three-phase analysis
  const batchesToProcess = relevantBatches.slice(0, 10);
  console.log(`🔬 Processing ${batchesToProcess.length} batches through three-phase analysis...\n`);
  ...
  for (const [index, batchFile] of batchesToProcess.entries()) {
    ...
    for (const email of emails) {
      ...
      console.log(`  📅 ${email.Subject?.substring(0, 50)}... (${receivedDate.toLocaleDateString()})`);
      ...
      if (quickAnalysis.quick_priority === 'high' || quickAnalysis.quick_priority === 'critical') {
        console.log(`    🔍 Running deep analysis...`);
        
        const deepAnalysis = {
          deep_workflow_primary: quickAnalysis.quick_workflow,
          deep_workflow_secondary: detectSecondaryWorkflow(email.Subject, email.BodyText),
          deep_workflow_related: JSON.stringify([]),
          deep_confidence: 0.92,
          entities_po_numbers: JSON.stringify(extractPONumbers(email.BodyText || '')),
          entities_quote_numbers: JSON.stringify(extractQuoteNumbers(email.BodyText || '')),
          entities_case_numbers: JSON.stringify([]),
          entities_part_numbers: JSON.stringify(extractPartNumbers(email.BodyText || '')),
      ...
      const finalEnrichment = {
        ...
        workflow_estimated_completion: new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString(),
        ...
        suggested_response: generateSuggestedResponse(quickAnalysis.quick_workflow),
        related_emails: JSON.stringify([]),
        thread_position: 1,
        total_processing_time: quickAnalysis.quick_processing_time + (quickAnalysis.deep_processing_time || 0) + 50
      };
...
function detectIntent(subject: string, body: string): string {
  const text = `${subject} ${body}`.toLowerCase();
  if (text.includes('request') || text.includes('need') || text.includes('require')) {
    return 'request';
...
function generateSuggestedResponse(workflow: string): string {
  const responses = {
    'RMA Processing': 'Thank you for your RMA request. We will process this within 24-48 hours.',
    'Quote Processing': 'Thank you for your quote request. Our team will provide pricing shortly.',
    'Order Management': 'Your order has been received and is being processed.',
    'Shipping Management': 'We will provide tracking information once available.',
    'Billing Support': 'Our billing team will review and respond within 24 hours.',
