...
// Load prompts
const LLAMA_PROMPT = fs.readFileSync(
  path.join(__dirname, '../prompts/MASTER_PROMPTS_REFERENCE.md'), 'utf-8'
...
async function analyzeBatch(batchNumber: number, startIndex: number = 0) {
  ...
  const analysisMap = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../data/email-batches/may-july-2025/analysis_plan.json'), 'utf-8')
  ...
  for (let i = startIndex; i < emails.length; i += CONCURRENT_ANALYSES) {
    const chunk = emails.slice(i, Math.min(i + CONCURRENT_ANALYSES, emails.length));
    const chunkMaps = analysisMap.slice(i, Math.min(i + CONCURRENT_ANALYSES, emails.length));
    ...
    const promises = chunk.map(async (email: any, idx: number) => {
      ...
      try {
        ...
        successCount++;
        console.log(`      ✅ Email ${i + idx + 1}: ${model} (${(processingTime/1000).toFixed(1)}s)`);
    ...
    fs.writeFileSync(
      path.join(__dirname, `../data/email-batches/may-july-2025/progress_batch_${batchNumber}.json`),
...
async function analyzeAllBatches() {
  ...
  console.log(`⏱️  Estimated time: ${(summary.total_emails * 15 / 3600).toFixed(1)} hours\n`);
  ...
  console.log(`🔬 Processing first ${batchesToProcess} batches as demonstration...\n`);
  
  for (let batch = 1; batch <= batchesToProcess; batch++) {
    // Check if batch has progress
    const progressFile = path.join(__dirname, `../data/email-batches/may-july-2025/progress_batch_${batch}.json`);
    let startIndex = 0;
  ...
  console.log(`Full analysis of ${summary.batches} batches would take ~${(summary.total_emails * 15 / 3600).toFixed(1)} hours.`);
  
  // Save overall progress
  fs.writeFileSync(
    path.join(__dirname, '../data/email-batches/may-july-2025/analysis_progress.json'),
    JSON.stringify({
