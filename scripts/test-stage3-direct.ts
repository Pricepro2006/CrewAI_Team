...
async function testStage3Direct() {
  ...
  try {
    ...
    const testBatch = JSON.parse(
      fs.readFileSync(
        "/home/pricepro2006/CrewAI_Team/data/email-batches/test_emails_batch_1.json",
        "utf-8",
      ),
    );

    // Take first 3 emails
    const testEmails = testBatch
      .slice(0, 3)
    ...
    results.forEach((result: any, i: number) => {
      ...
      console.log(
        `  - Recommendations: ${result.recommendedActions.length} actions`,
      );
      console.log();
    });
  } catch (error) {
