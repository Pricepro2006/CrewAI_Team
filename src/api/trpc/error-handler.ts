      ...
      return new TRPCError({
        ...
        cause: {
          ...
          userMessage: {
            ...
            message: 'A required service is not responding. Please try again later.',
      ...
      return new TRPCError({
        ...
        cause: {
          ...
          userMessage: {
            ...
            action: 'Please try again with a smaller request or check your connection.',
          },
          context,
        },
      });
  ...
  return async (opts: {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (error instanceof TRPCError && retryableErrors.includes(error.code)) {
          lastError = error;
          
          if (attempt < maxRetries - 1) {
            logger.info(`Retrying tRPC procedure ${opts.path}, attempt ${attempt + 1}/${maxRetries}`);
  ...
  middleware = async (opts: {
    ...
    try {
      ...
      if (currentFailures >= this.threshold) {
        ...
        logger.error(`Circuit breaker for ${opts.path} opened after ${currentFailures} failures`);
      }
