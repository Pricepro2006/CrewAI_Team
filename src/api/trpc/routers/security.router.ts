import { z } from 'zod';
import { router, publicProcedure, csrfTokenProcedure } from '../enhanced-router';
import type { Context } from '../context';
import { logger } from '../../../utils/logger';
import { getCSRFStats } from '../../middleware/security';
...
export const securityRouter = router({
  ...
  getCSRFStats: publicProcedure
    .query(async ({ ctx }: { ctx: Context }) => {
      ...
      return {
        ...
        userTokenCounts: Array.from(stats.tokensByUser.entries()).map(([userId, count]) => ({
          userId,
          count,
        })),
      };
  ...
  testCSRFProtection: publicProcedure
    ...
    .mutation(async ({ input, ctx }: { input: { message: string }, ctx: Context }) => {
