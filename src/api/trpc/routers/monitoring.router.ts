...
export const monitoringRouter = router({
  ...
  healthDetailed: protectedProcedure
    .query(async ({ ctx }) => {
  ...
  thresholdViolations: protectedProcedure
    .query(async ({ ctx }) => {
      if (!ctx.user?.isAdmin) {
        throw new TRPCError({
          code: 'FORBIDDEN',
  ...
  forceHealthCheck: protectedProcedure
    .mutation(async ({ ctx }) => {
      if (!ctx.user?.isAdmin) {
        throw new TRPCError({
          code: 'FORBIDDEN',
  ...
  setPerformanceThreshold: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      performanceMonitor.setThreshold(input.name, input.warningMs, input.criticalMs);
