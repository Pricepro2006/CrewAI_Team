...
export class QueryPerformanceMonitor {
    ...
    this.monitors.forEach(monitor => {
      if (this.matchesPattern(entry.query, monitor.pattern)) {
        ...
        monitor.statistics.averageExecutionTime = 
          monitor.statistics.totalExecutionTime / monitor.statistics.totalQueries;
  ...
  private getTopSlowQueries(queries: QueryPerformanceEntry[], limit: number): SlowQueryInfo[] {
    return queries
      .filter(q => !q.error)
  ...
  private calculatePerformanceTrend(): PerformanceTrend {
    ...
    const olderQueries = this.getRecentQueries(20 * 60 * 1000)
      .filter(q => q.timestamp < Date.now() - 10 * 60 * 1000); // 10-20 minutes ago
  ...
  private generateHourlyStatistics(queries: QueryPerformanceEntry[]): HourlyStats[] {
    ...
    return Array.from(hourlyMap.entries())
      .map(([hour, hourQueries]) => ({
        ...
        averageTime: hourQueries.reduce((sum, q) => sum + q.executionTime, 0) / hourQueries.length,
  ...
  private analyzeQueryTypes(queries: QueryPerformanceEntry[]): QueryTypeBreakdown[] {
    ...
    return Array.from(typeMap.entries()).map(([type, typeQueries]) => ({
      ...
      averageTime: typeQueries.reduce((sum, q) => sum + q.executionTime, 0) / typeQueries.length,
  ...
  private generatePerformanceRecommendations(stats: PerformanceStatistics): string[] {
  ...
  private estimateResourceUtilization(queries: QueryPerformanceEntry[]): ResourceUtilization {
