...
export class UnifiedEmailService {
   ...
   */
  async processIncomingEmail(emailData: GraphEmailData): Promise<UnifiedEmailData> {
    ...
    try {
      ...
      const emailId = await this.emailRepository.createEmail({
        ...
        bodyHtml: emailData.body.contentType === 'html' ? emailData.body.content : undefined,
      ...
      if (enrichedEmail.entities) {
        const entityList = [
          ...enrichedEmail.entities.people.map(e => ({ type: 'person', value: e })),
          ...enrichedEmail.entities.organizations.map(e => ({ type: 'organization', value: e })),
          ...enrichedEmail.entities.products.map(e => ({ type: 'product', value: e })),
          ...enrichedEmail.entities.orderNumbers.map(e => ({ type: 'order', value: e })),
          ...enrichedEmail.entities.trackingNumbers.map(e => ({ type: 'tracking', value: e }))
        ];
  ...
  private async storeRawEmail(emailData: GraphEmailData): Promise<any> {
    return await this.emailStorage.storeEmail({
      messageId: emailData.id,
      subject: emailData.subject,
      bodyText: emailData.body.content,
      from: emailData.from.emailAddress.address,
  ...
  private transformToUnifiedFormat(email: any, options: any = {}): UnifiedEmailData {
    // Parse JSON fields if needed
    const recipients = typeof email.recipients === 'string' ? 
      JSON.parse(email.recipients) : email.recipients;
    const ccRecipients = email.cc_recipients ? 
      (typeof email.cc_recipients === 'string' ? JSON.parse(email.cc_recipients) : email.cc_recipients) : [];
    ...
    return {
      ...
      agentAssignment: options.includeAgentInfo !== false && (email.assigned_to || email.assignedTo) ? {
        agentId: email.assigned_to || email.assignedTo,
        agentName: email.agent_name || email.assignedTo || 'Unknown Agent',
        assignedAt: email.assigned_at || email.assignedAt || new Date().toISOString(),
        status: email.assignment_status || 'assigned',
        progress: email.assignment_progress
      } : undefined,
      hasAttachments: Boolean(email.has_attachments !== undefined ? email.has_attachments : email.hasAttachments),
  ...
  private async getBaseAnalytics(dateRange?: { start: Date; end: Date }): Promise<any> {
  ...
  private async updateWorkflowChain(chainId: string, email: UnifiedEmailData): Promise<void> {
