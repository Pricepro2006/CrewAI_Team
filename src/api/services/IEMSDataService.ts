...
export class IEMSDataService {
  ...
  private readonly EMAIL_BATCHES_PATH = path.join(this.IEMS_BASE_PATH, 'db_backups/email_batches');
  private readonly MAILBOXES_FILE = path.join(this.IEMS_BASE_PATH, 'mailboxes.json');
  private readonly DISTRIBUTION_LIST_FILE = path.join(this.IEMS_BASE_PATH, 'distribution_list.json');
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.error('Failed to initialize IEMS Data Service', 'IEMS_DATA', error as Record<string, any>);
  ...
  async loadEmailBatches(batchNumbers?: number[]): Promise<RawIEMSEmail[]> {
    ...
    try {
      ...
      logger.error('Failed to load email batches', 'IEMS_DATA', error as Record<string, any>);
      throw error;
    }
  }
  ...
  async generateSummary(email: RawIEMSEmail): Promise<string> {
    ...
    try {
      ...
      logger.error('Failed to generate AI summary', 'IEMS_DATA', error as Record<string, any>);
   ...
   */
  private parseRecipients(recipientsStr: string): { to: string[], cc: string[] } {
    try {
   ...
   */
  private getAssignedTo(email: RawIEMSEmail, category: EmailCategory): string | undefined {
   ...
   */
  private getActionForEmail(email: RawIEMSEmail, category: EmailCategory): string | undefined {
  ...
  private getPriority(email: RawIEMSEmail): 'high' | 'medium' | 'low' {
    ...
    if (analysisPriority === 'urgent' || analysisPriority === 'high') return 'high';
