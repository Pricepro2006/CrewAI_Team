...
export class JWTManager {
  ...
  constructor() {
    ...
    if (!this.secret || this.secret === 'dev-secret-key-change-in-production') {
  ...
  generateTokenPair(user: {
    ...
    username: string;
    role: string;
  }, refreshTokenId: string): TokenPair {
  ...
  verifyAccessToken(token: string): JWTPayload {
    try {
      ...
      if (!decoded.sub || !decoded.email || !decoded.username || !decoded.role) {
