  ...
  list: publicProcedure
    .input(
      z.object({
        ...
        sort: z
          .object({
            field: z
              .enum(["created_at", "sla_deadline", "dollar_value", "priority"])
              .default("created_at"),
    ...
    .query(async ({ input, ctx }) => {
      ...
      try {
        ...
        // Get total count
        let countQuery =
          "SELECT COUNT(*) as total FROM workflow_tasks WHERE 1=1";
  ...
  update: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        // TODO: Emit WebSocket event when wsManager is available in context
        // Currently disabled as wsManager is not part of the tRPC context

        logger.info("Workflow task updated", "WORKFLOW", {
          taskId: input.taskId,
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        const slaDeadline = new Date(
          Date.now() + input.sla_hours * 60 * 60 * 1000,
        ).toISOString();
        ...
        logger.info("Workflow task created", "WORKFLOW", {
          taskId,
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        db.prepare("DELETE FROM workflow_status_history WHERE task_id = ?").run(
          input.taskId,
        );
        db.prepare("DELETE FROM workflow_tasks WHERE task_id = ?").run(
          input.taskId,
        );

        logger.info("Workflow task deleted", "WORKFLOW", {
          taskId: input.taskId,
        });
