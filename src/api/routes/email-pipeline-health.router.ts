...
import { authenticateJWT as requireAuth, optionalAuthenticateJWT } from '../middleware/auth.js';
...
const HealthCheckQuerySchema = z.object({
  ...
  services: z.array(z.enum(['database', 'redis', 'ollama', 'pipeline', 'processingQueue'])).optional()
});

// Middleware to track API performance
router.use((req, res, next) => {
  const endTimer = metrics.startTimer('email_pipeline_health.api_request_duration', {
    endpoint: req.path,
    method: req.method
  });
...
router.get('/email-pipeline', async (req, res) => {
  ...
  try {
    ...
    logger.info("Email pipeline health check requested", "EMAIL_PIPELINE_HEALTH", {
      status: healthStatus.status,
      forced: query.force,
      responseTime: Date.now() - startTime
    });
...
router.get('/email-pipeline/detailed', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const detailedResponse = {
      ...
      cacheInfo: {
        cached: !query.force,
        cacheAge: query.force ? 0 : Date.now() - new Date(healthStatus.timestamp).getTime()
      }
    };
    ...
    logger.info("Detailed email pipeline health check requested", "EMAIL_PIPELINE_HEALTH", {
      status: healthStatus.status,
      forced: query.force,
      responseTime: Date.now() - startTime,
    ...
    logger.error("Detailed email pipeline health check failed", "EMAIL_PIPELINE_HEALTH", error as Error);
  ...
  try {
    ...
    if (query.includeDetails) {
      ...
      response.metrics = {
        ...
        serviceDetails: healthStatus.services
      };
    }
    ...
    logger.error("Email pipeline metrics request failed", "EMAIL_PIPELINE_HEALTH", error as Error);
...
router.post('/email-pipeline/check', requireAuth, async (req, res) => {
  ...
  try {
    logger.info("Forced email pipeline health check initiated", "EMAIL_PIPELINE_HEALTH", {
      userId: (req as any).user?.id
    });
    ...
    logger.info("Forced email pipeline health check completed", "EMAIL_PIPELINE_HEALTH", {
      status: healthStatus.status,
      responseTime: Date.now() - startTime,
      userId: (req as any).user?.id
    });
    ...
    logger.error("Forced email pipeline health check failed", "EMAIL_PIPELINE_HEALTH", error as Error);
 ...
 */
router.get('/email-pipeline/services/:service', requireAuth, async (req, res) => {
  const startTime = Date.now();
  ...
  try {
    ...
    const statusCode = serviceHealth.status === 'healthy' ? 200 :
                      serviceHealth.status === 'degraded' ? 503 : 500;

    logger.info("Individual service health check requested", "EMAIL_PIPELINE_HEALTH", {
      service,
    ...
    logger.error("Individual service health check failed", "EMAIL_PIPELINE_HEALTH", error as Error);
...
router.get('/email-pipeline/history', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logger.info("Email pipeline health history requested", "EMAIL_PIPELINE_HEALTH", {
      limit,
      offset,
      responseTime: Date.now() - startTime,
      userId: (req as any).user?.id
    ...
    logger.error("Email pipeline health history request failed", "EMAIL_PIPELINE_HEALTH", error as Error);
...
router.delete('/email-pipeline/cache', requireAuth, async (req, res) => {
  try {
    ...
    logger.info("Email pipeline health cache cleared", "EMAIL_PIPELINE_HEALTH", {
      userId: (req as any).user?.id
    });
    ...
    logger.error("Failed to clear email pipeline health cache", "EMAIL_PIPELINE_HEALTH", error as Error);
