  ...
  getCategorizedEmails: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const categorizedEmails = await iemsDataService.getCategorizedEmails(limit);
        
        return categorizedEmails;
      } catch (error) {
        logger.error('Failed to get categorized emails', 'IEMS_ROUTER', error as Record<string, any>);
  ...
  updateEmailStatus: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        logger.error('Failed to update email status', 'IEMS_ROUTER', error as Record<string, any>);
  ...
  assignEmail: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        logger.error('Failed to assign email', 'IEMS_ROUTER', error as Record<string, any>);
  ...
  performEmailAction: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        logger.error('Failed to perform email action', 'IEMS_ROUTER', error as Record<string, any>);
  ...
  getEmailSummary: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        logger.error('Failed to get email summary', 'IEMS_ROUTER', error as Record<string, any>);
  ...
  getTeamMembers: publicProcedure
    .query(async () => {
      try {
        ...
        return [
          { id: '1', name: 'Nick Paul', email: 'Nick.Paul@TDSynnex.com', available: true },
          { id: '2', name: 'Sarah Johnson', email: 'Sarah.Johnson@TDSynnex.com', available: true },
          { id: '3', name: 'Mike Chen', email: 'Mike.Chen@TDSynnex.com', available: false },
          { id: '4', name: 'Lisa Williams', email: 'Lisa.Williams@TDSynnex.com', available: true }
        ];
      } catch (error) {
        logger.error('Failed to get team members', 'IEMS_ROUTER', error as Record<string, any>);
    ...
    .query(async () => {
      try {
        ...
        return {
          ...
          byStatus: {
            red: [...categorizedEmails.emailAlias, ...categorizedEmails.marketingSplunk, ...categorizedEmails.vmwareTDSynnex]
              .filter(e => e.status === 'red').length,
            yellow: [...categorizedEmails.emailAlias, ...categorizedEmails.marketingSplunk, ...categorizedEmails.vmwareTDSynnex]
              .filter(e => e.status === 'yellow').length,
            green: [...categorizedEmails.emailAlias, ...categorizedEmails.marketingSplunk, ...categorizedEmails.vmwareTDSynnex]
              .filter(e => e.status === 'green').length
          },
          avgResponseTime: 0, // TODO: Calculate from data
          urgentCount: [...categorizedEmails.emailAlias, ...categorizedEmails.marketingSplunk, ...categorizedEmails.vmwareTDSynnex]
            .filter(e => e.status === 'red').length,
          pendingAssignments: categorizedEmails.marketingSplunk.filter(e => !e.assignedTo).length
        };
      } catch (error) {
        logger.error('Failed to get analytics', 'IEMS_ROUTER', error as Record<string, any>);
