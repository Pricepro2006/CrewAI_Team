...
export function requestTracking(
  ...
  res.on("finish", () => {
    ...
    metricsCollector.histogram(
      "http_request_cpu_us",
      endCpuUsage.user + endCpuUsage.system,
      {
        method: req.method,
        route: req.route?.path || req.path,
      },
    ...
    if (res.statusCode >= 400) {
      ...
      errorTracker.trackError(
        new Error(`HTTP ${res.statusCode}: ${errorString}`),
        errorContext,
        severity,
        true,
...
export function requestSizeTracking(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
...
export function trackWebSocketMessage(
