...
describe('Security Headers Middleware', () => {
  ...
  describe('createOriginValidationMiddleware', () => {
    test('should allow same-origin requests', () => {
      const middleware = createOriginValidationMiddleware(['https://example.com']);
      const req = createMockRequest({
    ...
    test('should allow whitelisted origins', () => {
      const middleware = createOriginValidationMiddleware(['https://allowed.com']);
      const req = createMockRequest({
  ...
  describe('testSecurityHeaders', () => {
    ...
    test('should fail with missing headers', () => {
      ...
      expect(result.issues).toContain('Missing required header: Content-Security-Policy');
      expect(result.issues).toContain('Missing required header: X-Content-Type-Options');
    });
    ...
    test('should fail with unsafe CSP in production', () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.issues).toContain("CSP contains 'unsafe-inline' in production");
      expect(result.issues).toContain("CSP contains 'unsafe-eval' in production");
    });
