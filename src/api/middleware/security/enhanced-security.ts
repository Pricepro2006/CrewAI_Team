 ...
 */
export function createEnhancedInputValidation<T extends z.ZodTypeAny>(
  schema: T,
) {
  return async (opts: {
    ctx: Context;
...
export function createEnhancedSecurityAuditMiddleware() {
  return async (opts: {
    ...
    logger.debug(
      "tRPC Security Audit - Request Start",
      "SECURITY_AUDIT",
      securityContext,
    );
    ...
    try {
      ...
      const errorContext = {
        ...
        isSecurityError:
          error instanceof SqlInjectionError ||
          (error instanceof TRPCError &&
            ["UNAUTHORIZED", "FORBIDDEN", "TOO_MANY_REQUESTS"].includes(
              error.code,
            )),
      };
      ...
      if (errorContext.isSecurityError) {
        logger.error(
          "tRPC Security Audit - Security Violation",
          "SECURITY_AUDIT",
          errorContext,
        );
      } else {
        logger.warn(
          "tRPC Security Audit - Request Failed",
          "SECURITY_AUDIT",
          errorContext,
        );
      }
  ...
  return async (opts: {
    ...
    if (input && typeof input === "object") {
      try {
        ...
        if (error instanceof SqlInjectionError) {
          logger.error(
            "Security Test Failed - SQL Injection Detected",
            "SECURITY_TEST",
            {
              userId: ctx.user?.id,
              requestId: ctx.requestId,
