  ...
  return {
    ...
    csp: {
      ...
      connectSrc: [
        ...
        "https://localhost:*",
        ...(isProduction ? ["wss://*.crewai-team.com", "https://*.crewai-team.com"] : []),
      ...
      frameSrc: ["'none'"],
      workerSrc: ["'self'", "blob:"],
      mediaSrc: ["'self'", "blob:", "data:"], // For any media elements
  ...
  return cors({
    origin: function (origin, callback) {
      ...
      if (config.cors.origins.includes(origin)) {
        ...
        logger.warn('CORS: Blocked request from unauthorized origin', 'SECURITY', {
          origin,
 ...
 */
export function applySecurityHeaders(app: any, customConfig?: Partial<SecurityHeadersConfig>) {
  const config = { ...getSecurityHeadersConfig(), ...customConfig };
