...
async function main() {
  ...
  try {
    ...
    logger.info('Before migration:', {
      ...
      percentageNegative: ((beforeStats.negative_records / beforeStats.total_records) * 100).toFixed(2) + '%',
    ...
    if (beforeStats.negative_records === 0) {
      logger.info('No negative processing times found. Database is already clean!');
      return;
    }
    ...
    logger.info('Example negative records:', examples.map(e => ({
    ...
    // 7. Summary report
    logger.info('\n=== MIGRATION SUMMARY ===');
    logger.info(`Fixed ${beforeStats.negative_records} records with negative processing times`);
    logger.info(`Database now has ${afterStats.negative_records} negative records (should be 0)`);
    logger.info('Triggers installed to prevent future negative values');
    logger.info('Backup data preserved in email_analysis_backup_neg_times table');
    logger.info('=========================\n');
