...
class KnowledgeBasePopulator {
  ...
  async populateKnowledgeBase(): Promise<void> {
    ...
    try {
      ...
      const knowledgeBaseDir = join(
        process.env.HOME || "/home/pricepro2006",
        "master_knowledge_base/documents",
      );
  ...
  private async processDirectory(dirPath: string): Promise<void> {
    try {
      ...
      console.log(
        `📋 Found ${this.stats.totalFiles} markdown files to process`,
      );
      console.log("");
  ...
  private printSummary(): void {
    ...
    console.log(
      `⏱️  Processing time:      ${(this.stats.processingTime / 1000).toFixed(2)}s`,
    );
    ...
    if (
      this.stats.processedFiles === this.stats.totalFiles &&
