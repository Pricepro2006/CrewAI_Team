...
class MigrationRunner {
  ...
  async runMigrations(targetVersion?: number): Promise<void> {
    try {
      ...
      logger.info(
        `Current database version: ${currentVersion || "none"}`,
        "MIGRATION_RUNNER",
      );
    ...
    try {
      ...
      for (const migration of migrationsToRollback) {
        ...
        try {
          ...
          logger.info(
            `✓ Rollback ${migration.version} completed in ${Date.now() - startTime}ms`,
            "MIGRATION_RUNNER",
          );
        } catch (error) {
          logger.error(
            `✗ Rollback ${migration.version} failed: ${error}`,
            "MIGRATION_RUNNER",
          );
          throw error;
        }
  ...
  try {
    switch (command) {
      ...
      default:
        console.log("Usage:");
        console.log(
          "  npm run db:migrate [version]    - Run migrations up to version",
        );
