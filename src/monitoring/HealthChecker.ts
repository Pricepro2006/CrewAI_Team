...
export class HealthChecker extends EventEmitter {
  ...
  startHealthChecks(intervalMs: number = 30000): void {
    ...
    logger.info(`Health checks started with ${intervalMs}ms interval`, 'HEALTH_CHECKER');
  }
  ...
  async runCheck(name: string, check: HealthCheck): Promise<void> {
    ...
    try {
      ...
      const consecutiveFailures = result.status === 'healthy' ? 0 : 
      ...
      // Record metrics
      metricsCollector.gauge(`health_${name}_status`, 
      ...
      if (consecutiveFailures >= this.maxConsecutiveFailures && check.critical) {
        this.emit('service-down', { service: name, failures: consecutiveFailures });
      }
      ...
      // Log error
      logger.error(`Health check failed: ${name}`, 'HEALTH_CHECKER', { error: errorMessage });
      ...
      check: async () => {
        try {
          ...
          return {
            ...
            message: response.ok ? undefined : `ChromaDB returned status ${response.status}`
          };
        } catch (error) {
          return {
            status: 'unhealthy',
            message: error instanceof Error ? error.message : 'ChromaDB connection failed'
          };
        }
      }
    });
  ...
  private updateOverallHealth(): void {
    ...
    // Record metrics
    metricsCollector.gauge('health_overall_status', 
      overall.status === 'healthy' ? 1 : overall.status === 'degraded' ? 0.5 : 0
    );
    metricsCollector.gauge('health_services_healthy', overall.healthyServices);
    metricsCollector.gauge('health_services_degraded', overall.degradedServices);
    metricsCollector.gauge('health_services_unhealthy', overall.unhealthyServices);
