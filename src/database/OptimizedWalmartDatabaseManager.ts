...
export class OptimizedWalmartDatabaseManager {
  private static instance: OptimizedWalmartDatabaseManager;
  private connectionPool: OptimizedConnectionPool;
  private isInitialized: boolean = false;
  private config: WalmartDatabaseConfig;
  ...
  private constructor(config?: Partial<WalmartDatabaseConfig>) {
    ...
    logger.info(
      `Optimized Walmart Database Manager initialized with connection pool`,
      "WALMART_DB_OPTIMIZED",
      {
        dbPath,
        poolConfig: {
  ...
  static getInstance(config?: Partial<WalmartDatabaseConfig>): OptimizedWalmartDatabaseManager {
  ...
  get walmartProducts(): WalmartProductRepository {
    if (!this._walmartProducts) {
      this._walmartProducts = new WalmartProductRepository(this.connectionPool);
    }
  ...
  get substitutions(): SubstitutionRepository {
    if (!this._substitutions) {
      this._substitutions = new SubstitutionRepository(this.connectionPool);
    }
  ...
  get userPreferences(): UserPreferencesRepository {
    if (!this._userPreferences) {
      this._userPreferences = new UserPreferencesRepository(this.connectionPool);
    }
  ...
  get groceryLists(): GroceryListRepository {
    if (!this._groceryLists) {
      this._groceryLists = new GroceryListRepository(this.connectionPool);
    }
  ...
  get groceryItems(): GroceryItemRepository {
    if (!this._groceryItems) {
      this._groceryItems = new GroceryItemRepository(this.connectionPool);
    }
  ...
  get shoppingSessions(): ShoppingSessionRepository {
    if (!this._shoppingSessions) {
      this._shoppingSessions = new ShoppingSessionRepository(this.connectionPool);
    }
  ...
  private async seedSampleData(): Promise<void> {
    try {
      ...
      const productCount = await this.connectionPool.executeQuery(
        "SELECT COUNT(*) as count FROM walmart_products"
      ) as { count: number }[];
      
      const count = productCount?.[0]?.count ?? 0;
      if (count > 0) {
        logger.info(`Optimized Walmart database already has ${count} products`, "WALMART_DB_OPTIMIZED");
        return;
      }
      ...
      await this.connectionPool.executeTransaction((db: any) => {
        ...
        for (const product of sampleProducts) {
          insertStmt.run(
            product.product_id, product.name, product.brand, product.description,
            product.category_path, product.department, product.current_price,
            product.regular_price, product.unit_price, product.unit_measure,
            product.in_stock, product.stock_level, product.search_keywords
          );
        }
      ...
      logger.info(`Inserted ${sampleProducts.length} sample products into optimized Walmart database`, "WALMART_DB_OPTIMIZED");
    } catch (error) {
  ...
  async executeQuery<T>(query: string, params: any[] = []): Promise<T> {
    return this.connectionPool.executeQuery<T>(query, params);
  }

  /**
   * Execute transaction with automatic rollback on error
   */
  async executeTransaction<T>(callback: (db: any) => T): Promise<T> {
    return this.connectionPool.executeTransaction<T>(callback);
  }
...
export function getOptimizedWalmartDatabaseManager(): OptimizedWalmartDatabaseManager {
