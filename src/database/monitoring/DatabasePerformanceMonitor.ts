...
export class DatabasePerformanceMonitor extends EventEmitter {
   ...
   */
  private recordQuery(query: string, duration: number, rowsAffected: number, rowsReturned: number): void {
  ...
  private generateRecommendations(): string[] {
    ...
    const slowQueryRatio = this.slowQueryLog.length / Math.max(this.queryHistory.length, 1);
    if (slowQueryRatio > 0.1) {
      recommendations.push('More than 10% of queries are slow. Review query optimization');
    }

    const distribution = this.getQueryDistribution();
    if (distribution.SELECT > distribution.INSERT * 10) {
      recommendations.push('Read-heavy workload detected. Consider caching strategies');
    }
