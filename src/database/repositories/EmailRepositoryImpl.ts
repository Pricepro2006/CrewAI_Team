...
import { EmailRecord, AnalysisStatus, EmailPriority, EmailFolder } from '../../types/EmailTypes.js';
 ...
 */
export class EmailRepositoryImpl extends BaseRepository<EmailRecord> implements IEmailRepository {
  constructor() {
  ...
  protected mapEntityToRow(entity: Partial<EmailRecord>): any {
    ...
    if (entity.from_address !== undefined) row.from_address = entity.from_address;
    if (entity.to_addresses !== undefined) row.to_addresses = entity.to_addresses;
    if (entity.cc_addresses !== undefined) row.cc_addresses = entity.cc_addresses;
    if (entity.bcc_addresses !== undefined) row.bcc_addresses = entity.bcc_addresses;
    if (entity.received_time !== undefined) row.received_time = entity.received_time.toISOString();
    if (entity.sent_time !== undefined) row.sent_time = entity.sent_time?.toISOString();
    ...
    if (entity.has_attachments !== undefined) row.has_attachments = entity.has_attachments ? 1 : 0;
    ...
    if (entity.analyzed_at !== undefined) row.analyzed_at = entity.analyzed_at?.toISOString();
    if (entity.error_message !== undefined) row.error_message = entity.error_message;
   ...
   */
  async findByStatus(status: AnalysisStatus, limit?: number, offset?: number): Promise<EmailRecord[]> {
    return executeQuery((db) => {
   ...
   */
  async findPendingAnalysis(limit: number, offset: number): Promise<EmailRecord[]> {
    return executeQuery((db) => {
      const stmt = db.prepare(`
        SELECT * FROM ${this.tableName}
        WHERE status IN (?, ?)
        ...
        LIMIT ? OFFSET ?
      `);
      const rows = stmt.all(AnalysisStatus.PENDING, AnalysisStatus.ANALYZING, limit, offset);
      return rows.map(row => this.mapRowToEntity(row));
    });
  }
  ...
  async findByMessageId(messageId: string): Promise<EmailRecord | null> {
    return executeQuery((db) => {
      const stmt = db.prepare(`SELECT * FROM ${this.tableName} WHERE message_id = ?`);
   ...
   */
  async findByDateRange(startDate: Date, endDate: Date, limit?: number): Promise<EmailRecord[]> {
    return executeQuery((db) => {
      let query = `
        SELECT * FROM ${this.tableName}
   ...
   */
  async updateAnalysisStatus(id: string, status: AnalysisStatus, error?: string): Promise<void> {
   ...
   */
  async updateWorkflowState(id: string, workflowState: string, confidence?: number): Promise<void> {
    await executeQuery((db) => {
      let query = `
        UPDATE ${this.tableName}
      ...
      if (confidence !== undefined) {
        query = query.replace('updated_at = datetime(\'now\')', 
  ...
  async getStatistics(): Promise<{
    ...
    return executeQuery((db) => {
      ...
      const pending = statusCounts.find(s => s.status === AnalysisStatus.PENDING)?.count || 0;
      const analyzed = statusCounts.find(s => s.status === AnalysisStatus.ANALYZED)?.count || 0;
      const failed = statusCounts.find(s => s.status === AnalysisStatus.FAILED)?.count || 0;
  ...
  async batchCreate(emails: Omit<EmailRecord, 'id'>[]): Promise<EmailRecord[]> {
    return executeTransaction((db) => {
      ...
      logger.info(`Batch created ${createdEmails.length} emails`, 'EMAIL_REPOSITORY');
   ...
   */
  async searchByText(searchText: string, fields: string[], limit?: number): Promise<EmailRecord[]> {
    return executeQuery((db) => {
      // Validate fields are actual columns
      const validFields = ['subject', 'body_text', 'from_address', 'to_addresses'];
   ...
   */
  async update(id: string, data: Partial<Omit<EmailRecord, 'id' | 'created_at'>>): Promise<EmailRecord | null> {
    return executeQuery((db) => {
