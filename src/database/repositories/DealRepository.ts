...
export class DealRepository extends BaseRepository<Deal> {
  ...
  async getDealStatistics(): Promise<{
    ...
    `;
    const valueResult = this.executeQuery<{ total_value: number }>(
      valueQuery,
      [],
      "get",
    );
    ...
    const salesRepQuery = `
      ...
      GROUP BY sales_rep
    `;
    const salesRepResults =
      this.executeQuery<Array<{ sales_rep: string; count: number }>>(
        salesRepQuery,
      );
  ...
  async searchDeals(
    ...
    return this.search(
      searchTerm,
      ["customer_name", "deal_id", "sales_rep", "deal_name"],
      options,
    );
  }
}
...
export class DealItemRepository extends BaseRepository<DealItem> {
  ...
  async getItemStatistics(): Promise<{
    ...
    `;
    const familyResults = this.executeQuery<
      Array<{
        product_family: string;
        count: number;
...
export class ProductFamilyRepository extends BaseRepository<ProductFamily> {
  ...
  async createProductFamily(familyData: {
    ...
    if (existingFamily) {
      throw new Error(
        `Product family with code ${familyData.family_code} already exists`,
      );
    }
