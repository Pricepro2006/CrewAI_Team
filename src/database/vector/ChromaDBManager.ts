...
export class ChromaDBManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      throw new Error(
        `ChromaDB connection failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async createCollection(config: CollectionConfig): Promise<Collection> {
    ...
    try {
      ...
      try {
        ...
        logger.info(
          `Using existing ChromaDB collection: ${config.name}`,
          "CHROMA_DB",
        );
      } catch (getError) {
        ...
        logger.info(
          `Created new ChromaDB collection: ${config.name}`,
          "CHROMA_DB",
        );
      }
      ...
      logger.error(
        `Failed to create/get collection ${config.name}: ${error}`,
        "CHROMA_DB",
      );
      throw error;
    }
  ...
  async updateDocuments(
    ...
    try {
      ...
      logger.error(
        `Failed to update documents in ${collectionName}: ${error}`,
        "CHROMA_DB",
      );
      throw error;
    }
  ...
  async getDocuments(
    ...
    try {
      ...
      logger.error(
        `Failed to get documents from ${collectionName}: ${error}`,
        "CHROMA_DB",
      );
      throw error;
    }
  ...
  async deleteDocuments(collectionName: string, ids: string[]): Promise<void> {
    ...
    try {
      ...
      logger.error(
        `Failed to delete documents from ${collectionName}: ${error}`,
        "CHROMA_DB",
      );
      throw error;
    }
  ...
  async deleteDocumentsByFilter(
    ...
    try {
      ...
      logger.info(
        `Deleted documents from collection ${collectionName} with filter`,
        "CHROMA_DB",
      );
    } catch (error) {
      logger.error(
        `Failed to delete documents by filter from ${collectionName}: ${error}`,
        "CHROMA_DB",
      );
      throw error;
    }
  ...
  async getCollectionStats(collectionName: string): Promise<{
    ...
    try {
      ...
      logger.error(
        `Failed to get stats for collection ${collectionName}: ${error}`,
        "CHROMA_DB",
      );
      throw error;
    }
  ...
  async deleteCollection(name: string): Promise<void> {
    try {
      ...
      logger.error(
        `Failed to delete collection ${name}: ${error}`,
        "CHROMA_DB",
      );
      throw error;
    }
  ...
  async similaritySearch(
    ...
    if (!embeddingFunction) {
      throw new Error(
        "Embedding function is required for text-based similarity search",
      );
    }
  ...
  async createSystemCollections(): Promise<void> {
    const systemCollections: CollectionConfig[] = [
      ...
      },
      {
        ...
        description:
          "Code snippets and documentation for development assistance",
        metadataSchema: {
          file_path: {
            type: "string",
            required: true,
    ...
    for (const collectionConfig of systemCollections) {
      try {
        ...
        logger.info(
          `System collection '${collectionConfig.name}' ready`,
          "CHROMA_DB",
        );
      } catch (error) {
        logger.error(
          `Failed to create system collection '${collectionConfig.name}': ${error}`,
          "CHROMA_DB",
        );
      }
    }
