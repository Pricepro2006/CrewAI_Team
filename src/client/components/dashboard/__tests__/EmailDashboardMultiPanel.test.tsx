...
describe("EmailDashboardMultiPanel Component", () => {
  ...
  describe("Error State", () => {
    it("should display error message when error is provided", () => {
      ...
      render(
        <EmailDashboardMultiPanel {...defaultProps} error={errorMessage} />,
      );
  ...
  describe("Email Filtering", () => {
    ...
    it("should limit panel emails to 5 items", () => {
      ...
      const marketingPanel = screen
        .getByText("Marketing & Analytics")
        .closest("div");
      const emailItems = marketingPanel?.querySelectorAll(
  ...
  describe("Email Interactions", () => {
    ...
    it("should call onAssignEmail when email is assigned", async () => {
      ...
      expect(mockOnAssignEmail).toHaveBeenCalledWith(
        "email-1",
        "test-assignee",
      );
    });
  ...
  describe("Responsive Design", () => {
    it("should apply responsive CSS classes", () => {
      const { container } = render(
        <EmailDashboardMultiPanel {...defaultProps} />,
      );
  ...
  describe("Accessibility", () => {
    it("should have proper heading structure", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "Marketing & Analytics" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", { name: "Sales & Customer Success" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", { name: "Support & Technical" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", { name: "All Emails Table" }),
      ).toBeInTheDocument();
    });
