...
export const SLATrackingDashboard: React.FC<SLATrackingDashboardProps> = ({
          ...
          callbacks: {
            ...
            afterBody: function (tooltipItems: any[]) {
              if (tooltipItems.length > 0) {
                ...
                if (!priority) return [];
                const total =
                  priority.onTrack + priority.atRisk + priority.overdue;
                const avgHours = (
                  priority.averageTime /
                  (1000 * 60 * 60)
                ).toFixed(1);
  ...
  return (
    <div
      className={`sla-tracking-dashboard bg-white rounded-lg shadow-sm border border-gray-200 p-6 ${className}`}
    >
      {/* Header */}
      <div className="mb-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
          ...
          <div className="text-3xl font-bold text-yellow-700">
            {overallMetrics.atRiskRate}%
          </div>
          <div className="text-xs text-yellow-600">
            {overallSLA.atRisk} items
          </div>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {priorityBreakdown.map((item) => {
                ...
                const avgHours = (item.averageTime / (1000 * 60 * 60)).toFixed(
                  1,
                );
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`text-sm font-medium ${
                          parseFloat(compliance) >= complianceTarget
                            ? "text-green-600"
                            : "text-red-600"
                        }`}
                      >
          ...
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-red-700">
                <p>
                  You have {overallSLA.overdue} overdue items that require
                  immediate attention.
                  <button
