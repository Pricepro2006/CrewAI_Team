...
export const WorkflowTimelineChart: React.FC<WorkflowTimelineChartProps> = ({
  ...
  const chartData = useMemo(() => {
    const labels = data?.map((point: any) => {
    ...
    const datasets = [
      {
        label: "Total Emails",
        data: data?.map((point: any) => point.totalEmails),
        backgroundColor:
          chartType === "area"
      ...
      },
      {
        ...
        data: data?.map((point: any) => point.completedEmails),
        backgroundColor:
      ...
      },
      {
        ...
        data: data?.map((point: any) => point.criticalEmails),
        backgroundColor:
              ...
              if (tooltipItems?.length || 0 > 0) {
                ...
                if (originalData) {
                  const completionRate =
                    originalData.totalEmails > 0
                      ? (
                          (originalData.completedEmails /
                            originalData.totalEmails) *
                          100
                        ).toFixed(1)
                      : "0";
  ...
  const summaryStats = useMemo(() => {
    ...
    const totalCritical = data.reduce(
      (sum, point) => sum + point.criticalEmails,
      0,
    );
    ...
    return {
      ...
      completionRate:
        totalEmails > 0
          ? ((totalCompleted / totalEmails) * 100).toFixed(1)
          : "0",
      criticalRate:
        totalEmails > 0
          ? ((totalCritical / totalEmails) * 100).toFixed(1)
          : "0",
      avgProcessingTime: avgProcessingTime
        ? (avgProcessingTime / 60000).toFixed(2)
        : null,
    };
  ...
  return (
    <div
      className={`workflow-timeline-chart bg-white rounded-lg shadow-sm border border-gray-200 p-6 ${className}`}
    >
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">{title}</h3>
          <div className="text-sm text-gray-500">{data?.length || 0} data points</div>
        </div>
            ...
            {summaryStats.avgProcessingTime && (
              <div className="bg-orange-50 p-3 rounded-lg border border-orange-200">
                ...
                <div className="text-2xl font-bold text-orange-700">
                  {summaryStats.avgProcessingTime}m
                </div>
              </div>
            )}
          </div>
      ...
      <div className="flex justify-center mt-4 space-x-2">
        {(["24h", "7d", "30d", "90d"] as const).map((range: any) => (
          <button
            key={range}
            onClick={() => {
              /* Time range switching would be handled by parent */
            }}
