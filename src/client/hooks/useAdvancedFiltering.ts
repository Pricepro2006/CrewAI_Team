...
export const useAdvancedFiltering = <TData extends Record<string, any>>(
  ...
  const { initialPresets = [], storageKey = 'emailDashboard_filterPresets', debounceMs = 300 } = options;
  ...
  const advancedFilterFn: FilterFn<TData> = useCallback((row, columnId, filterValue) => {
      ...
      switch (rule.operator) {
        ...
        case 'between':
          if (typeof rule.value === 'object' && 'min' in rule.value && 'max' in rule.value) {
            ...
            return !isNaN(minValue) && !isNaN(maxValue) && !isNaN(betweenCellValue) &&
                   betweenCellValue >= minValue && betweenCellValue <= maxValue;
          }
          return true;
  ...
  const fuzzyFilterFn: FilterFn<TData> = useCallback((row, columnId, filterValue) => {
    if (!filterValue) return true;
    ...
    if (searchValue.length > 2) {
      ...
      return words.some(word => {
        ...
        if (word.length > 3 && searchValue.length > 3) {
          ...
          return differences <= 1 && Math.abs(word.length - searchValue.length) <= 1;
        }
  ...
  // Preset management
  const savePreset = useCallback((preset: Omit<FilterPreset, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const filterStats = useMemo(() => {
    const activeRules = filterRules.filter(rule => rule.enabled && rule.value);
  ...
  // Import filter state
  const importFilterState = useCallback((state: { filterRules: FilterRule[]; globalFilter: string }) => {
