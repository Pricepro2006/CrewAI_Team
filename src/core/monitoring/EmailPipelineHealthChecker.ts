...
export class EmailPipelineHealthChecker {
  ...
  async getHealthStatus(): Promise<EmailPipelineHealthStatus> {
    ...
    const startTime = now;
    logger.debug("Starting email pipeline health check", "EMAIL_PIPELINE_HEALTH");
    ...
    try {
      ...
      // Record metrics
      metrics.histogram('email_pipeline.health_check_duration', Date.now() - startTime);
      metrics.gauge('email_pipeline.overall_health', overallStatus === 'healthy' ? 1 : overallStatus === 'degraded' ? 0.5 : 0);

      logger.debug("Email pipeline health check completed", "EMAIL_PIPELINE_HEALTH", {
        status: overallStatus,
      ...
      logger.error("Email pipeline health check failed", "EMAIL_PIPELINE_HEALTH", error as Error);
      ...
      return {
        ...
        services: {
          ...
          redis: { status: "unhealthy", lastCheck: new Date().toISOString(), details: "Health check failed" },
          ollama: { status: "unhealthy", lastCheck: new Date().toISOString(), details: "Health check failed" },
          pipeline: { status: "unhealthy", lastCheck: new Date().toISOString(), details: "Health check failed" },
          processingQueue: { status: "unhealthy", lastCheck: new Date().toISOString(), details: "Health check failed" }
        },
        metrics: {
          totalEmails: 0,
          todaysEmails: 0,
  ...
  private async checkDatabaseHealth(): Promise<ServiceHealth> {
    ...
    try {
      ...
      const tables = ['emails_enhanced', 'email_analysis', 'pipeline_executions'];
      ...
      const dbInfo = this.db.prepare(`PRAGMA database_list`).all() as Array<{ name: string; file: string }>;
      ...
      logger.error("Database health check failed", "EMAIL_PIPELINE_HEALTH", error as Error);
      return {
        status: "unhealthy",
        lastCheck: new Date().toISOString(),
  ...
  private async checkOllamaHealth(): Promise<ServiceHealth> {
    ...
    try {
      ...
      const missingModels = requiredModels.filter(model => 
        !availableModels.some((available: string) => available.includes(model.split(':')[0]))
      );

      const status = responseTime < 2000 && missingModels.length === 0 ? "healthy" : 
                    responseTime < 5000 && missingModels.length <= 1 ? "degraded" : "unhealthy";
  ...
  private async checkPipelineHealth(): Promise<ServiceHealth> {
    ...
    try {
      ...
      const totalRecent = recentExecutions.reduce((sum, exec) => sum + exec.count, 0);
      const failedRecent = recentExecutions.find(exec => exec.status === 'failed')?.count || 0;
      const successRate = totalRecent > 0 ? ((totalRecent - failedRecent) / totalRecent) * 100 : 100;

      const responseTime = Date.now() - startTime;
      const status = successRate >= 95 && pipelineStatus.status !== 'failed' ? "healthy" :
  ...
  private async checkProcessingQueueHealth(): Promise<ServiceHealth> {
    ...
    try {
      ...
      const processed1h = this.db.prepare(`
        SELECT COUNT(*) as count FROM emails_enhanced 
        WHERE status = 'processed' AND updated_at >= datetime('now', '-1 hour')
      `).get() as { count: number };

      const responseTime = Date.now() - startTime;
      const status = stuck.count === 0 && unprocessed.count < 1000 ? "healthy" :
                    stuck.count < 10 && unprocessed.count < 5000 ? "degraded" : "unhealthy";
  ...
  async getPipelineMetrics(): Promise<EmailPipelineMetrics> {
    try {
      // Basic counts
      const totalEmails = this.db.prepare('SELECT COUNT(*) as count FROM emails_enhanced').get() as { count: number };
      const todaysEmails = this.db.prepare(`
        SELECT COUNT(*) as count FROM emails_enhanced 
      ...
      const failedEmails = this.db.prepare(`
        SELECT COUNT(*) as count FROM emails_enhanced 
        WHERE status = 'failed'
      `).get() as { count: number };
      ...
      const stageStats = this.db.prepare(`
        SELECT 
          SUM(CASE WHEN stage >= 1 THEN 1 ELSE 0 END) as stage1_total,
          SUM(CASE WHEN stage >= 2 THEN 1 ELSE 0 END) as stage2_total,
          SUM(CASE WHEN stage >= 3 THEN 1 ELSE 0 END) as stage3_total,
          COUNT(*) as total_processed
        FROM (
          SELECT 
            CASE 
              WHEN pipeline_stage >= 3 THEN 3
              WHEN pipeline_stage >= 2 THEN 2
              WHEN pipeline_stage >= 1 THEN 1
              ELSE 0
            END as stage
          FROM email_analysis
      ...
      const stage1Success = stageStats.total_processed > 0 ? (stageStats.stage1_total / stageStats.total_processed) * 100 : 0;
      const stage2Success = stageStats.stage1_total > 0 ? (stageStats.stage2_total / stageStats.stage1_total) * 100 : 0;
      const stage3Success = stageStats.stage2_total > 0 ? (stageStats.stage3_total / stageStats.stage2_total) * 100 : 0;
      const overallSuccessRate = stageStats.total_processed > 0 ? 
        ((stageStats.total_processed - failedEmails.count) / stageStats.total_processed) * 100 : 100;
      ...
      logger.error("Failed to get pipeline metrics", "EMAIL_PIPELINE_HEALTH", error as Error);
  ...
  private async getResourceMetrics(): Promise<{
    ...
    try {
      ...
      logger.error("Failed to get resource metrics", "EMAIL_PIPELINE_HEALTH", error as Error);
      return {
        memoryUsage: 0,
        cpuUsage: 0,
        diskUsage: 0,
