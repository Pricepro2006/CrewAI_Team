...
import { MODEL_CONFIG, getModelTimeout, getModelBatchSize } from "../../config/models.config";
...
export class Stage2LlamaAnalysis {
  ...
  async process(emails: Email[]): Promise<LlamaAnalysisResults> {
    ...
    logger.info(`Starting Llama 3.2:3b analysis for ${emails.length} priority emails`, "STAGE2");
    ...
    for (let i = 0; i < emails.length; i += this.batchSize) {
      ...
      const batchPromises = batch.map(email => 
        this.analyzeWithTimeout(email, this.timeout)
      );
      
      const batchResults = await Promise.allSettled(batchPromises);
      results.push(...this.handleBatchResults(batchResults, batch));
      
      // Progress logging
      const processed = Math.min(i + this.batchSize, emails.length);
      const progress = (processed / emails.length * 100).toFixed(1);
      logger.info(`Stage 2 Progress: ${processed}/${emails.length} (${progress}%)`, "STAGE2");
    ...
    logger.info(`Llama analysis completed in ${totalTime.toFixed(2)}s`, "STAGE2");
   ...
   */
  private async analyzeWithTimeout(email: Email, timeout: number): Promise<LlamaAnalysisResult> {
    ...
    try {
      ...
      logger.error(`Llama analysis error for email ${email.id}`, "STAGE2", error);
   ...
   */
  private async analyzeEmail(email: Email, signal: AbortSignal): Promise<LlamaAnalysisResult> {
    const startTime = Date.now();
...
Respond ONLY with valid JSON, no additional text.`;
  ...
  private calculateQualityScore(analysis: any): number {
    ...
    const weights = {
      ...
      entities: 0.20,
      businessProcess: 0.20,
      actionItems: 0.20,
      suggestedResponse: 0.15,
    };

    // Context understanding (0-2.5)
    if (analysis.contextual_summary && analysis.contextual_summary.length > 50) {
    ...
    if (analysis.action_items && analysis.action_items.length > 0) {
      score += weights.actionItems * Math.min(analysis.action_items.length * 3, 10);
    }
  ...
  private handleBatchResults(
    ...
    return results.map((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        logger.error(`Batch processing error for email ${emails[index].id}`, "STAGE2", result.reason);
        return this.createErrorResult(emails[index], result.reason?.message || 'Unknown error');
      }
    });
  }
   ...
   */
  private async saveIntermediateResults(results: LlamaAnalysisResult[]): Promise<void> {
    try {
      const fs = await import('fs/promises');
      const path = 'stage2_intermediate_results.json';
      await fs.writeFile(path, JSON.stringify(results, null, 2));
    } catch (error) {
      logger.warn("Failed to save intermediate results", "STAGE2", error as Error);
    }
  }
}