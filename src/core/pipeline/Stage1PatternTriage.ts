...
export class Stage1PatternTriage {
  ...
  private readonly URGENCY_KEYWORDS = {
    CRITICAL: ['urgent', 'critical', 'emergency', 'asap', 'immediately', 'escalation'],
  ...
  private readonly WORKFLOW_PATTERNS = {
    COMPLETION: ['completed', 'done', 'finished', 'shipped', 'delivered', 'resolved'],
    IN_PROGRESS: ['processing', 'working on', 'in progress', 'pending', 'reviewing'],
  ...
  async process(emails: Email[]): Promise<TriageResults> {
    ...
    logger.info(`Starting pattern triage for ${emails.length} emails`, "STAGE1");
    ...
    for (let i = 0; i < emails.length; i += batchSize) {
      ...
      logger.info(`Stage 1 Progress: ${processed}/${emails.length} (${progress}%)`, "STAGE1");
    }

    const totalTime = (Date.now() - startTime) / 1000;
    logger.info(`Pattern triage completed in ${totalTime.toFixed(2)}s`, "STAGE1");
  ...
  private analyzeEmail(email: Email): TriageResult {
    ...
    const priorityScore = this.calculatePriorityScore(entities, urgencyLevel, workflow);
  ...
  private extractPartNumbers(text: string): string[] {
    ...
    const matches = text.matchAll(this.PART_PATTERNS[0]);
    for (const match of matches) {
      const part = match[0];
    ...
    // SKU and PN patterns
    this.extractPatterns(text, this.PART_PATTERNS.slice(1)).forEach(p => parts.add(p));
  ...
  private extractCompanies(text: string): string[] {
    ...
    const suffixes = ['INC', 'LLC', 'CORP', 'LTD', 'CO', 'COMPANY', 'CORPORATION'];
    
    // Look for company patterns
    const pattern = new RegExp(`\\b([A-Z][\\w\\s&]+?)\\s*(${suffixes.join('|')})\\b`, 'gi');
    const matches = text.matchAll(pattern);
  ...
  private determineBusinessProcess(entities: any, text: string): string {
    ...
    if (text.toLowerCase().includes('ship') || text.toLowerCase().includes('freight')) {
   ...
   */
  private prioritizeEmails(results: TriageResult[], emails: Email[]): TriageResults {
