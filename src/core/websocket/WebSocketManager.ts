...
import { redisService, CacheKeys, CacheTTL } from '../cache/RedisService';
...
export class WebSocketManager {
  ...
  constructor(httpServer: HTTPServer, config?: WebSocketConfig) {
    ...
    logger.info('WebSocket server initialized', 'WEBSOCKET', { config: finalConfig });
  }
    ...
    this.io.use(async (socket: AuthenticatedSocket, next) => {
      try {
        ...
        if (!token) {
          ...
          logger.info('Anonymous WebSocket connection', 'WEBSOCKET', { sessionId: socket.sessionId });
          return next();
        }
  ...
  private async sendInitialData(socket: Socket, channel: string): Promise<void> {
    try {
      ...
      logger.error('Failed to send initial data', 'WEBSOCKET', { channel, error });
  ...
  private startBroadcastIntervals(): void {
    ...
    const volumeInterval = setInterval(async () => {
      try {
        ...
        logger.error('Failed to broadcast daily volume', 'WEBSOCKET', { error });
      }
    }, 30000);
    ...
    const entityInterval = setInterval(async () => {
      try {
        const entityMetrics = await this.getEntityMetrics();
        this.io.to('email:entityMetrics').emit('data:entityMetrics', entityMetrics);
      } catch (error) {
        logger.error('Failed to broadcast entity metrics', 'WEBSOCKET', { error });
      }
    }, 30000);
    ...
    const workflowInterval = setInterval(async () => {
      try {
        const workflowDist = await this.getWorkflowDistribution();
        this.io.to('email:workflowDistribution').emit('data:workflowDistribution', workflowDist);
      } catch (error) {
        logger.error('Failed to broadcast workflow distribution', 'WEBSOCKET', { error });
      }
    }, 60000);
