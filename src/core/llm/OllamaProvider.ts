...
export class OllamaProvider extends EventEmitter {
  ...
  async generate(
    ...
    try {
      ...
      const tokensUsed = (response.data && response.data.eval_count) || 0;
      tracker.success(tokensUsed);
      performanceMonitor.measure("ollama_generate", {
        model: this.config.model,
        promptLength: prompt.length || 0,
        responseLength: sanitizedResponse.content.length || 0,
        tokensUsed,
        duration: (response.data && response.data.total_duration) || 0,
      });
      ...
      this.emit("generation", {
        ...
        duration: (response.data && response.data.total_duration) || 0,
        tokens: response.data && response.data.tokens,
        logprobs: response.data && response.data.token_logprobs,
      });
  ...
  async generateWithLogProbs(
    ...
    try {
      ...
      if (response.data && response.data.token_logprobs) {
        logProbs = response.data.token_logprobs;
      } else if (response.data && response.data.logprobs && response.data.logprobs.length > 0) {
      ...
      if (
        ...
        error.response?.data?.error?.includes("logits")
      ) {
        console.warn(
          "Log probabilities not supported by this Ollama version. Falling back to standard generation.",
        );
