...
export const ContactPatterns = {
  ...
  phone: {
    ...
    // Toll-free numbers
    tollFree:
      /1[-.\s]?(?:800|888|877|866|855|844|833)[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}/gi,

    // Generic phone pattern (catches most formats)
    generic:
      /(?:(?:\+?[1-9]{1,3}[-.\s]?)?(?:\([0-9]{1,4}\)|[0-9]{1,4})[-.\s]?)?[0-9]{1,4}[-.\s]?[0-9]{1,4}[-.\s]?[0-9]{1,9}/g,
  ...
  address: {
    ...
    streetAddress:
      /\b(\d{1,5})\s+([A-Za-z0-9\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln|Drive|Dr|Court|Ct|Circle|Cir|Plaza|Pl|Parkway|Pkwy|Highway|Hwy|Way))\.?\b/gi,
    ...
    fullAddress:
      /\d{1,5}\s+[A-Za-z0-9\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln|Drive|Dr|Court|Ct|Circle|Cir|Plaza|Pl)\.?\s*(?:Suite|Ste|Apt|Unit|#)?\s*[A-Za-z0-9]*,?\s*[A-Za-z\s]+,\s*[A-Z]{2}\s*\d{5}(?:-\d{4})?/gi,
  ...
  hours: {
    ...
    standard:
      /\b(?:(\d{1,2})(?::(\d{2}))?(?:\s*(?:am|AM|pm|PM))?)\s*[-–—]\s*(?:(\d{1,2})(?::(\d{2}))?(?:\s*(?:am|AM|pm|PM))?)\b/g,

    // Day ranges (Mon-Fri, Monday-Friday)
    dayRange:
      /\b(?:Mon(?:day)?|Tue(?:sday)?|Wed(?:nesday)?|Thu(?:rsday)?|Fri(?:day)?|Sat(?:urday)?|Sun(?:day)?)\s*[-–—]\s*(?:Mon(?:day)?|Tue(?:sday)?|Wed(?:nesday)?|Thu(?:rsday)?|Fri(?:day)?|Sat(?:urday)?|Sun(?:day)?)\b/gi,

    // 24/7 variations
    twentyFourSeven:
      /\b(?:24\/7|24\s*hours|twenty[\s-]?four\s*seven|around\s*the\s*clock)\b/gi,

    // Closed days
    closed:
      /\b(?:closed\s*(?:on\s*)?(?:Mon(?:day)?|Tue(?:sday)?|Wed(?:nesday)?|Thu(?:rsday)?|Fri(?:day)?|Sat(?:urday)?|Sun(?:day)?s?))\b/gi,

    // Full hours pattern
    fullHours:
      /(?:(?:Mon(?:day)?|Tue(?:sday)?|Wed(?:nesday)?|Thu(?:rsday)?|Fri(?:day)?|Sat(?:urday)?|Sun(?:day)?)\s*[-–—:]?\s*)?(?:\d{1,2}(?::\d{2})?\s*(?:am|AM|pm|PM)?)\s*[-–—]\s*(?:\d{1,2}(?::\d{2})?\s*(?:am|AM|pm|PM)?)/gi,
  },
  ...
  businessName: {
    ...
    entityTypes:
      /\b(?:Inc|LLC|Ltd|Corp|Corporation|Company|Co|LLP|LP|Partners|Associates|Group|Services|Solutions|Enterprises|Holdings|Industries)\b\.?/gi,

    // Business name with entity type
    withEntity:
      /\b[A-Z][A-Za-z0-9\s&'.-]+(?:Inc|LLC|Ltd|Corp|Corporation|Company|Co|LLP|LP)\.?\b/g,
  ...
  email: {
    ...
    withName:
      /(?:[A-Za-z\s]+\s*<)?([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,})>?/gi,
  },
...
export const PatternHelpers = {
  ...
  extractMatches: (
    text: string,
    pattern: RegExp,
    confidenceFunc?: (match: string) => number,
  ) => {
  ...
  normalizeAddress: (address: string): string => {
    return address.replace(/\s+/g, " ").replace(/\./g, "").trim();
  },
