...
export class AnalysisScorer {
  ...
  private weights = {
    contextUnderstanding: 0.20,    // 20% - Understanding the email context
    entityExtraction: 0.25,        // 25% - Accurate entity extraction
    businessProcessing: 0.20,      // 20% - Correct workflow/process identification
    actionableInsights: 0.20,      // 20% - Quality of action items
  ...
  scoreAnalysis(
    ...
    const overall = Object.entries(dimensions).reduce(
      (sum, [key, score]) => sum + score * this.weights[key as keyof ScoringDimensions],
      0
    );
  ...
  private scoreContextUnderstanding(
    ...
    if (analysis.workflow_state === baseline.workflow_state) {
      score += 4;
    } else if (this.isCloseWorkflowState(analysis.workflow_state, baseline.workflow_state)) {
      score += 2;
    }
  ...
  private scoreEntityExtraction(
    ...
    for (const entityType of entityTypes) {
      ...
      const precision = this.calculatePrecision(analysisEntities, baselineEntities);
  ...
  private scoreBusinessProcessing(
    ...
    if (analysis.business_process === baseline.business_process) {
      score += 5;
    } else if (this.isRelatedProcess(analysis.business_process, baseline.business_process)) {
      score += 2.5;
    }
    ...
    if (analysis.sla_status && baseline.sla_status) {
      if (analysis.sla_status === baseline.sla_status) {
        score += 3;
      } else if (this.isSimilarSLAStatus(analysis.sla_status, baseline.sla_status)) {
        score += 1.5;
      }
    }
  ...
  private scoreActionableInsights(
    ...
    for (const baselineAction of baselineActions) {
      const bestMatch = this.findBestActionMatch(baselineAction, analysisActions);
      if (bestMatch) {
        matchedActions++;
    ...
    const precision = analysisActions.length > 0 ? matchedActions / analysisActions.length : 0;
  ...
  private scoreResponseQuality(
    ...
    if (!analysis.suggested_response || !baseline.suggested_response) {
      return analysis.suggested_response === baseline.suggested_response ? 10 : 0;
    }
  ...
  private isCloseWorkflowState(state1: string, state2: string): boolean {
    ...
    for (const [key, values] of Object.entries(stateGroups)) {
      if ((state1 === key || values.includes(state1)) &&
  ...
  private compareUrgencyIndicators(indicators1: string[], indicators2: string[]): number {
    if (indicators2.length === 0) return indicators1.length === 0 ? 1 : 0;
    
    const matches = indicators1.filter(i => 
      indicators2.some(j => i.toLowerCase().includes(j.toLowerCase()) || 
  ...
  private isRelatedProcess(process1: string, process2: string): boolean {
    ...
    for (const [key, values] of Object.entries(processGroups)) {
      if ((process1 === key || values.includes(process1)) &&
  ...
  private scoreActionMatch(action1: any, action2: any): number {
    ...
    // Owner match (25%)
    if (action1.owner && action2.owner && 
    ...
    if (action1.deadline && action2.deadline) {
      const deadlineSimilarity = this.compareDeadlines(action1.deadline, action2.deadline);
  ...
  private compareDeadlines(deadline1: string, deadline2: string): number {
    ...
    const isUrgent1 = urgentTerms.some(t => deadline1.toLowerCase().includes(t));
    const isUrgent2 = urgentTerms.some(t => deadline2.toLowerCase().includes(t));
  ...
  private assessProfessionalQuality(response: string): number {
    ...
    const professionalTerms = [
      'acknowledge', 'assist', 'ensure', 'appreciate',
      'confirm', 'review', 'process', 'update'
    ];
  ...
  private generateScoringDetails(
    ...
    if (analysis.workflow_state !== baseline.workflow_state) {
      details.push(`Workflow state mismatch: ${analysis.workflow_state} vs ${baseline.workflow_state}`);
    }
    ...
    for (const type of entityTypes) {
      ...
      if (analysisCount !== baselineCount) {
        details.push(`${type}: found ${analysisCount}, expected ${baselineCount}`);
      }
    }
    ...
    if (analysisActionCount !== baselineActionCount) {
      details.push(`Action items: found ${analysisActionCount}, expected ${baselineActionCount}`);
    }
