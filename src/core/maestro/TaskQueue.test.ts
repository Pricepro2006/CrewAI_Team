...
describe('TaskQueue', () => {
  ...
  describe('toArray', () => {
    it('should return all tasks in priority order', () => {
      const tasks: Task[] = [
        { id: 'low', type: 'test', priority: 1, data: {}, createdAt: new Date() },
        { id: 'high', type: 'test', priority: 10, data: {}, createdAt: new Date() },
        { id: 'medium', type: 'test', priority: 5, data: {}, createdAt: new Date() },
      ];
  ...
  describe('findById', () => {
    it('should find a task by ID', () => {
      ...
      queue.enqueue(task);
      queue.enqueue({ id: 'other', type: 'test', data: {}, createdAt: new Date() });
    ...
    it('should return null if task not found', () => {
      queue.enqueue({ id: 'task-1', type: 'test', data: {}, createdAt: new Date() });
  ...
  describe('removeById', () => {
    it('should remove a task by ID', () => {
      queue.enqueue({ id: 'remove-me', type: 'test', data: {}, createdAt: new Date() });
      queue.enqueue({ id: 'keep-me', type: 'test', data: {}, createdAt: new Date() });
  ...
  describe('concurrent operations', () => {
    it('should handle multiple operations safely', () => {
      const operations = [
        () => queue.enqueue({ id: '1', type: 'test', priority: 5, data: {}, createdAt: new Date() }),
        () => queue.enqueue({ id: '2', type: 'test', priority: 3, data: {}, createdAt: new Date() }),
        () => queue.dequeue(),
        () => queue.enqueue({ id: '3', type: 'test', priority: 7, data: {}, createdAt: new Date() }),
        () => queue.peek(),
        () => queue.removeById('2'),
      ];
