...
describe('BusinessSearchMiddleware - Caching Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockProvider = {
      generate: jest.fn().mockResolvedValue('Fresh response with contact info'),
      generateWithLogProbs: jest.fn().mockResolvedValue({
        text: 'Fresh response with contact info',
        logProbs: []
      }),
      generateStream: jest.fn().mockResolvedValue('Fresh stream response')
    } as any;
  ...
  describe('Cache Management', () => {
    ...
    it('should preload common queries', async () => {
      const commonQueries = [
        { query: 'Find emergency plumber', location: '90210', response: 'Emergency plumbers...' },
        { query: 'Find 24/7 electrician', location: '90210', response: '24/7 electricians...' }
      ];
  ...
  describe('Cache with Validation', () => {
    it('should only cache valid responses', async () => {
      ...
      validator.validateResponse = jest.fn()
  ...
  describe('Cache with Rate Limiting', () => {
    it('should not cache rate-limited requests', async () => {
      ...
      // Reset rate limit
      jest.spyOn(middleware as any, 'checkRateLimit').mockResolvedValue(true);
