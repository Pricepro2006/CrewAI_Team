...
Response format:
{
  ...
  private async deepWorkflowAnalysis(email: Email, quickAnalysis: QuickAnalysis): Promise<DeepWorkflowAnalysis> {
...
Focus on TD SYNNEX operations: orders, quotes, shipping, support, deals, approvals, renewals, vendors.`;
  ...
  private async extractEnhancedEntities(email: Email): Promise<EnhancedEmailEntities> {
  ...
  private extractMatchesWithContext(text: string, pattern: RegExp): Array<{ value: string; context: string }> {
  ...
  private detectPOFormat(po: string): '8-digit' | '10-digit' | '11-digit' | 'alphanumeric' {
  ...
  private detectActionType(action: string): ActionItem['type'] {
    ...
    if (lower.includes('forward') || lower.includes('send to')) return 'forward';
    if (lower.includes('approve') || lower.includes('authorization')) return 'approval';
    if (lower.includes('follow up') || lower.includes('check')) return 'follow-up';
    return 'task';
  }
  ...
  private async extractActionSummary(email: Email, analysis: DeepWorkflowAnalysis): Promise<string> {
...
Summary:`;
  ...
  private inferSecondaryWorkflows(primary: string, entities: EnhancedEmailEntities): string[] {
    const secondary = [];
  ...
  private inferActionItems(analysis: QuickAnalysis, entities: EnhancedEmailEntities): ActionItem[] {
  ...
  private async trackWorkflowPattern(email: Email, analysis: EmailAnalysisResult): Promise<void> {
