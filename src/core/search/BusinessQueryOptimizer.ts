...
export class BusinessQueryOptimizer {
  ...
  private static readonly LOCATION_PATTERNS = {
    ...
    streetAddress:
      /\d+\s+[A-Za-z0-9\s,.-]+(?:street|st|avenue|ave|road|rd|boulevard|blvd|lane|ln|drive|dr|court|ct|circle|cir|plaza|pl)\b/i,
  };

  private static readonly TIME_PATTERNS = {
    emergency: /\b(emergency|urgent|immediately|asap|now|right away)\b/i,
    availability:
      /\b(24\/7|24 hours|open now|weekend|weekday|after hours|holiday)\b/i,
    schedule:
      /\b(morning|afternoon|evening|night|today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/i,
  };
  ...
  private static extractLocation(query: string): LocationInfo {
    ...
    const locationWords = query.match(
  ...
  private static extractTimeConstraints(query: string): TimeConstraint[] {
    ...
    if (availMatch) {
      ...
      if (
        availMatch[0].includes("24/7") ||
        availMatch[0].includes("24 hours")
      ) {
  ...
  private static buildOptimizedQuery(components: QueryComponents): string {
    ...
    if (
      ...
      components.serviceType !== "general service"
    ) {
  ...
  private static getSecurityType(
    pattern: RegExp,
  ): "sql_injection" | "xss" | "suspicious_pattern" {
