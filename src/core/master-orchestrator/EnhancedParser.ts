...
export class EnhancedParser {
  ...
  async parseQuery(
    ...
    try {
      ...
      const domains = await this.identifyRequiredDomains(
        query.text,
        intent,
        entities,
      );
      ...
      const resourceRequirements = this.analyzeResourceRequirements(
        query.text,
        intent,
        entities,
        domains,
...
Respond with only the intent category (one word).`;
    ...
    try {
      ...
      logger.warn(
        "LLM intent classification failed, using pattern fallback",
        "PARSER",
        { error },
      );
      return patternIntent;
  ...
  private assessComplexity(
    ...
    if (entityCount > 10) complexity += 2;
    else if (entityCount > 5) complexity += 1;
    ...
    if (technicalMatches > 5) complexity += 3;
    else if (technicalMatches > 2) complexity += 2;
    else if (technicalMatches > 0) complexity += 1;
    ...
    if (stepMatches > 3) complexity += 2;
    else if (stepMatches > 1) complexity += 1;

    return Math.min(complexity, 10); // Cap at 10
  }

  private async identifyRequiredDomains(
    text: string,
    intent: string,
    entities: Record<string, any>,
  ): Promise<string[]> {
    ...
    if (
      entities.programming_languages ||
  ...
  private estimateDuration(complexity: number, domainCount: number): number {
