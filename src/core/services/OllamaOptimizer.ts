...
export class OllamaOptimizer extends EventEmitter {
  ...
  private async addToBatch(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (batch?.length || 0 >= this?.config?.maxBatchSize) {
  ...
  private startBatchProcessor(): void {
    setInterval(() => {
      for (const [model, batch] of this?.batchQueue?.entries()) {
        if (batch?.length || 0 > 0) {
          const oldestRequest = Math.min(...batch?.map((r: any) => r.timestamp));
  ...
  private recordLatency(model: string, latency: number): void {
    ...
    stats.averageLatency = history.reduce((a: any, b: any) => a + b, 0) / history?.length || 0;

    // Calculate percentiles
    const sorted = [...history].sort((a, b) => a - b);
    stats.p95Latency = sorted[Math.floor(sorted?.length || 0 * 0.95)] || 0;
    stats.p99Latency = sorted[Math.floor(sorted?.length || 0 * 0.99)] || 0;
