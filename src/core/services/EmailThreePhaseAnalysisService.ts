...
export class EmailThreePhaseAnalysisService extends EventEmitter {
  ...
  async analyzeEmail(
    ...
    try {
      ...
      logger.info(`Phase 1 complete in ${phase1Results.processing_time}ms`, COMPONENT);
      ...
      if (isCompleteChain || options.forceAllPhases) {
        logger.info("Running Phase 3 for complete chain analysis", COMPONENT);
        ...
        try {
          ...
          logger.warn("Failed to track three-phase analysis performance", COMPONENT, { error: perfError });
        }
        ...
        try {
          ...
          logger.warn("Failed to track two-phase analysis performance", COMPONENT, { error: perfError });
        }
      ...
      logger.error("Email analysis failed:", COMPONENT, {}, error as Error);
      ...
      try {
        ...
        logger.warn("Failed to track performance metrics", COMPONENT, { error: perfError });
      }
      ...
      try {
        ...
        logger.warn("Failed to emit analysis error event", COMPONENT, { error: emitError });
      }
  ...
  private async runPhase1(
    ...
    if ((email as any).chainAnalysis) {
      ...
      try {
        ...
        logger.error(`Chain analysis failed for ${email.id}:`, COMPONENT, {}, error as Error);
  ...
  private async runPhase2(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        logger.debug(`Phase 2 attempt ${attempt + 1}/${maxRetries + 1}`, COMPONENT);
        ...
        if (!this.validatePhase2Response(phase2Data)) {
          logger.warn(`Invalid Phase 2 response structure on attempt ${attempt + 1}`, COMPONENT, {
        ...
        logger.warn(`Phase 2 attempt ${attempt + 1} failed:`, COMPONENT, { error: (error as Error).message });

        // Check for special parsing failure error
        if ((error as Error).message === "PARSING_FAILED_ALL_RETRIES") {
          logger.info("All parsing attempts failed, using pure fallback immediately", COMPONENT);
    ...
    logger.error("Phase 2 failed after all retries:", COMPONENT, {}, lastError || undefined);
  ...
  private buildPhase2Prompt(
    ...
    try {
      ...
      logger.warn("Failed to sanitize email content, using fallback", COMPONENT, { error });
      sanitizedEmail = null;
    }
    ...
    try {
      if (PromptSanitizer.detectInjectionAttempt(safeEmail.subject) ||
        ...
        logger.warn("Potential prompt injection detected in email", COMPONENT, {
          emailId: email.id,
          sender: email.sender_email,
        });
      }
    } catch (error) {
      logger.warn("Failed to check for injection attempts", COMPONENT, { error });
    }
    ...
    try {
      ...
      logger.error("Failed to build prompt, using fallback", COMPONENT, { error });
  ...
  private async callLlama3(
    ...
    try {
      ...
      logger.warn("Rate limit check failed, proceeding with request", COMPONENT, { error });
      rateLimitResult = { allowed: true }; // Fallback to allow the request
    }

    if (rateLimitResult && !rateLimitResult.allowed) {
      logger.warn("LLM rate limit exceeded for Llama 3.2", COMPONENT, {
  ...
  private async runPhase3(
    ...
    try {
      ...
      try {
        ...
        logger.warn("Failed to sanitize email content in Phase 3, using fallback", COMPONENT, { error });
        sanitizedEmail = null;
      }
      ...
      try {
        if (PromptSanitizer.detectInjectionAttempt(safeEmail.subject) ||
            PromptSanitizer.detectInjectionAttempt(safeEmail.body)) {
          logger.warn("Potential prompt injection detected in Phase 3", COMPONENT, {
            emailId: email.id,
            sender: email.sender_email,
          });
        }
      } catch (error) {
        logger.warn("Failed to check for injection attempts in Phase 3", COMPONENT, { error });
      }
      ...
      try {
        ...
        logger.error("Failed to build Phase 3 prompt, using fallback", COMPONENT, { error });
      ...
      try {
        ...
        logger.warn("Rate limit check failed for Phase 3, proceeding with request", COMPONENT, { error });
        rateLimitResult = { allowed: true }; // Fallback to allow the request
      }

      if (rateLimitResult && !rateLimitResult.allowed) {
        logger.warn("LLM rate limit exceeded for Phi-4", COMPONENT, {
          remainingRequests: rateLimitResult.remainingRequests,
          resetTime: rateLimitResult.resetTime,
        });
      ...
      logger.error("Phase 3 error:", COMPONENT, {}, error as Error);
  ...
  private parseJsonResponse(response: string, attemptNumber: number = 0, maxRetries: number = 2): Record<string, unknown> {
    try {
      ...
      logger.error("JSON parse error:", COMPONENT, {}, error as Error);
      logger.debug("Raw response: " + response.substring(0, 500) + "...", COMPONENT);
      ...
      if (fallbackResult) {
        logger.info("Successfully used fallback extraction method", COMPONENT);
  ...
  private extractJsonFromResponse(response: string, isRetryAttempt: boolean = false): string | null {
    ...
    try {
      ...
      logger.info("Response is valid JSON after comment removal", COMPONENT);
      return quickCleaned;
    } catch (error) {
      logger.info("Response not valid JSON after comment removal:", COMPONENT, { error: error instanceof Error ? error.message : String(error) });
    ...
    try {
      JSON.parse(cleaned);
      logger.debug("JSON is already valid, skipping cleanup steps", COMPONENT);
    } catch (error) {
      // Only apply fixes if JSON is invalid
      logger.debug("JSON is invalid, applying cleanup:", COMPONENT, { error: error instanceof Error ? error.message : String(error) });
      cleaned = cleaned
    ...
    try {
      const parsed = JSON.parse(cleaned);
      logger.debug("Strategy 1 successful: Direct parsing", COMPONENT);
      return cleaned;
    } catch (error) {
      logger.debug("Strategy 1 failed:", COMPONENT, { error: error instanceof Error ? error.message : String(error) });
    ...
    if (firstBrace !== -1) {
      ...
      if (endIndex !== -1) {
        try {
          ...
          logger.debug("Strategy 2 failed with balanced braces:", COMPONENT, { error: error instanceof Error ? error.message : String(error) });
          // Continue to next strategy
        }
      }
    }
