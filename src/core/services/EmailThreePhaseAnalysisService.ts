...
export class EmailThreePhaseAnalysisService extends EventEmitter {
  ...
  constructor(
    databasePath: string = './data/crewai.db',
    redisUrl?: string
  ) {
    super();
    this.db = new Database(databasePath);
    this.redisService = new RedisService(redisUrl ? { host: redisUrl.split('://')[1]?.split(':')[0] || 'localhost', port: parseInt(redisUrl.split(':').pop() || '6379') } : undefined);
    this.performanceMonitor = new QueryPerformanceMonitor();
    this.analysisCache = new EmailAnalysisCache({ maxSize: 2000, ttl: 3600000 }); // 1 hour TTL
  ...
  async analyzeEmail(
    ...
    try {
      ...
      const isCompleteChain = phase1Results.chain_analysis?.is_complete_chain || false;
      const chainScore = phase1Results.chain_analysis?.completeness_score || 0;
      
      logger.info(`Chain analysis: Complete=${isCompleteChain}, Score=${chainScore}, Type=${phase1Results.chain_analysis?.chain_type}`);
      ...
      logger.info(`Phase 2 complete in ${phase2Results.phase2_processing_time}ms`);
      ...
      if (isCompleteChain || options.forceAllPhases) {
        ...
        const phase3Results = await this.runPhase3(email, phase1Results, phase2Results, options);
        this.emit('phase:complete', { phase: 3, results: phase3Results });
        logger.info(`Phase 3 complete in ${phase3Results.phase3_processing_time}ms`);
        ...
        this.performanceMonitor.trackOperation('three_phase_analysis', totalTime, true);
        
        logger.info(`Three-phase analysis complete in ${totalTime}ms for complete chain`);
        this.emit('analysis:complete', { email: email.id, results: phase3Results, totalTime, phases: 3 });
        
        return phase3Results;
      } else {
        logger.info(`Skipping Phase 3 - incomplete chain (score: ${chainScore})`);
        
        // Create Phase 2 results with strategic defaults
        const phase2WithDefaults: Phase3Results = {
          ...phase2Results,
          strategic_insights: {
            opportunity: 'Incomplete chain - limited strategic analysis available',
            risk: 'Cannot assess full risk without complete workflow',
            relationship: 'Standard processing'
          },
          executive_summary: phase2Results.risk_assessment || 'Incomplete email chain - standard processing recommended',
          ...
          workflow_intelligence: {
            predicted_next_steps: ['Await additional correspondence for complete analysis'],
            bottleneck_risks: ['Incomplete information may delay processing'],
            optimization_opportunities: ['Complete email chain needed for workflow optimization']
          }
        };
        ...
        // Track performance
        const totalTime = Date.now() - startTime;
        this.performanceMonitor.trackOperation('two_phase_analysis', totalTime, true);
        
        logger.info(`Two-phase analysis complete in ${totalTime}ms for incomplete chain`);
        this.emit('analysis:complete', { email: email.id, results: phase2WithDefaults, totalTime, phases: 2 });
      ...
      this.performanceMonitor.trackOperation('email_analysis', Date.now() - startTime, false);
  ...
  private async runPhase1(
    ...
    const financial_impact = this.calculateFinancialImpact(entities.dollar_amounts);
  ...
  private async runPhase2(
    ...
    try {
      ...
      let prompt = enhancePromptForEmailType(PHASE2_ENHANCED_PROMPT, emailCharacteristics);
      prompt = prompt
      ...
      const results: Phase2Results = {
        ...phase1Results,
        workflow_validation: phase2Data.workflow_validation || `Confirmed: ${phase1Results.workflow_state}`,
        ...
        initial_response: phase2Data.initial_response || 'Thank you for your email. We are processing your request.',
      ...
      return {
        ...
        initial_response: 'Thank you for your email. We are reviewing your request.',
        confidence: 0.5,
  ...
  private async runPhase3(
    ...
    try {
      ...
      const prompt = PHASE3_STRATEGIC_PROMPT
        .replace('{PHASE1_RESULTS}', JSON.stringify(phase1Results, null, 2))
      ...
      const results: Phase3Results = {
        ...
        executive_summary: phase3Data.executive_summary || phase2Results.risk_assessment,
        escalation_needed: phase3Data.escalation_needed || false,
        revenue_impact: phase3Data.revenue_impact || `$${phase1Results.financial_impact}`,
  ...
  private extractPartNumbers(text: string): string[] {
    ...
    patterns.forEach(pattern => {
      ...
      matches.forEach(m => {
        ...
        if (!m.match(/^(THE|AND|FOR|WITH|FROM|THIS|THAT|HAVE|WILL|BEEN|EMAIL|PHONE|TODAY)$/)) {
          results.add(m);
        }
      });
    });
  ...
  private detectWorkflowState(content: string): string {
    if (content.includes('resolved') || content.includes('completed') || content.includes('closed')) {
      ...
      return 'IN_PROGRESS';
    } else if (content.includes('quote') || content.includes('pricing') || content.includes('cost')) {
      return 'QUOTE_PROCESSING';
    } else if (content.includes('order') || content.includes('purchase')) {
      return 'ORDER_MANAGEMENT';
    } else if (content.includes('ship') || content.includes('deliver') || content.includes('tracking')) {
      return 'SHIPPING';
    } else if (content.includes('return') || content.includes('rma') || content.includes('refund')) {
      return 'RETURNS';
    }
  ...
  private categorizeSender(senderEmail: string): string {
    ...
    const partnerDomains = ['@hp.com', '@dell.com', '@microsoft.com', '@cisco.com'];
  ...
  private detectPatterns(content: string, entities: Phase1Results['entities']): string[] {
    ...
    if (content.includes('dissatisfied') || content.includes('frustrated') || content.includes('disappointed')) {
      patterns.push('customer_dissatisfaction');
    }
    
    if (content.includes('competitor') || content.includes('alternative') || content.includes('switch')) {
  ...
  private async saveAnalysis(email: EmailInput, results: Phase3Results): Promise<void> {
    const id = `3phase_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const now = new Date().toISOString();
    
    const totalTime = results.processing_time + results.phase2_processing_time + results.phase3_processing_time;
   ...
   */
  async getAnalysisStats(startDate?: Date, endDate?: Date): Promise<AnalysisStats> {
