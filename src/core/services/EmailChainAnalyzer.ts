...
interface ChainAnalysis {
  ...
  chain_type: 'quote_request' | 'order_processing' | 'support_ticket' | 'general_inquiry' | 'unknown';
...
export class EmailChainAnalyzer {
   ...
   */
  async analyzeMultipleChains(emailIds: string[]): Promise<Map<string, ChainAnalysis>> {
  ...
  private getEmail(emailId: string): EmailChainNode | null {
    ...
    email.workflow_state = this.detectWorkflowState(email.subject + ' ' + (email.body || ''));
  ...
  private getEmailChain(email: EmailChainNode): EmailChainNode[] {
    ...
    if (email.thread_id) {
      ...
      threads.forEach(e => {
        e.workflow_state = this.detectWorkflowState(e.subject + ' ' + (e.body || ''));
        chainEmails.push(e);
  ...
  private analyzeChainStructure(emails: EmailChainNode[]): ChainAnalysis {
    ...
    const durationMs = new Date(lastEmail.received_at).getTime() - 
                      new Date(firstEmail.received_at).getTime();
    const durationHours = durationMs / (1000 * 60 * 60);
    
    // Analyze completeness
    const hasStartPoint = workflowStates.includes('START_POINT') || 
                         workflowStates.includes('QUOTE_PROCESSING') ||
                         workflowStates.includes('ORDER_MANAGEMENT');
    
    const hasMiddle = workflowStates.includes('IN_PROGRESS') ||
                     emails.length > 2;
    
    const hasCompletion = workflowStates.includes('COMPLETION') ||
  ...
  private detectWorkflowState(content: string): string {
    ...
    if (lowerContent.includes('resolved') || 
   ...
   */
  private extractChainEntities(emails: EmailChainNode[]): ChainAnalysis['key_entities'] {
    const entities = {
    ...
    emails.forEach(email => {
      const content = (email.subject + ' ' + (email as any).body || '').toLowerCase();
      
      // Extract quote numbers
      const quoteMatches = content.match(/\b(?:quote|q)\s*#?\s*(\d{6,10})\b/gi) || [];
      ...
      // Extract PO numbers
      const poMatches = content.match(/\b(?:po|p\.o\.|purchase\s*order)\s*#?\s*(\d{7,12})\b/gi) || [];
      poMatches.forEach(m => {
      ...
      const caseMatches = content.match(/\b(?:case|ticket|sr|inc)\s*#?\s*(\d{6,10})\b/gi) || [];
   ...
   */
  private detectChainType(emails: EmailChainNode[]): ChainAnalysis['chain_type'] {
    const allContent = emails.map(e => 
      (e.subject + ' ' + (e as any).body || '').toLowerCase()
    ).join(' ');
    ...
    if (allContent.includes('ticket') || allContent.includes('case') || 
        allContent.includes('issue') || allContent.includes('problem')) {
  ...
  private detectCompletionSignals(emails: EmailChainNode[]): boolean {
    const lastEmail = emails[emails.length - 1];
    const lastContent = (lastEmail.subject + ' ' + (lastEmail as any).body || '').toLowerCase();
    ...
    switch (params.chainType) {
      case 'quote_request':
        const hasQuoteNumber = params.emails.some(e => 
          (e.subject + ' ' + (e as any).body || '').match(/quote\s*#?\s*\d{6,10}/i)
        );
        if (!hasQuoteNumber) {
          score -= 10;
      ...
      case 'order_processing':
        const hasPONumber = params.emails.some(e => 
          (e.subject + ' ' + (e as any).body || '').match(/po\s*#?\s*\d{7,12}/i)
        );
        if (!hasPONumber) {
          score -= 10;
  ...
  async getChainStatistics(): Promise<{
    ...
    stats.average_chain_length = chains.size > 0 
      ? Math.round(totalLength / chains.size * 10) / 10 
      : 0;
