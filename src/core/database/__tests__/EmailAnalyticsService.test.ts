...
describe('EmailAnalyticsService', () => {
  ...
  describe('getTotalEmailsCount', () => {
    it('should return total email count', () => {
      ...
      expect(mockDb.prepare).toHaveBeenCalledWith('SELECT COUNT(*) as total FROM emails');
    });
  ...
  describe('getPendingEmailsCount', () => {
    it('should return pending email count', () => {
      ...
      expect(mockDb.prepare).toHaveBeenCalledWith(
        expect.stringContaining("WHERE ea.id IS NULL OR ea.workflow_state = 'PENDING'")
      );
    });
  ...
  describe('constructor', () => {
    it('should throw error if database connection fails', () => {
      ...
      expect(() => new EmailAnalyticsService(':memory:')).toThrow('Database connection failed');
    });
  });
});