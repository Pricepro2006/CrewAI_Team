...
export class ConfidenceResponseGenerator {
  ...
  async generateWithConfidence(
    ...
    try {
      ...
      return {
        response:
          "I'm sorry, but I encountered an error while generating a response. Please try rephrasing your question.",
        rawConfidence: 0.1,
        tokenConfidence: [],
  ...
  private buildConfidencePrompt(
    ...
    // Add system context
    sections.push(
      "You are an AI assistant that provides accurate and confident responses.",
    );
    sections.push(
      "When you're uncertain about information, clearly indicate your uncertainty.",
    );
    sections.push(
      "Focus on providing helpful, accurate information based on the context provided.",
    );
    ...
    if (options.includeUncertainty) {
      ...
      sections.push(
        "Use phrases like 'I'm not entirely sure', 'this might be', or 'according to the available information' when appropriate.",
      );
    }
  ...
  private identifyUncertaintyAreas(
    ...
    if (
      /\b(don't know|not sure|no information|unavailable|unknown)\b/i.test(
        response,
      )
    ) {
  ...
  private generateReasoning(
    ...
    const uncertaintyCount = (
      response.match(
        /\b(maybe|perhaps|possibly|might|could|may|uncertain|unclear|unsure)\b/gi,
      ) || []
    ).length;
