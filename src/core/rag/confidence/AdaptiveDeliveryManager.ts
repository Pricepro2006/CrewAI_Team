...
export class AdaptiveDeliveryManager {
  ...
  async deliver(
    ...
    const confidenceDisplay: ConfidenceDisplay = {
      ...
      display:
        options.confidenceFormat === "percentage"
      ...
      averageConfidence:
        this.deliveryHistory.length > 0
          ? this.deliveryHistory.reduce(
              (sum, d) => sum + d.confidence.score,
              0,
            ) / this.deliveryHistory.length
          : 0,
      humanReviewRate:
        this.deliveryHistory.filter((d) => d.metadata.humanReviewNeeded)
          .length / Math.max(1, this.deliveryHistory.length),
    };
  }
