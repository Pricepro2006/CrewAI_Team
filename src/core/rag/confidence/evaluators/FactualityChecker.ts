...
export class FactualityChecker {
   ...
   */
  private verifyClaim(claim: string, sources: ScoredDocument[]): {
    isSupported: boolean;
  ...
  private supportsClaimDirectly(claim: string, sourceText: string): boolean {
    ...
    const claimWords = claim.split(/\s+/).filter(word => 
   ...
   * Calculate confidence in factuality assessment
   */
  private calculateConfidence(sources: ScoredDocument[], claims: string[]): number {
   ...
   */
  private extractAndVerifyNumbers(response: string, sources: ScoredDocument[]): { claim: string; verified: boolean }[] {
   ...
   */
  private extractAndVerifyDates(response: string, sources: ScoredDocument[]): { claim: string; verified: boolean }[] {
   ...
   */
  private extractAndVerifyNames(response: string, sources: ScoredDocument[]): { claim: string; verified: boolean }[] {
   ...
   */
  private extractAndVerifyLocations(response: string, sources: ScoredDocument[]): { claim: string; verified: boolean }[] {
