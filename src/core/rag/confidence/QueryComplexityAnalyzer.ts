...
export class QueryComplexityAnalyzer {
  ...
  private calculateSemanticComplexity(query: string): number {
    ...
    if (
      /\b(compare|contrast|difference|similar|versus|vs|between)\b/.test(
        lowerQuery,
      )
    ) {
      score += 0.2;
    ...
    if (
      /\b(why|because|cause|effect|reason|due to|result)\b/.test(lowerQuery)
    ) {
      score += 0.1;
    }
    ...
    // Temporal complexity
    if (
      /\b(before|after|during|while|meanwhile|simultaneously)\b/.test(
        lowerQuery,
      )
    ) {
      score += 0.1;
  ...
  private generateReasoning(
    factors: QueryComplexity["factors"],
    score: number,
  ): string {
