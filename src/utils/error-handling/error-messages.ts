...
const errorMessages: Record<ErrorCode, (details?: any) => UserFriendlyError> = {
  [ErrorCode.BAD_REQUEST]: (details) => ({
    ...
    action:
      "Review the form fields and ensure all required information is provided correctly.",
  ...
  [ErrorCode.CONFLICT]: () => ({
    ...
    message:
      "This action conflicts with the current state. The resource may have been modified.",
  ...
  [ErrorCode.RATE_LIMIT_EXCEEDED]: (details) => ({
    ...
    action: details?.retryAfter
      ? `Try again in ${Math.ceil(details.retryAfter / 1000)} seconds.`
  ...
  [ErrorCode.EXTERNAL_SERVICE_ERROR]: (details) => ({
    ...
    message: details?.service
      ...
      : "An external service is not responding.",
  ...
  [ErrorCode.OLLAMA_MODEL_NOT_FOUND]: (details) => ({
    ...
    action:
      "Install the model or select a different one from available models.",
  }),
