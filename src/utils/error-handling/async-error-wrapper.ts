...
>(fn: T, options: AsyncErrorOptions = {}): T {
  const {
    fallbackValue,
    retries = 0,
    retryDelay = 1000,
  ...
  return (async (...args: Parameters<T>): Promise<ReturnType<T>> => {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        logger.error(
          `${context}: Error on attempt ${attempt + 1}/${retries + 1}`,
          "ASYNC_ERROR_WRAPPER",
          {
        ...
        if (attempt === retries) {
          if (critical) {
            ...
            logger.warn(
              `${context}: Returning fallback value after all retries failed`,
            );
...
export class GracefulShutdown {
  ...
  setupSignalHandlers(): void {
    if (this.signalHandlersRegistered) {
      logger.warn(
        "Signal handlers already registered, skipping duplicate registration",
      );
      return;
    }
    ...
    process.on("unhandledRejection", (reason, promise) => {
      ...
      if (
        ...
        logger.warn(
          "Ignoring empty/malformed promise rejection",
          "GRACEFUL_SHUTDOWN",
          { reason, promise },
        );
        return;
      ...
      logger.error("Unhandled rejection", "GRACEFUL_SHUTDOWN", {
        reason,
        promise,
      });
