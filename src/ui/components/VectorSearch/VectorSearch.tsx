...
export const VectorSearch: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const transformedResults: SearchResult[] = response.map((item: any, index: number) => ({
        id: item.metadata?.id || `result-${index}`,
        content: item.content || item.text || 'No content available',
        metadata: {
          source: item.metadata?.title || item.metadata?.source || 'Unknown source',
    ...
    <div className="vector-search-container">
      <div className="vector-search-header">
        ...
        <p className="vector-search-description">
          Semantic search powered by vector embeddings. Find relevant information using natural language queries.
        </p>
      </div>
                ...
                <div className="result-header">
                  <div className="result-metadata">
                    ...
                    {result.metadata.page && (
                      <span className="metadata-item">Page {result.metadata.page}</span>
                    )}
                  </div>
      ...
      <div className="info-section">
        ...
        <div className="info-grid">
          <div className="info-card">
            ...
            <p>Documents are converted into high-dimensional vectors that capture semantic meaning.</p>
          </div>
          <div className="info-card">
            <div className="info-icon">
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <p>Find documents with similar semantic meaning using cosine similarity.</p>
          </div>
          <div className="info-card">
            <div className="info-icon">
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <p>Optimized vector databases enable lightning-fast similarity searches.</p>
          </div>
        </div>
      </div>
    </div>
