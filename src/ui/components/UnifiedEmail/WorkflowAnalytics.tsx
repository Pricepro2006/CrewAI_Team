...
export const WorkflowAnalytics: React.FC<WorkflowAnalyticsProps> = ({ className }) => {
  const { data, isLoading } = trpc.unifiedEmail.getWorkflowAnalytics.useQuery();
  ...
  const completePercentage = (workflowData.completeChains / workflowData.totalChains * 100).toFixed(1);
  const partialPercentage = (workflowData.partialChains / workflowData.totalChains * 100).toFixed(1);
  const brokenPercentage = (workflowData.brokenChains / workflowData.totalChains * 100).toFixed(1);
      ...
      <div className="workflow-analytics__header">
        <div className="workflow-analytics__alert">
          ...
          <div>
            ...
            <p>
              Only <strong>{completePercentage}%</strong> of workflows have complete chains 
              (Start → In Progress → Completion). This significantly impacts visibility and tracking.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="card-content">
            <div className="workflow-stats">
              <div className="workflow-stat workflow-stat--complete">
                <div className="workflow-stat__value">{completePercentage}%</div>
                <div className="workflow-stat__label">Complete Chains</div>
                <div className="workflow-stat__count">({workflowData.completeChains.toLocaleString()} workflows)</div>
              </div>
              <div className="workflow-stat workflow-stat--partial">
                ...
                <div className="workflow-stat__count">({workflowData.partialChains.toLocaleString()} workflows)</div>
              </div>
              <div className="workflow-stat workflow-stat--broken">
                ...
                <div className="workflow-stat__count">({workflowData.brokenChains.toLocaleString()} workflows)</div>
              </div>
            </div>
              ...
              {workflowData.workflowTypes.map((type) => (
                <div key={type.type} className="workflow-type">
                  <div className="workflow-type__header">
                    ...
                    <span className="workflow-type__count">{type.count.toLocaleString()}</span>
                  </div>
              ...
              {workflowData.bottlenecks.map((bottleneck) => (
                <div key={bottleneck.stage} className="bottleneck">
                  <div className="bottleneck__info">
                    <span className="bottleneck__stage">{bottleneck.stage}</span>
                    <span className="bottleneck__count">{bottleneck.count.toLocaleString()} stuck</span>
                  </div>
                  <div className="bottleneck__delay">
                    <span className="bottleneck__delay-label">Avg delay:</span>
                    <span className="bottleneck__delay-value">{bottleneck.avgDelayHours.toFixed(1)}h</span>
                  </div>
                </div>
              ))}
            </div>
                ...
                .map((rec, index) => (
                  <div key={index} className={`recommendation recommendation--${rec.priority}`}>
                    <div className="recommendation__header">
                      <span className="recommendation__priority">{rec.priority.toUpperCase()}</span>
                      <h4>{rec.title}</h4>
                    </div>
      ...
      <div className="workflow-analytics__insights">
        ...
        <div className="insights-grid">
          <div className="insight">
            <h4>Root Causes</h4>
            <ul>
              <li>Multi-channel communication causing workflow fragmentation</li>
