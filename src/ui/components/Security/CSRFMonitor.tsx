...
export const CSRFMonitor: React.FC<CSRFMonitorProps> = ({
  ...
  const { hasToken, tokenAge, lastRefresh, error, isLoading, status } =
    useCSRFStatus();
  ...
  return (
    <div className={`inline-flex items-center ${className}`}>
      ...
      {error && (
        <div
          className="ml-2 text-xs text-red-600 max-w-[200px] truncate"
          title={error.message}
        >
          {error.message}
        </div>
 ...
 */
export const CSRFStatusBadge: React.FC<{ className?: string }> = ({
  className = "",
}) => {
        ...
        <div className="flex items-center justify-center min-h-[200px] p-4">
          <div className="text-center max-w-md">
            ...
            <p className="text-sm text-gray-600 mb-4">
              We encountered a security validation error. This is usually
