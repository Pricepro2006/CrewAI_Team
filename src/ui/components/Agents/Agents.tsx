...
const formatAgentName = (type: string): string => {
  return type.replace(/Agent$/, ' Agent').replace(/([A-Z])/g, ' $1').trim();
};

const mapCapabilitiesToExpertise = (capabilities: string[]): string[] => {
  return capabilities.map(cap => 
    cap.split('_').map(word => 
...
export const Agents: React.FC = () => {
  ...
  const { data: agentsData, isLoading, error } = (api.agent as any).list.useQuery();
  ...
  const agents: Agent[] = React.useMemo(() => {
    ...
    return agentsData.map((agent: AgentData) => {
      ...
      const isActive = agentStatus?.some((status: any) => 
            ...
            <div key={i} className="agent-card" style={{ opacity: 0.5 }}>
              <div className="agent-card-header">
                <div className="agent-icon">
                  <div style={{ width: 48, height: 48, backgroundColor: '#e5e7eb', borderRadius: '50%' }} />
                </div>
                <div className="agent-status" style={{ backgroundColor: '#e5e7eb' }} />
              </div>
              <div style={{ height: 24, backgroundColor: '#e5e7eb', marginBottom: 8, borderRadius: 4 }} />
              <div style={{ height: 48, backgroundColor: '#e5e7eb', marginBottom: 16, borderRadius: 4 }} />
              <div style={{ height: 80, backgroundColor: '#e5e7eb', borderRadius: 4 }} />
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div className="agents-container">
        <div className="agents-header">
          ...
          <button className="chat-with-all-button" onClick={() => window.location.reload()}>
            Retry
          </button>
        </div>
      </div>
