import React, { useState, useEffect } from "react";
import { toastManager, Toast } from "./useToast.js";
import "./ToastContainer.css";

interface ToastContainerProps {
  position?: "top-left" | "top-right" | "bottom-left" | "bottom-right" | "top-center" | "bottom-center";
  maxToasts?: number;
}

export const EnhancedToastContainer: React.FC<ToastContainerProps> = ({
  position = "top-right",
  maxToasts = 5,
}) => {
  const [toasts, setToasts] = useState<Toast[]>([]);

  useEffect(() => {
    // Subscribe to toast manager
    const unsubscribe = toastManager.subscribe((newToasts: any) => {
      setToasts(newToasts.slice(0, maxToasts));
    });

    return unsubscribe;
  }, [maxToasts]);

  const getToastIcon = (type: Toast["type"]) => {
    switch (type) {
      case "success":
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
              clipRule="evenodd"
            />
          </svg>
        );
      case "error":
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
              clipRule="evenodd"
            />
          </svg>
        );
      case "warning":
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
              clipRule="evenodd"
            />
          </svg>
        );
      case "info":
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
              clipRule="evenodd"
            />
          </svg>
        );
    }
  };

  const getToastClasses = (type: Toast["type"]) => {
    const baseClasses = "toast-item flex items-start p-4 mb-3 rounded-lg shadow-lg transition-all duration-300 animate-slide-in";
    
    switch (type) {
      case "success":
        return `${baseClasses} bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-200`;
      case "error":
        return `${baseClasses} bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-200`;
      case "warning":
        return `${baseClasses} bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-yellow-800 dark:text-yellow-200`;
      case "info":
        return `${baseClasses} bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-200`;
    }
  };

  const handleDismiss = (id: string) => {
    toastManager.remove(id);
  };

  return (
    <div
      className={`toast-container fixed z-50 pointer-events-none ${position}`}
      style={{
        top: position.includes("top") ? "1rem" : undefined,
        bottom: position.includes("bottom") ? "1rem" : undefined,
        left: position.includes("left") ? "1rem" : position === "top-center" || position === "bottom-center" ? "50%" : undefined,
        right: position.includes("right") ? "1rem" : undefined,
        transform: (position === "top-center" || position === "bottom-center") ? "translateX(-50%)" : undefined,
      }}
    >
      {toasts?.map((toast: any) => (
        <div
          key={toast.id}
          className={getToastClasses(toast.type)}
          style={{ pointerEvents: "auto" }}
        >
          <div className="flex-shrink-0 mr-3">
            {getToastIcon(toast.type)}
          </div>
          <div className="flex-1 mr-2">
            <p className="text-sm font-medium">{toast.message}</p>
            {toast.options?.action && (
              <button
                onClick={toast?.options?.action.onClick}
                className="mt-2 text-sm font-medium underline hover:no-underline"
              >
                {toast?.options?.action.label}
              </button>
            )}
          </div>
          {toast.options?.duration !== 0 && (
            <button
              onClick={() => handleDismiss(toast.id)}
              className="flex-shrink-0 ml-auto text-current opacity-70 hover:opacity-100 transition-opacity"
              aria-label="Dismiss"
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
                  d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
          )}
        </div>
      ))}
    </div>
  );
};

// CSS for animations (add to ToastContainer.css)
const toastStyles = `
@keyframes slide-in {
  from {
    transform: translateX(100%);
    opacity: 0;
  }
  to {
    transform: translateX(0);
    opacity: 1;
  }
}

.animate-slide-in {
  animation: slide-in 0.3s ease-out;
}

.toast-container.top-left .animate-slide-in,
.toast-container.bottom-left .animate-slide-in {
  animation-name: slide-in-left;
}

@keyframes slide-in-left {
  from {
    transform: translateX(-100%);
    opacity: 0;
  }
  to {
    transform: translateX(0);
    opacity: 1;
  }
}

.toast-container.top-center .animate-slide-in,
.toast-container.bottom-center .animate-slide-in {
  animation-name: slide-in-center;
}

@keyframes slide-in-center {
  from {
    transform: translateY(-100%);
    opacity: 0;
  }
  to {
    transform: translateY(0);
    opacity: 1;
  }
}

.toast-container.bottom-center .animate-slide-in {
  animation-name: slide-in-bottom;
}

@keyframes slide-in-bottom {
  from {
    transform: translateY(100%);
    opacity: 0;
  }
  to {
    transform: translateY(0);
    opacity: 1;
  }
}
`;