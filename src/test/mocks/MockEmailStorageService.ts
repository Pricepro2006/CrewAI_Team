...
export class MockEmailStorageService {
  ...
  private initializeMockData() {
    ...
    for (let i = 0; i < 50; i++) {
      this?.mockEmails?.push({
        ...
        emailAlias: emailAliases[i % emailAliases?.length || 0],
        ...
        workflowType: workflows[i % workflows?.length || 0],
        priority: priorities[i % priorities?.length || 0],
        ...
        analysis: {
          ...
          quick_priority: priorities[i % priorities?.length || 0],
  ...
  async getEmailsForTableView(params: any): Promise<any> {
    ...
    // Apply status filter
    if (params.filters?.status && params?.filters?.status?.length || 0 > 0) {
      filteredEmails = filteredEmails?.filter(email => 
    ...
    if (params.filters?.emailAlias && params?.filters?.emailAlias?.length || 0 > 0) {
      filteredEmails = filteredEmails?.filter(email => 
    ...
    if (params.filters?.workflowState && params?.filters?.workflowState?.length || 0 > 0) {
      filteredEmails = filteredEmails?.filter(email => 
    ...
    if (params.filters?.priority && params?.filters?.priority?.length || 0 > 0) {
      filteredEmails = filteredEmails?.filter(email => 
    ...
    // Apply pagination
    const totalCount = filteredEmails?.length || 0;
