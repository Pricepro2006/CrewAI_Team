  ...
  async runAllChecks(): Promise<Map<string, HealthCheckResult>> {
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === "fulfilled") {
        ...
        healthResults.set(checkName || `unknown-check-${index}`, {
          status: "unhealthy",
    ...
    check: async (): Promise<HealthCheckResult> => {
      try {
        ...
        return {
          status: "unhealthy",
          message:
            error instanceof Error ? error.message : "Ollama check failed",
          responseTime: 0,
        };
      }
    },
    ...
    check: async (): Promise<HealthCheckResult> => {
      try {
        ...
        return {
          status: "unhealthy",
          message:
            error instanceof Error ? error.message : "ChromaDB check failed",
          responseTime: 0,
        };
      }
    },
...
export function createHttpMetricsCollector(): MetricCollectorFunction {
  return async (collector: MetricsCollector) => {
    ...
    collector.gauge("system_memory_usage_bytes", memUsage.heapUsed, {
      type: "heap_used",
    });
    collector.gauge("system_memory_usage_bytes", memUsage.heapTotal, {
      type: "heap_total",
    });
    collector.gauge("system_memory_usage_bytes", memUsage.external, {
      type: "external",
    });
