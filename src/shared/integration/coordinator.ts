...
export class IntegrationCoordinator {
  ...
  private async startHealthChecks(): Promise<void> {
    ...
    if (health.status === 'unhealthy') {
      console.warn('⚠️  Some critical services are unhealthy:', 
  ...
  async createTestEnvironment(config: Partial<TestConfig> = {}): Promise<TestContext> {
    if (!this.initialized) {
      throw new Error('Integration coordinator must be initialized before creating test environment');
    }

    const testConfig: TestConfig = {
      baseUrl: process.env.TEST_BASE_URL || 'http://localhost:3001',
      websocketUrl: process.env.TEST_WEBSOCKET_URL || 'ws://localhost:3002/trpc-ws',
      timeout: 10000,
      retries: 2,
      parallel: false,
      cleanup: true,
  ...
  async executeWithResilience<T>(
    ...
    const circuitBreakerOp = () => this.executeWithCircuitBreaker(serviceName, operation);
  ...
  recordHttpRequest(method: string, route: string, status: number, duration: number): void {
    ...
    globalMetricsCollector.histogram('http_request_duration_seconds', duration / 1000, {
      method,
      route
    });
  }
  ...
  recordAgentTask(agentType: string, status: string, duration: number): void {
    ...
    globalMetricsCollector.histogram('agent_task_duration_seconds', duration / 1000, {
      agent_type: agentType
    });
  }

  recordLlmRequest(model: string, provider: string, tokens: number, duration: number): void {
    globalMetricsCollector.increment('llm_requests_total', {
      model,
      provider
    });
    
    globalMetricsCollector.increment('llm_tokens_total', {
      model,
      type: 'total'
    }, tokens);
    
    globalMetricsCollector.histogram('llm_request_duration_seconds', duration / 1000, {
      model
    });
  }

  recordDatabaseOperation(operation: string, duration: number, error?: boolean): void {
    globalMetricsCollector.histogram('database_query_duration_seconds', duration / 1000, {
      operation
    });
  ...
  setActiveConnections(type: 'http' | 'websocket' | 'database', count: number): void {
    const metricMap = {
