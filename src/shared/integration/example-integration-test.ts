...
describe('CrewAI Team Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    await integrationCoordinator.initialize({
      ...
      services: {
        ...
        monitoring: {
          metrics: { enabled: true, interval: 5000, retention: 300, aggregation: ['count'], exports: [] },
          logging: { level: 'debug', format: 'json', outputs: [{ type: 'console' }] },
          tracing: { enabled: false, serviceName: 'test-crewai', sampleRate: 1.0 },
          alerts: []
        }
      },
      security: {
        ...
        authorization: {
          rbac: {
            roles: [
              ...
              { name: 'user', description: 'Test user', permissions: ['read', 'write'] }
            ],
            inheritance: false,
            caching: false
          },
        ...
        cors: { origins: ['http://localhost:3000'], methods: ['GET', 'POST'], headers: ['Content-Type'], credentials: true },
        csrf: { enabled: false, cookie: { name: '_test_csrf', secure: false, httpOnly: true, sameSite: 'lax' } },
        rateLimit: {
  ...
  describe('Authentication Flow', () => {
    it('should authenticate user and return valid token', async () => {
      ...
      authToken = await authenticateUser(testContext, testContext.config.authentication.defaultUser);
  ...
  describe('Email Dashboard Integration', () => {
    ...
    it('should list emails with pagination', async () => {
      const response = await testContext.services.http.get('/api/emails?page=1&pageSize=10');
      
      expect(response).toHaveStatus(200);
      expect(response).toHaveResponseTime(5000); // Max 5 seconds
    ...
    it('should filter emails by status', async () => {
      const response = await testContext.services.http.get('/api/emails?statuses=red,yellow&page=1&pageSize=20');
      ...
      console.log(`✅ Filtered emails by status: ${data.emails.length} results`);
    });
    ...
    it('should create and update email records', async () => {
      ...
      const updateResponse = await testContext.services.http.patch(`/api/emails/${createdEmail.id}`, updateData);
      ...
      const deleteResponse = await testContext.services.http.delete(`/api/emails/${createdEmail.id}`);
  ...
  describe('WebSocket Real-time Updates', () => {
    it('should connect to WebSocket and receive messages', async () => {
      ...
      const wsMessage = await testContext.services.websocket.waitForMessage('email.create', 10000);
  ...
  describe('Agent Task Processing', () => {
    it('should create and execute agent tasks', async () => {
      ...
      const response = await testContext.services.http.post('/api/tasks', taskRequest);
      ...
      while (attempts < maxAttempts && !completedTask) {
        ...
        const statusResponse = await testContext.services.http.get(`/api/tasks/${task.id}`);
  ...
  describe('Error Handling and Resilience', () => {
    it('should handle API errors gracefully', async () => {
      // Test 404 error
      const notFoundResponse = await testContext.services.http.get('/api/emails/non-existent-id');
      expect(notFoundResponse).toHaveStatus(404);
      ...
      const validationResponse = await testContext.services.http.post('/api/emails', invalidEmail);
      expect(validationResponse).toHaveStatus(400);
    ...
    it('should implement rate limiting', async () => {
      ...
      const response = await testContext.services.http.get('/api/emails?page=1&pageSize=1');
      ...
      if (hasRateLimitHeaders) {
        ...
        console.log('ℹ️  Rate limiting headers not detected (may not be implemented)');
      }
    });
  });
 ...
 */
export async function createSampleEmailData(context: TestContext, count: number = 5): Promise<EmailRecord[]> {
  ...
  for (let i = 0; i < count; i++) {
    const email = {
      ...
      workflow_state: (['START_POINT', 'IN_PROGRESS', 'COMPLETION'] as const)[i % 3],
