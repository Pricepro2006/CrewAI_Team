...
export const CONFIDENCE_PROFILES: Record<string, ConfidenceProfile> = {
  conservative: {
    name: "Conservative",
    description:
      "High confidence requirements, suitable for critical applications",
    config: {
      retrieval: {
        minimum: 0.7,
        preferred: 0.85,
  ...
  research: {
    name: "Research",
    description:
      "Optimized for research and exploration with detailed evidence",
    config: {
      retrieval: {
        minimum: 0.5,
        preferred: 0.7,
...
export function getConfidenceProfile(
  ...
  const profile = CONFIDENCE_PROFILES[profileName as keyof typeof CONFIDENCE_PROFILES];
  if (!profile) {
 ...
 */
export function getEnvironmentProfile(): ConfidenceProfile {
