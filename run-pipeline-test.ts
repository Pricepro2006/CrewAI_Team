...
async function runPipelineTest() {
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(
        () => reject(new Error("Pipeline test timeout after 10 minutes")),
        timeoutMs,
      );
    });
    ...
    if (results.results && results.results.length > 0) {
      ...
      for (const result of sampleResults) {
        ...
        if (result.stage2) {
          console.log(
            `     Business Process: ${result.stage2.businessProcess}`,
          );
    ...
    if (executionRecord) {
      ...
      console.log(
        `   Processing Time: ${((executionRecord as any).total_processing_time_seconds || 0).toFixed(2)}s`,
      );
    }
