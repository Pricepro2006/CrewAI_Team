...
async function testFullPipeline() {
  ...
  try {
    ...
    if (emailCount.count === 0) {
      console.log(
        "⚠️  No emails found in database. Pipeline test will still verify components work.",
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        "ℹ️  This may be due to model loading time or memory constraints",
      );
    }
    ...
    console.log();
    console.log(
      "🚀 Ready to run full pipeline with: orchestrator.runThreeStagePipeline()",
    );
  } catch (error) {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("ECONNREFUSED")) {
        ...
        console.log(
          "  - Verify port 11434 is open: netstat -tulpn | grep 11434",
        );
      }
      ...
      if (error.message.includes("timeout")) {
        ...
        console.log(
          "  - Try running: ollama run doomgrave/phi-4:14b-tools-Q3_K_S",
        );
