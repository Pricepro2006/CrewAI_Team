...
import { webSearchRateLimit, businessSearchRateLimit, premiumRateLimit, } from "../../core/middleware/RateLimiter";
 ...
 */
router.post("/search/business", businessSearchRateLimit, // Apply business search rate limit
async (req, res) => {
    try {
 ...
 */
router.post("/search/web", webSearchRateLimit, // Apply web search rate limit
async (req, res) => {
    try {
 ...
 */
router.post("/search/premium", premiumRateLimit, // Apply premium rate limit
async (req, res) => {
    try {
...
router.get("/metrics", async (req, res) => {
    ...
    res.json({
        ...
        rateLimiting: {
            ...
            percentageRateLimited: metrics.totalRequests > 0
...
businessSearchMiddleware.on("validation_failed", (event) => {
    logger.warn("Response validation failed", JSON.stringify({
        prompt: typeof event.prompt === "string"
...
//# sourceMappingURL=businessSearch.js.map