...
export const emailRouter = router({
    ...
    getDashboardStats: publicProcedure
        ...
        try {
            ...
            try {
                ...
                logger.error("Failed to broadcast stats update", "EMAIL_ROUTER", {
                    error,
                });
            }
            return {
    ...
    getAnalytics: publicProcedure
        ...
        try {
            ...
            try {
                const { wsService } = await import("../services/WebSocketService");
                wsService.broadcastEmailAnalyticsUpdated(analytics.totalEmails, analytics.workflowDistribution, analytics.slaCompliance, analytics.averageProcessingTime);
            }
            catch (error) {
                logger.error("Failed to broadcast analytics update", "EMAIL_ROUTER", {
                    error,
                });
            }
            return {
    ...
    getList: publicProcedure
        ...
        try {
            ...
            let emails = [];
            if (input.workflow) {
                emails = await emailStorage.getEmailsByWorkflow(input.workflow, input.limit, input.offset);
            }
            else {
            ...
            if (input.search) {
                emails = emails.filter((email) => email.subject
                    .toLowerCase()
            ...
            if (input.priority) {
                emails = emails.filter((email) => email.analysis?.quick_priority === input.priority);
            }
            if (input.status) {
                emails = emails.filter((email) => email.analysis?.workflow_state === input.status);
            }
            if (input.slaStatus) {
                emails = emails.filter((email) => email.analysis?.action_sla_status === input.slaStatus);
            }
            if (input.dateRange) {
                emails = emails.filter((email) => {
                    const emailDate = new Date(email.receivedDateTime);
                    return (emailDate >= input.dateRange.start &&
    ...
    updateWorkflowState: protectedProcedure
        ...
        try {
            ...
            const changedBy = ctx.user?.email || ctx.user?.name || "system";
    ...
    updateStatus: protectedProcedure
        ...
        try {
            ...
            await emailStorage.updateEmailStatus(input.id, input.status, input.status_text, changedBy);
    ...
    batchCreateEmails: protectedProcedure
        ...
        try {
            ...
            try {
                const { wsService } = await import("../services/WebSocketService");
                wsService.broadcastEmailBatchCreated(input.batchId || `batch_${Date.now()}`, results.length, errors.length);
            }
            catch (error) {
    ...
    batchUpdateStatuses: protectedProcedure
        ...
        try {
            ...
            const results = [];
            const errors = [];
            const changedBy = input.changedBy || ctx.user?.email || "system";
            for (const update of input.updates) {
                try {
                    await emailStorage.updateEmailStatus(update.emailId, update.status, update.statusText, changedBy);
            ...
            try {
                const { wsService } = await import("../services/WebSocketService");
                wsService.broadcastEmailBatchStatusUpdated(input.updates.map((u) => u.emailId), results.length, errors.length, changedBy);
            }
            catch (error) {
    ...
    batchDelete: protectedProcedure
        ...
        try {
            ...
            try {
                const { wsService } = await import("../services/WebSocketService");
                wsService.broadcastEmailBatchDeleted(input.emailIds, results.length, errors.length, input.softDelete);
            }
            catch (error) {
