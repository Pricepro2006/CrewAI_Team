...
describe("Email Router", () => {
    ...
    beforeEach(() => {
        ...
        EmailStorageService.mockImplementation(() => mockEmailStorage);
    ...
    describe("Analytics Endpoint", () => {
        it("should return email analytics and broadcast update", async () => {
            ...
            mockEmailStorage.getWorkflowAnalytics.mockResolvedValueOnce(mockAnalytics);
            ...
            expect(wsService.broadcastEmailAnalyticsUpdated).toHaveBeenCalledWith(mockAnalytics.totalEmails, mockAnalytics.workflowDistribution, mockAnalytics.slaCompliance, mockAnalytics.averageProcessingTime);
            expect(logger.info).toHaveBeenCalledWith("Fetching email analytics", "EMAIL_ROUTER");
        });
        it("should handle analytics fetch errors", async () => {
            const errorMessage = "Database connection failed";
            mockEmailStorage.getWorkflowAnalytics.mockRejectedValueOnce(new Error(errorMessage));
            const caller = emailRouter.createCaller(mockContext);
            await expect(caller.getAnalytics({ refreshKey: 1 })).rejects.toThrow("Failed to fetch email analytics");
            expect(logger.error).toHaveBeenCalledWith("Failed to fetch email analytics", "EMAIL_ROUTER", { error: expect.any(Error) });
        });
        it("should handle WebSocket broadcast failures gracefully", async () => {
            ...
            mockEmailStorage.getWorkflowAnalytics.mockResolvedValueOnce(mockAnalytics);
            wsService.broadcastEmailAnalyticsUpdated.mockRejectedValueOnce(new Error("WebSocket error"));
            const caller = emailRouter.createCaller(mockContext);
            const result = await caller.getAnalytics({ refreshKey: 1 });
            expect(result.success).toBe(true);
            expect(result.data).toEqual(mockAnalytics);
            expect(logger.error).toHaveBeenCalledWith("Failed to broadcast analytics update", "EMAIL_ROUTER", { error: expect.any(Error) });
        });
    });
    describe("Email List Endpoint", () => {
        it("should return filtered email list", async () => {
            ...
            expect(mockEmailStorage.getEmailsByWorkflow).toHaveBeenCalledWith("Order Management", 50, 0);
            expect(logger.info).toHaveBeenCalledWith("Fetching email list", "EMAIL_ROUTER", { filters: { workflow: "Order Management", limit: 50, offset: 0 } });
        });
        ...
        it("should handle list fetch errors", async () => {
            mockEmailStorage.getEmailsByWorkflow.mockRejectedValueOnce(new Error("Database error"));
            const caller = emailRouter.createCaller(mockContext);
            await expect(caller.getList({
                workflow: "Order Management",
                limit: 50,
            })).rejects.toThrow("Failed to fetch email list");
            expect(logger.error).toHaveBeenCalledWith("Failed to fetch email list", "EMAIL_ROUTER", { error: expect.any(Error) });
        });
    });
    describe("Email Details Endpoint", () => {
        it("should return email details by ID", async () => {
            ...
            expect(mockEmailStorage.getEmailWithAnalysis).toHaveBeenCalledWith("email-1");
        });
        it("should handle email not found", async () => {
            mockEmailStorage.getEmailWithAnalysis.mockResolvedValueOnce(null);
            const caller = emailRouter.createCaller(mockContext);
            await expect(caller.getById({ id: "nonexistent-email" })).rejects.toThrow("Failed to fetch email");
        });
        it("should handle fetch errors", async () => {
            mockEmailStorage.getEmailWithAnalysis.mockRejectedValueOnce(new Error("Database error"));
            const caller = emailRouter.createCaller(mockContext);
            await expect(caller.getById({ id: "email-1" })).rejects.toThrow("Failed to fetch email details");
        });
    });
    describe("Workflow State Update Endpoint", () => {
        it("should update workflow state with user context", async () => {
            ...
            expect(mockEmailStorage.updateWorkflowState).toHaveBeenCalledWith("email-1", "Completed", "test@example.com");
            expect(logger.info).toHaveBeenCalledWith("Updating workflow state", "EMAIL_ROUTER", { emailId: "email-1", newState: "Completed" });
        });
        it("should handle update errors", async () => {
            mockEmailStorage.updateWorkflowState.mockRejectedValueOnce(new Error("Database error"));
            const caller = emailRouter.createCaller(mockContext);
            await expect(caller.updateWorkflowState({
                emailId: "email-1",
                newState: "Completed",
    ...
    describe("Bulk Update Endpoint", () => {
        it("should perform bulk archive operation", async () => {
            ...
            expect(mockEmailStorage.updateWorkflowState).toHaveBeenCalledWith("email-1", "Archived");
            expect(mockEmailStorage.updateWorkflowState).toHaveBeenCalledWith("email-2", "Archived");
            expect(wsService.broadcastEmailBulkUpdate).toHaveBeenCalledWith("archive", ["email-1", "email-2"], { successful: 2, failed: 0, total: 2 });
        });
        it("should handle partial failures in bulk operations", async () => {
            ...
            expect(wsService.broadcastEmailBulkUpdate).toHaveBeenCalledWith("archive", ["email-1", "email-2"], { successful: 1, failed: 1, total: 2 });
        });
        it("should handle change-state bulk operation", async () => {
            ...
            expect(result.success).toBe(true);
            expect(mockEmailStorage.updateWorkflowState).toHaveBeenCalledWith("email-1", "In Progress");
        });
        it("should handle WebSocket broadcast failures in bulk operations", async () => {
            mockEmailStorage.updateWorkflowState.mockResolvedValue(undefined);
            wsService.broadcastEmailBulkUpdate.mockRejectedValueOnce(new Error("WebSocket error"));
            const caller = emailRouter.createCaller(mockContext);
            const result = await caller.bulkUpdate({
                emailIds: ["email-1"],
                action: "archive",
            ...
            expect(result.success).toBe(true);
            expect(logger.error).toHaveBeenCalledWith("Failed to broadcast bulk update completion", "EMAIL_ROUTER", { error: "Error: WebSocket error" });
        });
    });
    describe("Workflow Patterns Endpoint", () => {
        it("should return workflow patterns", async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith("Fetching workflow patterns", "EMAIL_ROUTER");
        });
        it("should handle workflow patterns fetch errors", async () => {
            mockEmailStorage.getWorkflowPatterns.mockRejectedValueOnce(new Error("Database error"));
            const caller = emailRouter.createCaller(mockContext);
            await expect(caller.getWorkflowPatterns()).rejects.toThrow("Failed to fetch workflow patterns");
        });
    });
    ...
    describe("Send Email Endpoint", () => {
        it("should simulate sending email", async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith("Sending email", "EMAIL_ROUTER", { to: 1, subject: "Test Email", template: undefined });
        });
    ...
    describe("Search Endpoint", () => {
        it("should return search results placeholder", async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith("Searching emails", "EMAIL_ROUTER", { query: "test query", filters: { workflow: "Order Management" } });
        });
    ...
    describe("WebSocket Subscriptions", () => {
        it("should create WebSocket subscription", async () => {
            ...
            wsService.off.mockImplementation(() => { });
            wsService.unsubscribe.mockImplementation(() => { });
            const caller = emailRouter.createCaller(mockContext);
...
//# sourceMappingURL=email.router.test.js.map