...
export const confidenceChatRouter = createFeatureRouter("confidence-chat", router({
    // Create a new conversation with confidence scoring
    create: publicProcedure
        .input(confidenceChatSchemas.message)
        .mutation(async ({ input, ctx }) => {
        logger.info("Creating confidence-scored chat conversation", "CONFIDENCE_CHAT", {
            userId: ctx.user?.id,
        ...
        await ctx.conversationService.addMessage(conversation.id, {
            ...
            metadata: {
                ...
                humanReviewNeeded: result.deliveredResponse.metadata.humanReviewNeeded,
            },
        });
    ...
    exportWithConfidence: publicProcedure
        ...
        switch (input.format) {
            case "markdown":
                data = formatConversationAsMarkdown(conversation, input.includeEvaluations);
                break;
            case "csv":
                data = formatConversationAsCSV(conversation, input.includeEvaluations);
                break;
            default:
                data = conversation;
        }
...
function formatConversationAsMarkdown(conversation, includeEvaluations) {
...
function formatConversationAsCSV(conversation, includeEvaluations) {
    ...
    conversation.messages.forEach((msg, idx) => {
        ...
        if (includeEvaluations) {
            row.push(msg.metadata?.confidence?.toFixed(3) || "", msg.metadata?.processingPath || "", msg.metadata?.action || "", msg.metadata?.feedbackId || "");
        }
        rows.push(row.join(","));
    });
    return rows.join("\n");
}
//# sourceMappingURL=confidence-chat.router.js.map