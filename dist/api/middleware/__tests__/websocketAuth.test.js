...
import { WebSocketAuthManager, } from "../websocketAuth";
...
describe("WebSocketAuthManager", () => {
    ...
    describe("handleAuthMessage", () => {
        it("should handle valid auth message", async () => {
            ...
            expect(mockWs.send).toHaveBeenCalledWith(expect.stringContaining('"type":"auth_response"'));
        ...
        it("should handle invalid auth message format", async () => {
            ...
            expect(mockWs.send).toHaveBeenCalledWith(expect.stringContaining("Invalid authentication message"));
        });
        it("should handle authentication failure", async () => {
            ...
            const result = await authManager.handleAuthMessage(mockWs, message);
            expect(result).toBe(false);
            expect(mockWs.send).toHaveBeenCalledWith(expect.stringContaining('"error":"Invalid token"'));
        });
    });
    ...
    describe("client management", () => {
        ...
        it("should remove client on cleanup", async () => {
            ...
            authManager.authenticatedClients.set(mockWs.clientId, {
        ...
        it("should get authentication statistics", async () => {
            ...
            clients.forEach((client) => {
                authManager.authenticatedClients.set(client.clientId, {
    ...
    describe("activity tracking", () => {
        it("should update last activity", () => {
            ...
            expect(mockWs.lastActivity.getTime()).toBeGreaterThan(initialActivity.getTime());
        });
    ...
    describe("disconnectUser", () => {
        it("should disconnect all clients for a user", () => {
            ...
            ["c1", "c2", "c3"].forEach((clientId) => {
                authManager.authenticatedClients.set(clientId, {
                    userId,
                    userRole: "user",
                    permissions: ["read"],
                    expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000),
...
//# sourceMappingURL=websocketAuth.test.js.map