...
export class ConversationService {
    ...
    `)
            .all(limit, offset);
        return Promise.all(conversations.map((c) => this.get(c.id).then((conv) => conv)));
    }
    async addMessage(conversationId, message) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        stmt.run(messageId, conversationId, message.role, message.content, timestamp, message.metadata ? JSON.stringify(message.metadata) : null);
    ...
    `)
        ...
        const conversations = await Promise.all(Array.from(conversationIds)
            .slice(0, limit)
            .map((id) => this.get(id)));
        return conversations.filter((c) => c !== null);
    }
    async getRecentConversations(days = 7, limit = 50) {
    ...
    async exportAllConversations(format = "json") {
        ...
        let csv = "Conversation ID,Title,Created At,Updated At,Message ID,Role,Content,Timestamp\n";
