...
export class IEMSDataService {
    ...
    emailAnalysisAgent;
    // IEMS project paths
    IEMS_BASE_PATH = '/home/pricepro2006/iems_project';
    EMAIL_BATCHES_PATH = path.join(this.IEMS_BASE_PATH, 'db_backups/email_batches');
    MAILBOXES_FILE = path.join(this.IEMS_BASE_PATH, 'mailboxes.json');
    DISTRIBUTION_LIST_FILE = path.join(this.IEMS_BASE_PATH, 'distribution_list.json');
    constructor() {
    ...
    async initialize() {
        try {
            ...
            try {
                ...
                logger.warn('IEMS project directory not found, using defaults', 'IEMS_DATA', { path: this.IEMS_BASE_PATH });
                this.mailboxes = [];
            ...
            try {
                ...
                logger.info('Loaded mailboxes', 'IEMS_DATA', { count: this.mailboxes.length });
            }
            catch (error) {
                logger.warn('Could not load mailboxes file', 'IEMS_DATA', { file: this.MAILBOXES_FILE });
                this.mailboxes = [];
            }
            // Load distribution lists
            try {
                const distListData = await fs.readFile(this.DISTRIBUTION_LIST_FILE, 'utf-8');
                this.distributionLists = JSON.parse(distListData);
                logger.info('Loaded distribution lists', 'IEMS_DATA', { count: this.distributionLists.length });
            }
            catch (error) {
                logger.warn('Could not load distribution lists file', 'IEMS_DATA', { file: this.DISTRIBUTION_LIST_FILE });
                this.distributionLists = [];
            }
            ...
            logger.error('Failed to initialize IEMS Data Service', 'IEMS_DATA', error);
     ...
     */
    async loadEmailBatches(batchNumbers) {
        const emails = [];
        try {
            ...
            const emailFiles = files
                ...
                if (!batchNumbers || batchNumbers.length === 0)
                    return true;
                const batchNum = parseInt(f.match(/emails_batch_(\d+)\.json/)?.[1] || '0');
            ...
            for (const file of emailFiles) {
                try {
                    ...
                    const batchEmails = JSON.parse(data);
                    ...
                    logger.error('Failed to load email batch', 'IEMS_DATA', { file, error });
                }
            }
            ...
            logger.error('Failed to load email batches', 'IEMS_DATA', error);
            throw error;
        }
    }
    /**
    ...
    categorizeEmail(email) {
        ...
        const isMailboxEmail = this.mailboxes.some(m => m.email.toLowerCase() === primaryRecipient);
        // Check distribution lists
        const isDistListEmail = this.distributionLists.some(d => d.email.toLowerCase() === primaryRecipient);
        // Categorize based on recipient patterns
        if (primaryRecipient.includes('marketing') || primaryRecipient.includes('splunk')) {
     ...
     */
    determineEmailStatus(email) {
        // Check workflow state
        if (email.workflow_state === 'completed' || email.workflow_state === 'resolved') {
            return 'green';
        }
        // Check if it's been analyzed
        if (email.AnalyzedAt) {
            // Check priority from analysis
            if (email.FullAnalysis?.quick_priority === 'urgent' ||
            ...
            if (email.workflow_state === 'in_progress' ||
        ...
        if (email.IsRead === 0) {
            ...
            const hoursSinceReceived = (Date.now() - receivedTime.getTime()) / (1000 * 60 * 60);
    ...
    async generateSummary(email) {
        ...
        try {
            ...
            logger.error('Failed to generate AI summary', 'IEMS_DATA', error);
    ...
    getEmailAlias(email) {
        ...
        const mailbox = this.mailboxes.find(m => m.email.toLowerCase() === primaryRecipient.toLowerCase());
        if (mailbox) {
            return mailbox.name;
        }
        const distList = this.distributionLists.find(d => d.email.toLowerCase() === primaryRecipient.toLowerCase());
        if (distList) {
     ...
     */
    getActionForEmail(email, category) {
    ...
    getPriority(email) {
        ...
        if (analysisPriority === 'urgent' || analysisPriority === 'high')
            return 'high';
        if (analysisPriority === 'low')
            return 'low';
        // Check mailbox priority
        const recipients = this.parseRecipients(email.Recipients);
        const primaryRecipient = recipients.to[0]?.toLowerCase() || '';
        const mailbox = this.mailboxes.find(m => m.email.toLowerCase() === primaryRecipient);
        if (mailbox) {
     ...
     */
    async getCategorizedEmails(limit) {
...
//# sourceMappingURL=IEMSDataService.js.map