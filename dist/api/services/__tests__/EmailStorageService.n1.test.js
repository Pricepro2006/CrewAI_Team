...
describe("EmailStorageService N+1 Query Tests", () => {
    ...
    afterEach(() => {
        ...
        if (originalPrepare) {
            service.db.prepare = originalPrepare;
        }
        service.close();
    ...
    function createTestData() {
      ...
      `).run(emailId, `graph-${i}`, `Test Email ${i}`, `sender${i}@example.com`, `Sender ${i}`, JSON.stringify([`recipient${i}@example.com`]), new Date().toISOString(), 0, 0, `Preview ${i}`, `Body ${i}`, "normal", JSON.stringify(["test"]), new Date().toISOString(), new Date().toISOString());
            // Insert analysis
            db.prepare(`
      ...
      `).run(uuidv4(), emailId, workflow, "Medium", "request", "normal", 0.85, "pending", workflow, JSON.stringify([]), JSON.stringify([]), 0.9, JSON.stringify([`PO-${i}`]), JSON.stringify([`Q-${i}`]), `Action summary ${i}`, JSON.stringify([{ action: "review", slaStatus: "on-track" }]), "IN_PROGRESS", `Summary ${i}`, new Date().toISOString(), new Date().toISOString());
        }
    }
    describe("getEmailsByWorkflow", () => {
        it("should execute only 1 query regardless of result count", async () => {
            ...
            const emails = await service.getEmailsByWorkflow("Order Processing", 10, 0);
        ...
        `).run(uuidv4(), emailId, "Order Processing", "Order Processing", "Medium", "IN_PROGRESS", new Date().toISOString(), new Date().toISOString());
            }
            // Reset query count
            queryCount = 0;
            // Get 50 emails
            const emails = await service.getEmailsByWorkflow("Order Processing", 50, 0);
    ...
    describe("checkSLAStatus", () => {
        ...
        `).run(emailId, `graph-sla-${i}`, `SLA Test Email ${i}`, `sender@example.com`, `Sender`, JSON.stringify(["recipient@example.com"]), oldDate.toISOString(), 0, 0, `Preview`, `Body`, "normal", JSON.stringify(["test"]), new Date().toISOString(), new Date().toISOString());
                db.prepare(`
          INSERT INTO email_analysis (
        ...
        `).run(uuidv4(), emailId, "Order Processing", priority, "Order Processing", "IN_PROGRESS", new Date().toISOString(), new Date().toISOString());
            }
            // Reset query count
            queryCount = 0;
            // Check SLA status
    ...
    describe("Performance comparison", () => {
        ...
        `).run(emailId, `graph-${i}`, `Performance Test Email ${i}`, `sender${i}@example.com`, `Sender ${i}`, JSON.stringify([`recipient${i}@example.com`]), new Date().toISOString(), 0, 0, `Preview ${i}`, `Body ${i}`, "normal", JSON.stringify(["test"]), new Date().toISOString(), new Date().toISOString());
                db.prepare(`
          INSERT INTO email_analysis (
            id, email_id, quick_workflow, deep_workflow_primary,
            quick_priority, workflow_state, created_at, updated_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run(uuidv4(), emailId, "Performance Test", "Performance Test", "Medium", "IN_PROGRESS", new Date().toISOString(), new Date().toISOString());
            }
            // Measure optimized method
            queryCount = 0;
            const startOptimized = Date.now();
            const optimizedEmails = await service.getEmailsByWorkflow("Performance Test", 100, 0);
            ...
            console.log(`Optimized: ${optimizedQueries} queries in ${optimizedTime}ms`);
            // The optimized version should use significantly fewer queries
            expect(optimizedQueries).toBeLessThan(10);
        });
    });
});
//# sourceMappingURL=EmailStorageService.n1.test.js.map