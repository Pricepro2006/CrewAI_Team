...
export function analyzeQueryComplexity(query) {
    ...
    return {
        ...
        technicalTerms: technicalKeywords.filter((term) => lowerQuery.includes(term)).length,
        multiStepRequired: query.split(/[.!?]/).length > 2 || lowerQuery.includes("then"),
        requiresSearch: searchKeywords.some((term) => lowerQuery.includes(term)),
        requiresAnalysis: analysisKeywords.some((term) => lowerQuery.includes(term)),
 ...
 */
export function selectModel(query, context) {
 ...
 */
export function getAgentModel(agentType, taskType) {
 ...
 */
export function getModelForSystemLoad(preferredModel, systemLoad) {
...
//# sourceMappingURL=model-selection.config.js.map