...
maliciousQueries.forEach((query, index) => {
    console.log(`Test ${index + 1}: "${query.substring(0, 50)}${query.length > 50 ? '...' : ''}"`);
    ...
    if (result.securityFlags.length > 0) {
        ...
        passedCount++;
        console.log("❌ PASSED - No security issues detected (potential vulnerability)");
...
console.log(`Blocked: ${blockedCount} (${((blockedCount / maliciousQueries.length) * 100).toFixed(1)}%)`);
console.log(`Passed: ${passedCount} (${((passedCount / maliciousQueries.length) * 100).toFixed(1)}%)`);
...
console.log(`Passed: ${legitPassed} (${((legitPassed / legitimateQueries.length) * 100).toFixed(1)}%)`);
console.log(`Blocked: ${legitBlocked} (${((legitBlocked / legitimateQueries.length) * 100).toFixed(1)}%)`);
...
//# sourceMappingURL=securityTest.js.map