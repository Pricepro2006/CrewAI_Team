...
export class BusinessQueryOptimizer {
     ...
     */
    static extractServiceType(query) {
     ...
     */
    static extractTimeConstraints(query) {
        const constraints = [];
     ...
     */
    static extractBusinessIndicators(query) {
        const indicators = [];
        ...
        if (!indicators.includes('phone number'))
            indicators.push('phone number');
        if (!indicators.includes('address'))
            indicators.push('address');
        if (!indicators.includes('hours'))
     ...
     */
    static extractSearchOperators(query) {
        const operators = [];
        if (/\bnear me\b/i.test(query)) {
     ...
     */
    static buildOptimizedQuery(components) {
        const parts = [];
        // Service type
     ...
     * Generate search suggestions
     */
    static generateSuggestions(components) {
        const suggestions = [];
        // Base suggestion
     ...
     */
    static getSecurityType(pattern) {
     ...
     */
    static createEmptyComponents(originalQuery) {
        return {
            serviceType: '',
...
//# sourceMappingURL=BusinessQueryOptimizer.js.map