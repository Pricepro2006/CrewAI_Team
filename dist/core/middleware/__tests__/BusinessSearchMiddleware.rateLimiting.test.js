...
describe("BusinessSearchMiddleware - Rate Limiting Integration", () => {
    ...
    describe("Rate Limiting Behavior", () => {
        ...
        it("should track rate limited requests in metrics", async () => {
            ...
            const checkRateLimitSpy = vi
                .spyOn(middleware, "checkRateLimit")
        ...
        it("should emit rate_limited event when rate limit is hit", async () => {
            ...
            // Mock rate limit exceeded
            vi.spyOn(middleware, "checkRateLimit").mockResolvedValue(false);
            // Make request
            await wrappedProvider.generate(businessQuery);
            // Verify event was emitted
            expect(rateLimitedHandler).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it("should continue processing if rate limit check fails", async () => {
            ...
            vi.spyOn(middleware, "checkRateLimit").mockRejectedValue(new Error("Rate limit check failed"));
            // Request should still succeed with enhancement
            const response = await wrappedProvider.generate(businessQuery);
            expect(response).toBe("Mock response");
            // Verify original method was called with enhanced prompt
            expect(mockProvider.generate).toHaveBeenCalledWith(expect.stringContaining("WebSearch"), undefined);
        });
        ...
        it("should respect rate limits in streaming mode", async () => {
            ...
            vi.spyOn(middleware, "checkRateLimit").mockResolvedValue(false);
            // Stream request
            const response = await wrappedProvider.generateStream(businessQuery, undefined, onChunk);
            expect(response).toBe("Mock stream response");
            // Verify no enhancement was applied
            expect(mockProvider.generateStream).toHaveBeenCalledWith(businessQuery, // Original prompt without enhancement
            undefined, onChunk);
        });
    });
    describe("Rate Limiting with Circuit Breaker", () => {
        it("should bypass rate limiting when circuit breaker is open", async () => {
            ...
            middleware.circuitBreakerStatus = "open";
            const checkRateLimitSpy = vi.spyOn(middleware, "checkRateLimit");
            // Make request
    ...
    describe("Performance Impact", () => {
        it("should track latency including rate limit checks", async () => {
            ...
            vi.spyOn(middleware, "checkRateLimit").mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve(true), 50)));
    ...
    describe("Metrics Tracking", () => {
        it("should accurately track rate limited vs total requests", async () => {
            const checkRateLimitMock = vi.spyOn(middleware, "checkRateLimit");
        ...
        it("should reset rate limited count with resetMetrics", () => {
            ...
            middleware.rateLimitedRequests = 10;
            middleware.metrics.rateLimitedRequests = 10;
            middleware.resetMetrics();
            const metrics = middleware.getMetrics();
            expect(metrics.rateLimitedRequests).toBe(0);
            expect(middleware.rateLimitedRequests).toBe(0);
        });
    });
});
//# sourceMappingURL=BusinessSearchMiddleware.rateLimiting.test.js.map