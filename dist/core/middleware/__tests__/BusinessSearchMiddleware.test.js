...
describe('BusinessSearchMiddleware', () => {
    ...
    let originalGenerateWithLogProbs;
    let originalGenerateStream;
    beforeEach(() => {
        // Reset all mocks
        vi.clearAllMocks();
        ...
        originalGenerateStream = vi.fn().mockResolvedValue('Original streamed response');
        mockProvider = {
    ...
    describe('Business Query Detection', () => {
        ...
        it('should not enhance non-business queries', async () => {
            ...
            expect(originalGenerate).toHaveBeenCalledWith(nonBusinessQuery, undefined);
        });
    ...
    describe('Response Validation', () => {
        it('should validate responses with business information', async () => {
            const wrappedProvider = middleware.wrapProvider(mockProvider);
            originalGenerate.mockResolvedValue("Joe's Plumbing - (555) 123-4567, 123 Main St, Open 24/7");
        ...
        it('should track failed validations', async () => {
            const wrappedProvider = middleware.wrapProvider(mockProvider);
            originalGenerate.mockResolvedValue('You should search online for plumbers in your area');
            await wrappedProvider.generate('Find a plumber');
            const metrics = middleware.getMetrics();
            expect(metrics.validatedResponses).toBe(1);
    ...
    describe('Performance Monitoring', () => {
        ...
        it('should emit high latency warning', async () => {
            ...
            // Mock slow response
            originalGenerate.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve('Slow response'), 3000)));
            await wrappedProvider.generate('Find a restaurant');
    ...
    describe('Circuit Breaker', () => {
        ...
        it('should bypass middleware when circuit breaker is open', async () => {
            ...
            expect(originalGenerate).toHaveBeenLastCalledWith('Find a restaurant', undefined);
        });
    });
    ...
    describe('generateStream', () => {
        ...
        it('should validate complete streamed response', async () => {
            const wrappedProvider = middleware.wrapProvider(mockProvider);
            originalGenerateStream.mockResolvedValue("City Auto Repair - 456 Oak Ave, Hours: Mon-Fri 8AM-6PM");
            const chunks = [];
            await wrappedProvider.generateStream('Find auto repair', undefined, (chunk) => chunks.push(chunk));
            const metrics = middleware.getMetrics();
            expect(metrics.validatedResponses).toBe(1);
            expect(metrics.failedValidations).toBe(0);
        });
    ...
    describe('Configuration', () => {
        ...
        it('should respect bypass patterns', async () => {
            ...
            // Should not enhance
            expect(originalGenerate).toHaveBeenCalledWith('internal system query', undefined);
        });
    ...
    describe('Error Handling', () => {
        it('should handle provider errors gracefully', async () => {
            ...
            await expect(wrappedProvider.generate('Find a store')).rejects.toThrow('Provider error');
            const metrics = middleware.getMetrics();
...
//# sourceMappingURL=BusinessSearchMiddleware.test.js.map