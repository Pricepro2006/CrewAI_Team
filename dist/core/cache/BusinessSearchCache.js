...
export class BusinessSearchCache {
        ...
        try {
            ...
            if (entry) {
                ...
                if (this.config.useRedis && this.redisClient) {
                    const redisKey = `${this.config.redisPrefix}${key}`;
                    await this.redisClient.set(redisKey, JSON.stringify(entry), 'PX', this.config.maxAge);
                }
                this.memoryCache.set(key, entry);
                this.trackResponseTime(Date.now() - startTime);
     ...
     */
    async set(query, location, response, validation, metadata) {
        const key = this.generateKey(query, location);
        const entry = {
            response,
            validation,
    ...
    analyzePerformance() {
        ...
        // Count stale entries
        const staleEntries = entries.filter(e => now - e.timestamp > this.config.maxAge).length;
     ...
     */
    async importCache(entries) {
        for (const { key, ...entry } of entries) {
            this.memoryCache.set(key, entry);
            if (this.config.useRedis && this.redisClient) {
                const redisKey = `${this.config.redisPrefix}${key}`;
                await this.redisClient.set(redisKey, JSON.stringify(entry), 'PX', this.config.maxAge - (Date.now() - entry.timestamp));
            }
        }
    }
}
//# sourceMappingURL=BusinessSearchCache.js.map