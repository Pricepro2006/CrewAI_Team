...
import { BusinessSearchCache } from "../BusinessSearchCache";
// Mock Redis
vi.mock("ioredis", () => {
    ...
    return vi.fn().mockImplementation(() => ({
        ...
        del: vi.fn().mockImplementation((...keys) => {
            keys.forEach((key) => mockRedisData.delete(key));
            return Promise.resolve(keys.length);
        }),
        keys: vi.fn().mockImplementation((pattern) => {
...
describe("BusinessSearchCache", () => {
    ...
    const testResponse = "Here are plumbers in your area: ABC Plumbing (555-1234)";
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    describe("Search Functionality", () => {
        it("should search cache entries by pattern", async () => {
            ...
            await cache.set("Find plumbers in Chicago", "Chicago", "Chicago plumbers");
            const plumberResults = await cache.search(/plumbers/i);
            expect(plumberResults).toHaveLength(2);
            expect(plumberResults.every((r) => r.entry.metadata.query.includes("plumbers"))).toBe(true);
...
//# sourceMappingURL=BusinessSearchCache.test.js.map