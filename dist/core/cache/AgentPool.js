...
export class AgentPool {
    ...
    async getAgent(type) {
        ...
        try {
            ...
            logger.error("Failed to get agent from pool", "AGENT_POOL", undefined, error);
    ...
    async preWarmPool() {
        ...
        try {
            ...
            logger.error("Agent pool pre-warming failed", "AGENT_POOL", undefined, error);
        }
    }
    /**
     * Get pool statistics
    ...
    async clearPools() {
        ...
        try {
            ...
            logger.error("Failed to clear agent pools", "AGENT_POOL", undefined, error);
        }
    }
    /**
    ...
    initializePools() {
        ...
        if (this.config.initializeOnStartup) {
            ...
            this.preWarmPool().catch((error) => {
                logger.error("Background pre-warming failed", "AGENT_POOL", undefined, error);
            });
        }
    }
    /**
    ...
    async createAgent(type) {
        ...
        try {
            ...
            logger.error("Failed to create agent", "AGENT_POOL", undefined, error);
            throw error;
        }
    }
    /**
    ...
    async cleanupAgent(agent) {
        try {
            ...
            const agentWithCleanup = agent;
            if (agentWithCleanup.cleanup &&
                typeof agentWithCleanup.cleanup === "function") {
                await agentWithCleanup.cleanup();
            }
        }
        catch (error) {
            logger.warn("Agent cleanup failed", "AGENT_POOL", {
                error: error.message,
            });
        }
    }
    ...
    updateStats() {
        ...
        // Reset type counts
        for (const type of Object.keys(this.stats.agentsByType)) {
