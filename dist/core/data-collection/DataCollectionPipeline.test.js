...
describe("DataCollectionPipeline", () => {
    ...
    describe("data source management", () => {
        ...
        it("should throw error when updating non-existent source", async () => {
            await expect(pipeline.updateDataSource("non-existent", { name: "Updated" })).rejects.toThrow("Data source not found");
        });
        it("should throw error when removing non-existent source", async () => {
            await expect(pipeline.removeDataSource("non-existent")).rejects.toThrow("Data source not found");
        });
    });
    describe("data collection", () => {
        it("should collect data from a search engine source", async () => {
            ...
            const collectSearchResultsSpy = vi.spyOn(pipeline.brightDataService, "collectSearchResults");
        ...
        it("should handle collection errors gracefully", async () => {
            ...
            const collectWebScrapingDataSpy = vi.spyOn(pipeline.brightDataService, "collectWebScrapingData");
            collectWebScrapingDataSpy.mockRejectedValue(new Error("Collection failed"));
        ...
        it("should not collect from inactive sources", async () => {
            ...
            const sourceId = await pipeline.addDataSource(source);
            await expect(pipeline.collectFromSource(sourceId)).rejects.toThrow("Data source is not active");
        });
    });
    describe("statistics", () => {
    ...
    describe("event emission", () => {
        it("should emit events for source operations", async () => {
            const sourceAddedEvents = [];
            const sourceUpdatedEvents = [];
            const sourceRemovedEvents = [];
            pipeline.on("source:added", (source) => sourceAddedEvents.push(source));
            pipeline.on("source:updated", (source) => sourceUpdatedEvents.push(source));
            pipeline.on("source:removed", (source) => sourceRemovedEvents.push(source));
            const source = {
        ...
        it("should emit events for job operations", async () => {
            const jobStartedEvents = [];
            const jobCompletedEvents = [];
...
//# sourceMappingURL=DataCollectionPipeline.test.js.map