...
import { QueryComplexityAnalyzer, ConfidenceRAGRetriever, ConfidenceContextBuilder, ConfidenceResponseGenerator, MultiModalEvaluator, ConfidenceCalibrator, AdaptiveDeliveryManager, ActionType, ResponseEvaluationResult, DeliveredResponse, } from "../rag/confidence";
...
import { selectModel, getModelForSystemLoad, MODEL_CONFIGS, } from "../../config/model-selection.config";
export class ConfidenceMasterOrchestrator extends EventEmitter {
    ...
    enhancedParser;
    agentRouter;
    perfMonitor = createPerformanceMonitor("ConfidenceMasterOrchestrator");
    ...
    constructor(config) {
        super();
        logger.info("Initializing ConfidenceMasterOrchestrator", "CONFIDENCE_ORCHESTRATOR", { config });
        ...
        logger.info("ConfidenceMasterOrchestrator initialized successfully", "CONFIDENCE_ORCHESTRATOR");
    }
    async initialize() {
        ...
        try {
            ...
            logger.warn("RAG initialization failed, continuing with limited functionality", "CONFIDENCE_ORCHESTRATOR", { error: error.message });
        }
        // Initialize agents
        await this.agentRegistry.initialize();
        // Load calibration parameters if available
        await this.loadCalibrationParameters();
        logger.info("ConfidenceMasterOrchestrator fully initialized", "CONFIDENCE_ORCHESTRATOR");
    }
    async processQuery(query) {
        const perf = this.perfMonitor.start("processQuery");
        const startTime = Date.now();
        logger.info("Processing query with confidence scoring", "CONFIDENCE_ORCHESTRATOR", {
            query: query.text.substring(0, 100),
            conversationId: query.conversationId,
        });
        try {
            // Step 1: Analyze query complexity (with caching)
            const complexityCacheKey = this.performanceOptimizer.generateQueryKey(`complexity:${query.text}`);
            const complexity = await this.performanceOptimizer.withCache(complexityCacheKey, async () => this.confidenceRAG.analyzer.assessComplexity(query.text), 60000);
            ...
            const adjustedModelConfig = getModelForSystemLoad(modelConfig, systemLoad);
            // Switch model if different from current
            if (adjustedModelConfig.model !== this.llm.config.model) {
                logger.info("Switching model based on complexity and system load", "CONFIDENCE_ORCHESTRATOR", {
            ...
            let result;
            if (complexity.score <= this.confidenceConfig.complexityThresholds.simple) {
                result = await this.handleSimpleQuery(query, complexity);
            }
            else if (complexity.score <= this.confidenceConfig.complexityThresholds.medium) {
            ...
            logger.error("Query processing failed", "CONFIDENCE_ORCHESTRATOR", { query: query.text }, error);
            perf.end({ success: false });
            // Return fallback response
            return this.createFallbackResponse(query, error);
        }
    }
    /**
     * Handle simple queries with direct response
     */
    async handleSimpleQuery(query, _complexity) {
        ...
        const response = await simpleModel.generate(`Answer this simple question concisely: ${query.text}`, {
            temperature: MODEL_CONFIGS.SIMPLE.temperature,
            maxTokens: MODEL_CONFIGS.SIMPLE.maxTokens,
        });
        // Quick evaluation
        const _evaluation = this.confidenceRAG.evaluator.quickEvaluate(query.text, response, 0.85);
        // Deliver response
     ...
     */
    async handleConfidenceRAG(query, _complexity) {
        logger.info("Handling with confidence RAG", "CONFIDENCE_ORCHESTRATOR");
        // Step 1: Retrieve relevant documents (with caching)
        const retrievalCacheKey = this.performanceOptimizer.generateQueryKey(`retrieval:${query.text}`);
        const retrieval = await this.performanceOptimizer.withCache(retrievalCacheKey, async () => this.confidenceRAG.retriever.retrieve(query.text, {
            topK: 5,
            minConfidence: 0.6,
        }), 180000);
        this.emit("confidence:update", {
        ...
        const optimizedDocs = await this.performanceOptimizer.optimizeRetrieval(retrieval.documents, 5);
        // Step 2: Build context
        const context = this.confidenceRAG.contextBuilder.buildContext(optimizedDocs, query.text, { mode: "unified", includeConfidence: true });
        ...
        const evaluation = await this.confidenceRAG.evaluator.evaluate(query.text, generation.response, retrieval.documents, generation.tokenConfidence);
        ...
        const calibrated = this.confidenceRAG.calibrator.calibrate(evaluation.overallConfidence, { method: "temperature_scaling" });
        ...
        return this.createOrchestratorResult(generation.response, delivered, "confidence-rag");
    }
    /**
     * Handle complex queries with full agent orchestration
     */
    async handleComplexAgentTask(query, _complexity) {
        logger.info("Handling complex query with agents", "CONFIDENCE_ORCHESTRATOR");
        ...
        const plan = await this.createEnhancedPlan(query, queryAnalysis, routingPlan);
        ...
        const evaluation = await this.confidenceRAG.evaluator.evaluate(query.text, consolidatedResponse, [], // No direct sources for agent responses
        []);
        ...
        return this.createOrchestratorResult(consolidatedResponse, delivered, "agent-orchestration");
    }
    /**
     * Create enhanced plan for complex queries
     */
    async createEnhancedPlan(query, analysis, routingPlan) {
    ...
    parsePlan(response, query) {
        try {
            ...
            logger.error("Failed to parse plan", "CONFIDENCE_ORCHESTRATOR", {}, error);
     ...
     */
    consolidateAgentResults(executionResult) {
     ...
     */
    calculateAgentConfidence(executionResult) {
        const results = executionResult.results;
        if (results.length === 0)
            return 0.5;
        const successRate = results.filter((r) => r.success).length / results.length;
        const avgStepConfidence = results
     ...
     */
    createFallbackResponse(query, error) {
        const evaluation = {
            overallConfidence: 0,
     ...
     */
    createOrchestratorResult(response, delivered, processingPath) {
        return {
            results: [
                {
                    success: true,
     ...
     */
    async cleanup() {
        logger.info("Cleaning up ConfidenceMasterOrchestrator", "CONFIDENCE_ORCHESTRATOR");
...
//# sourceMappingURL=ConfidenceMasterOrchestrator.js.map