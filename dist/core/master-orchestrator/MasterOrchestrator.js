...
export class MasterOrchestrator {
    ...
    async initialize() {
        ...
        if (this.ragSystem) {
            try {
                ...
                logger.warn("RAG system initialization failed, continuing without RAG", "ORCHESTRATOR", {
                    error: error.message,
                });
    ...
    async processQuery(query) {
        ...
        try {
            ...
            const queryAnalysis = await withTimeout(this.enhancedParser.parseQuery(query), DEFAULT_TIMEOUTS.QUERY_PROCESSING, "Query analysis timed out");
            ...
            const routingPlan = await withTimeout(this.agentRouter.createRoutingPlan(queryAnalysis), DEFAULT_TIMEOUTS.PLAN_CREATION, "Agent routing plan creation timed out");
            ...
            do {
                ...
                const review = await withTimeout(this.planReviewer.review(query, plan, executionResult), DEFAULT_TIMEOUTS.PLAN_CREATION, "Plan review timed out");
    ...
    `;
        const response = await withTimeout(this.llm.generate(prompt, {
            format: "json",
            temperature: 0.3,
            maxTokens: 2000,
