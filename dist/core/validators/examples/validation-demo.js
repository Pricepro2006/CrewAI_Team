import { IntegratedValidationService, BusinessResponseValidator, ContactPatterns } from '../index.js';
...
async function basicValidationExample() {
  ...
  `;
    ...
    console.log('- Business Names:', result.contactInfo.businessNames.map(b => b.value));
    console.log('- Phones:', result.contactInfo.phones.map(p => p.value));
    console.log('- Address:', result.contactInfo.addresses[0]?.value);
    console.log('- Hours:', result.contactInfo.hours.map(h => h.value));
...
export { basicValidationExample, privacyModeExample, integratedValidationExample, patternMatchingExample, edgeCaseExample, performanceTest, runAllExamples };
