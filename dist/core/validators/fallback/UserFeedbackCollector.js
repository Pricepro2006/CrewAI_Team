export class UserFeedbackCollector {
    ...
    getStats(timeRange) {
        ...
        if (timeRange) {
            relevantFeedback = allFeedback.filter(f => f.timestamp >= timeRange.start && f.timestamp <= timeRange.end);
        }
     ...
     */
    generateFeedbackForm(query, validationResult) {
        return {
            fields: [
                {
                    name: 'userRating',
     ...
     */
    processSuggestedCorrections(query, corrections) {
        if (!corrections)
            return;
        const suggestions = [];
...
//# sourceMappingURL=UserFeedbackCollector.js.map