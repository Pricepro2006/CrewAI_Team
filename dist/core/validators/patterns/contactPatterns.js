...
export const ContactPatterns = {
    ...
    phone: {
        ...
        getConfidence: (match) => {
            if (match.match(/^\+?1?[-.\s]?\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}$/))
                return 0.95;
            if (match.match(/^\+[0-9]{1,3}[-.\s]?/))
                return 0.85;
...
export const PatternHelpers = {
     ...
     */
    extractMatches: (text, pattern, confidenceFunc) => {
        const matches = [];
        let match;
    ...
    scoreBusinessName: (name) => {
        ...
        if (name.match(ContactPatterns.businessName.entityTypes))
            score += 0.3;
...
//# sourceMappingURL=contactPatterns.js.map