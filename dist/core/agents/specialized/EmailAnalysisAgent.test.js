...
describe('EmailAnalysisAgent', () => {
    ...
    beforeEach(() => {
        ...
        mockOllamaProvider = OllamaProvider.mock.results[0].value;
    });
    describe('Entity Extraction', () => {
        it('should extract PO numbers correctly', async () => {
            ...
            mockOllamaProvider.generate.mockResolvedValueOnce('Order confirmation for two purchase orders');
        ...
        it('should extract quote numbers correctly', async () => {
            ...
            mockOllamaProvider.generate.mockResolvedValueOnce('Multiple quotes ready for review');
            const analysis = await agent.analyzeEmail(email);
        ...
        it('should extract tracking numbers correctly', async () => {
            ...
            mockOllamaProvider.generate.mockResolvedValueOnce('Shipment notification with tracking numbers');
            const analysis = await agent.analyzeEmail(email);
        ...
        it('should extract amounts correctly', async () => {
            ...
            mockOllamaProvider.generate.mockResolvedValueOnce('Invoice with payment due');
            const analysis = await agent.analyzeEmail(email);
            expect(analysis.entities.amounts).toHaveLength(2);
            expect(analysis.entities.amounts[0]).toEqual({ value: 1234.56, currency: 'USD' });
            expect(analysis.entities.amounts[1]).toEqual({ value: 500, currency: 'EUR' });
        });
    });
    describe('Workflow State Determination', () => {
        it('should set state to New for unread emails', async () => {
            ...
            mockOllamaProvider.generate.mockResolvedValueOnce('New order request from customer');
            const analysis = await agent.analyzeEmail(email);
        ...
        it('should set state to Pending External for emails with tracking numbers', async () => {
            ...
            mockOllamaProvider.generate.mockResolvedValueOnce('Shipment tracking information');
            const analysis = await agent.analyzeEmail(email);
    ...
    describe('Categorization', () => {
        it('should categorize order-related emails correctly', async () => {
            ...
            mockOllamaProvider.generate.mockResolvedValueOnce('Order confirmation notification');
            const analysis = await agent.analyzeEmail(email);
        ...
        it('should handle urgent emails with high priority', async () => {
            ...
            mockOllamaProvider.generate.mockResolvedValueOnce('Critical system failure requiring immediate action');
            const analysis = await agent.analyzeEmail(email);
    ...
    describe('Fallback Handling', () => {
        it('should use fallback categorization when LLM fails', async () => {
            ...
            // Mock LLM failure
            mockOllamaProvider.generate.mockRejectedValueOnce(new Error('LLM unavailable'));
            mockOllamaProvider.generate.mockResolvedValueOnce('Order processing update');
            const analysis = await agent.analyzeEmail(email);
...
//# sourceMappingURL=EmailAnalysisAgent.test.js.map