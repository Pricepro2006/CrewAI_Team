...
export class EmailAnalysisAgentEnhanced extends BaseAgent {
    ...
    constructor() {
        super('EmailAnalysisAgentEnhanced', 'Advanced email analysis with TD SYNNEX workflow optimization', 'qwen3:0.6b');
...
Response format:
{
  ...
  "confidence": 0.0-1.0
        ...
        try {
            ...
            return {
                workflow: parsed.workflow || { primary: 'Customer Support', confidence: 0.5 },
            ...
            logger.error('Quick categorization failed, using fallback', 'EMAIL_AGENT_ENHANCED', { error });
            return this.fallbackQuickCategorization(email);
        }
    }
    async deepWorkflowAnalysis(email, quickAnalysis) {
...
Focus on TD SYNNEX operations: orders, quotes, shipping, support, deals, approvals, renewals, vendors.`;
        try {
            ...
            const deepAnalysis = await this.parseDeepAnalysis(response, quickAnalysis, entities);
            // Calculate SLA status for action items
            deepAnalysis.actionItems = await this.calculateSLAStatus(deepAnalysis.actionItems, quickAnalysis.priority, email.receivedDateTime);
            return deepAnalysis;
        }
        catch (error) {
            logger.error('Deep analysis failed, using enhanced quick analysis', 'EMAIL_AGENT_ENHANCED', { error });
            return this.enhanceQuickAnalysis(quickAnalysis, entities);
        }
    }
    async extractEnhancedEntities(email) {
        ...
        entities.orderReferences = this.extractMatches(text, this.patterns.orderReference);
        // Extract amounts with context
        const amountMatches = this.extractMatchesWithContext(text, this.patterns.amount);
        entities.amounts = amountMatches.map(match => {
            const value = parseFloat(match.value.replace(/[$,]/g, '').replace(/\s*[A-Z]{3}$/, ''));
            const currency = match.value.match(/[A-Z]{3}$/)?.[0] || 'USD';
            return { value, currency, context: match.context };
        });
        // Extract dates with type detection
        const dateMatches = this.extractMatchesWithContext(text, this.patterns.date);
    ...
    extractMatchesWithContext(text, pattern) {
        const matches = [];
        let match;
    ...
    extractNameNearEmail(email, text) {
    ...
    async calculateSLAStatus(actionItems, priority, receivedDate) {
    ...
    async parseDeepAnalysis(response, quickAnalysis, entities) {
        ...
        if (summaryMatch?.[1]) {
            ...
            analysis.contextualSummary = this.generateContextualSummary(entities, quickAnalysis);
        }
    ...
    generateContextualSummary(entities, analysis) {
        const parts = [];
        ...
        const entityString = parts.length > 0 ? `regarding ${parts.join(', ')}` : '';
    ...
    async extractActionSummary(email, analysis) {
...
Summary:`;
    ...
    fallbackQuickCategorization(email) {
        ...
        let confidence = 0.6;
        if (content.includes('order') || content.includes('po ') || content.includes('purchase')) {
            workflow = 'Order Management';
            confidence = 0.8;
        }
        else if (content.includes('ship') || content.includes('tracking') || content.includes('delivery')) {
            workflow = 'Shipping/Logistics';
            confidence = 0.8;
        }
        else if (content.includes('quote') || content.includes('pricing') || content.includes('proposal')) {
    ...
    enhanceQuickAnalysis(quickAnalysis, entities) {
        // Create a basic deep analysis from quick analysis
        return {
            ...quickAnalysis,
            detailedWorkflow: {
                primary: quickAnalysis.workflow.primary,
                secondary: this.inferSecondaryWorkflows(quickAnalysis.workflow.primary, entities),
                relatedCategories: [],
            ...
            businessImpact: {
                customerSatisfaction: 'neutral',
                urgencyReason: quickAnalysis.urgency === 'Immediate' ? 'High priority request' : ''
            },
            contextualSummary: this.generateContextualSummary(entities, quickAnalysis),
            relatedEmails: []
        };
    }
    inferSecondaryWorkflows(primary, entities) {
        const secondary = [];
    ...
    inferActionItems(analysis, entities) {
        const actions = [];
    ...
    async trackWorkflowPattern(email, analysis) {
        ...
        logger.debug('Tracking workflow pattern', 'EMAIL_AGENT_ENHANCED', {
            ...
            entities: Object.keys(analysis.deep.entities).reduce((acc, key) => {
                const entityValue = analysis.deep.entities[key];
...
ACTION ITEMS:
${deep.actionItems.map((item, i) => `${i + 1}. ${item.action}
   - Type: ${item.type}
   - Priority: ${item.priority}
   - SLA Status: ${item.slaStatus || 'N/A'}
   ${item.deadline ? `- Deadline: ${new Date(item.deadline).toLocaleString()}` : ''}`).join('\n')}
...
//# sourceMappingURL=EmailAnalysisAgentEnhanced.js.map