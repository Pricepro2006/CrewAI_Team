...
export class CoherenceAnalyzer {
     ...
     */
    analyzeCoherence(response, tokenConfidence) {
     ...
     */
    analyzeStructuralCoherence(response) {
        ...
        const transitionCount = transitionWords.filter(word => response.toLowerCase().includes(word)).length;
    ...
    analyzeLogicalFlow(response) {
        ...
        const hasProgression = progressionMarkers.some(marker => response.toLowerCase().includes(marker));
        if (hasProgression) {
            score += 0.15;
        }
        ...
        const hasCausal = causalMarkers.some(marker => response.toLowerCase().includes(marker));
        if (hasCausal) {
            score += 0.15;
        }
        ...
        const hasContradictions = contradictionMarkers.some(marker => response.toLowerCase().includes(marker));
     ...
     */
    analyzeLinguisticCoherence(response, tokenConfidence) {
     ...
     */
    analyzeTopicConsistency(response) {
    ...
    calculateOverallCoherence(metrics) {
        ...
        return (metrics.structuralCoherence * weights.structuralCoherence +
     ...
     */
    identifyCoherenceIssues(response, metrics) {
        const issues = [];
...
//# sourceMappingURL=CoherenceAnalyzer.js.map