...
export class ConfidenceContextBuilder {
     ...
     */
    buildContext(documents, query, options) {
        ...
        switch (options.mode) {
            case 'unified':
                ({ content, usedDocs, estimatedTokens } = this.buildUnifiedContext(prioritizedDocs, query, maxTokens, options));
                break;
            case 'sectioned':
                ({ content, usedDocs, estimatedTokens } = this.buildSectionedContext(prioritizedDocs, query, maxTokens, options));
                break;
            case 'hierarchical':
                ({ content, usedDocs, estimatedTokens } = this.buildHierarchicalContext(prioritizedDocs, query, maxTokens, options));
                break;
            default:
                ({ content, usedDocs, estimatedTokens } = this.buildUnifiedContext(prioritizedDocs, query, maxTokens, options));
        }
     ...
     */
    prioritizeDocuments(documents, query, options) {
     ...
     */
    applyTemporalPrioritization(documents) {
        const now = Date.now();
     ...
     */
    buildUnifiedContext(documents, query, maxTokens, options) {
        const usedDocs = [];
        const sections = [];
     ...
     */
    buildSectionedContext(documents, query, maxTokens, options) {
        const usedDocs = [];
        const sections = [];
        let estimatedTokens = 0;
        // Add query context
     ...
     */
    buildHierarchicalContext(documents, query, maxTokens, options) {
        const usedDocs = [];
        const sections = [];
        let estimatedTokens = 0;
        // Add query context
        ...
        if (highConfidence.length > 0) {
            const highSection = this.buildConfidenceSection('High Confidence Information', highConfidence, maxTokens - estimatedTokens, options);
        ...
        if (mediumConfidence.length > 0 && estimatedTokens < maxTokens * 0.7) {
            const mediumSection = this.buildConfidenceSection('Medium Confidence Information', mediumConfidence, maxTokens - estimatedTokens, options);
        ...
        if (lowConfidence.length > 0 && estimatedTokens < maxTokens * 0.8) {
            const lowSection = this.buildConfidenceSection('Additional Information (Lower Confidence)', lowConfidence, maxTokens - estimatedTokens, options);
     ...
     */
    buildConfidenceSection(title, documents, maxTokens, options) {
        const usedDocs = [];
        const sections = [];
        let estimatedTokens = 0;
        // Add section header
     ...
     */
    formatDocumentForUnified(doc, options) {
     ...
     */
    formatDocumentForSectioned(doc, index, options) {
     ...
     */
    formatDocumentForHierarchical(doc, options) {
        let formatted = doc.content;
        if (options.includeConfidence) {
     ...
     */
    calculateContextConfidence(documents, query) {
...
//# sourceMappingURL=ConfidenceContextBuilder.js.map