...
export class MultiModalEvaluator {
     ...
     */
    async evaluate(query, response, sources, tokenConfidence) {
        ...
        try {
            ...
            return this.createFallbackEvaluation(evaluationId, query, response, error);
        }
    }
    /**
     * Quick evaluation for simple cases
     */
    quickEvaluate(query, response, baseConfidence = 0.7) {
     ...
     * Evaluate relevance
     */
    async evaluateRelevance(query, response, sources) {
     ...
     * Evaluate factuality
     */
    async evaluateFactuality(query, response, sources) {
     ...
     * Evaluate coherence
     */
    async evaluateCoherence(response, tokenConfidence) {
     ...
     * Calculate completeness score
     */
    calculateCompleteness(query, response, sources) {
        // Extract query intentions
        const queryWords = query.toLowerCase().split(/\s+/);
        const questionWords = queryWords.filter(word => ['what', 'how', 'why', 'when', 'where', 'which', 'who'].includes(word));
     ...
     * Calculate consistency score
     */
    calculateConsistency(response, sources) {
        if (sources.length === 0)
            return 0.5;
     ...
     * Calculate overall confidence
     */
    calculateOverallConfidence(metrics, tokenConfidence) {
        ...
        const metricsScore = metrics.factuality * weights.factuality +
     ...
     * Determine recommended action
     */
    determineAction(confidence, metrics) {
     ...
     * Check if human review is needed
     */
    requiresHumanReview(confidence, metrics) {
        if (confidence < 0.6)
            return true;
     ...
     * Identify uncertainty areas
     */
    identifyUncertaintyAreas(metrics, tokenConfidence) {
        const areas = [];
        if (metrics.factuality < 0.6) {
     ...
     */
    extractSupportingEvidence(response, sources) {
        const evidence = [];
     ...
     */
    extractContradictoryEvidence(response, sources) {
        const evidence = [];
     ...
     */
    createFallbackEvaluation(id, query, response, error) {
        const qualityMetrics = {
            factuality: 0.1,
            relevance: 0.1,
            coherence: 0.1,
...
//# sourceMappingURL=MultiModalEvaluator.js.map