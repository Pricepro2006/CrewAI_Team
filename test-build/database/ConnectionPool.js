...
export class DatabaseConnection {
    ...
    configureSQLite() {
        try {
            ...
            logger.error(`Failed to configure SQLite for connection ${this.metrics.id}:`, this.metrics.id, undefined, error instanceof Error ? error : new Error(String(error)));
            throw error;
        }
    }
    /**
    ...
    dispose() {
        ...
        try {
            ...
            logger.debug(`Disposed connection ${this.metrics.id} on thread ${threadId}`);
        }
        catch (error) {
            logger.error(`Error disposing connection ${this.metrics.id}:`, this.metrics.id, undefined, error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
...
export class DatabaseConnectionPool {
    ...
    constructor(config) {
        ...
        logger.info(`Connection pool initialized for thread ${threadId}`, "ConnectionPool", {
            config: this.config,
            isMainThread,
        });
    }
    ...
    getConnection() {
        ...
        if (!connection || connection.isDisposed()) {
            ...
            logger.debug(`Created new connection for thread ${currentThreadId}`, "ConnectionPool", {
    ...
    async executeQuery(queryFn) {
        ...
        try {
            ...
            logger.error(`Query execution failed on thread ${threadId}:`, "ConnectionPool", undefined, error instanceof Error ? error : new Error(String(error)));
            throw error;
        }
    }
    /**
     ...
     */
    getStats() {
        const activeConnections = Array.from(this.connections.values()).filter((conn) => !conn.isDisposed()).length;
        const idleConnections = Array.from(this.connections.values()).filter((conn) => !conn.isDisposed() && conn.isIdle()).length;
        ...
        const avgQueryTime = this.queryTimes.length > 0
            ...
            : 0;
        const totalMemory = Array.from(this.connectionMetrics.values()).reduce((total, metrics) => total + (metrics.memoryUsage || 0), 0);
        return {
            totalConnections: this.connections.size,
            activeConnections,
            idleConnections,
    ...
    async shutdown() {
        ...
        for (const [threadId, connection] of this.connections) {
            try {
                ...
                logger.error(`Error closing connection for thread ${threadId}:`, "ConnectionPool", undefined, error instanceof Error ? error : new Error(String(error)));
            }
        }
 ...
 */
export async function executeQuery(queryFn, config) {
    const pool = DatabaseConnectionPool.getInstance(config);
 ...
 */
export async function executeTransaction(transactionFn, config) {
    const pool = DatabaseConnectionPool.getInstance(config);
...
if (isMainThread) {
    const gracefulShutdown = async (signal) => {
        ...
        try {
            ...
            logger.error("Error during graceful shutdown:", "ConnectionPool", undefined, error instanceof Error ? error : new Error(String(error)));
            process.exit(1);
        }
    };
