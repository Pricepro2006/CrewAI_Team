...
export class GroceryVectorCollections {
    ...
    async initializeCollections() {
        ...
        for (const [key, config] of Object.entries(this.COLLECTIONS)) {
            try {
                ...
                logger.error(`Failed to create collection ${key}: ${error}`, "GROCERY_VECTORS");
                throw error;
            }
        }
        logger.info("All grocery vector collections initialized", "GROCERY_VECTORS");
    }
    /**
     * Add product to vector database
     */
    async addProduct(product) {
        const collection = await this?.chromaManager?.getCollection("walmart_products");
        const document = {
     ...
     */
    async searchSimilarProducts(query, filters, limit = 10) {
        const whereClause = {};
        if (filters?.category) {
        ...
        return results?.map((result) => ({
            ...
            search_text: "",
            price_range: result?.metadata?.price_range,
            dietary_tags: result?.metadata?.dietary_tags,
        }));
    }
    /**
     ...
     */
    async findRecipesByIngredients(ingredients, dietaryRestrictions, limit = 5) {
        ...
        return results?.map((result) => ({
            recipe_id: result?.metadata?.recipe_id,
            name: result?.metadata?.name,
            ingredients: result?.metadata?.ingredients,
            cuisine_type: result?.metadata?.cuisine_type,
            meal_type: result?.metadata?.meal_type,
            prep_time: result?.metadata?.prep_time,
            difficulty: result?.metadata?.difficulty,
            dietary_info: result?.metadata?.dietary_info,
        }));
    }
    /**
     * Store user shopping pattern
     */
    async storeShoppingPattern(pattern) {
        const document = {
     ...
     */
    async getUserRecommendations(userId, currentList) {
        ...
        for (const pattern of patterns) {
            const commonItems = pattern?.metadata?.common_items;
     ...
     */
    async findSubstitutions(productId, pricePreference) {
        const whereClause = { original_product_id: productId };
        ...
        return results
            .map((result) => ({
            substitute_id: result?.metadata?.substitute_product_id,
            similarity: result?.metadata?.similarity_score,
            price_difference: result?.metadata?.price_difference,
            reason: result?.metadata?.substitution_reason,
        }))
            .filter((sub) => {
            if (pricePreference === "cheaper")
                return sub.price_difference < 0;
     ...
     */
    async cleanupOldPatterns(daysOld = 180) {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - daysOld);
        // This would need to be implemented based on ChromaDB's capabilities
        // for now returning 0
        logger.info(`Cleanup of patterns older than ${daysOld} days requested`, "GROCERY_VECTORS");
        return 0;
    }
}
