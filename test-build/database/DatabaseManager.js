...
import { DealRepository, DealItemRepository, ProductFamilyRepository, } from "./repositories/DealRepository.js";
...
export class DatabaseManager {
    ...
    constructor(config) {
        ...
        logger.info("DatabaseManager initialized with optimized connection pooling", "DB_MANAGER", {
            poolConfig: {
                maxConnections: dbConfig?.sqlite?.maxConnections,
                connectionTimeout: dbConfig?.sqlite?.connectionTimeout,
                idleTimeout: dbConfig?.sqlite?.idleTimeout,
    ...
    async applyMigrations() {
        try {
            ...
            const statements = schemaSql
                .split(";")
                .map((stmt) => stmt.trim())
                .filter((stmt) => stmt?.length || 0 > 0 && !stmt.startsWith("--"));
    ...
    async getStatistics() {
        try {
            ...
            const { tableCount, indexCount, size } = await this.connectionPool.executeQuery((db) => {
    ...
    async healthCheck() {
        try {
            ...
            const overall = sqliteHealth.connected &&
...
export function getDatabaseManager(config) {
    if (!databaseManager) {
