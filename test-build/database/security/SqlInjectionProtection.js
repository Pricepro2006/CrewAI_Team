...
export class SqlInjectionError extends Error {
    suspiciousInput;
    constructor(message, suspiciousInput) {
        super(message);
...
export class SqlInjectionProtection {
    ...
    static ALLOWED_COLUMN_CHARS = /^[a-zA-Z0-9_]+$/;
    static ALLOWED_TABLE_CHARS = /^[a-zA-Z0-9_]+$/;
    config;
    constructor(config = {}) {
        this.config = {
    ...
    validateQueryParameters(params) {
        if (params.length > this.config.maxParameterCount) {
            throw new SqlInjectionError(`Too many parameters: ${params.length} (max: ${this.config.maxParameterCount})`);
        }
        return params.map((param, index) => {
            try {
                ...
                throw new SqlInjectionError(`Invalid parameter at index ${index}: ${error instanceof Error ? error.message : String(error)}`, typeof param === "string" ? param : undefined);
            }
        });
    }
    /**
    ...
    containsSqlInjection(input) {
        ...
        return SqlInjectionProtection.SQL_INJECTION_PATTERNS.some((pattern) => pattern.test(normalizedInput));
    }
    /**
     ...
     */
    createSecureWhereClause(conditions) {
     ...
     */
    createSecureOrderClause(orderBy, orderDirection = "ASC") {
        if (!orderBy) {
     ...
     */
    validateQueryExecution(query, params = []) {
        try {
 ...
 */
export function createSqlInjectionProtection(config) {
