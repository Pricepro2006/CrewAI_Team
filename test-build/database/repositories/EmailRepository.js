...
export class EmailRepository {
    ...
    `));
        this.statements.set("insertRecipient", this.db.prepare(`
    ...
    `));
        this.statements.set("getEmailRecipients", this.db.prepare(`
     ...
     */
    async updateEmail(emailId, params) {
        const startTime = Date.now();
        try {
            const updateData = {
                id: emailId,
     ...
     */
    async storeEmailEntities(emailId, entities) {
        const startTime = Date.now();
        try {
    ...
    async queryEmails(params) {
        ...
        try {
            ...
            if (validatedParams.search) {
                whereClauses.push(`(subject LIKE ? OR body_content LIKE ? OR sender_email LIKE ?)`);
            ...
            const { total } = this.db.prepare(countQuery).get(...queryParams);
    ...
    async getEmailById(emailId) {
        try {
            ...
            email.recipients = recipients.filter((r) => r.recipient_type === 'to')
                .map((r) => ({ address: r.email_address, name: r.name }));
            email.ccRecipients = recipients.filter((r) => r.recipient_type === 'cc')
                .map((r) => ({ address: r.email_address, name: r.name }));
            email.bccRecipients = recipients.filter((r) => r.recipient_type === 'bcc')
                .map((r) => ({ address: r.email_address, name: r.name }));
            return email;
        }
        catch (error) {
     ...
     */
    async getEmailByGraphId(graphId) {
        try {
    ...
    async getAnalytics(dateRange) {
        try {
            ...
            const urgentCount = this.statements
                .get("countByPriority")
                .all("critical", "high", null, null, null, null, null, null, null, null)[0]?.count || 0;
