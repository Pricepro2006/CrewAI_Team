...
import { WebSocketAuthManager, createWebSocketAuthMiddleware, } from "../middleware/websocketAuth.js";
 ...
 */
export function setupAuthenticatedWebSocketServer(wss, userService) {
    // Create auth manager
 ...
 */
function getDefaultSubscriptions(ws) {
    const subscriptions = [];
    if (!ws.permissions)
 ...
 */
function filterAllowedSubscriptions(requested, ws) {
    if (!ws.permissions)
        return [];
    ...
    for (const channel of requested) {
        ...
        else if (channel.startsWith("grocery-nlp.") &&
 ...
 */
export function createAuthenticatedWebSocketServer(port, userService) {
