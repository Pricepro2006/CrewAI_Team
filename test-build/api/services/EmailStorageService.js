...
export class EmailStorageService {
    ...
    useConnectionPool;
    masterOrchestrator;
    constructor(dbPath, enableConnectionPool = true) {
        ...
        if (this.useConnectionPool) {
            ...
            logger.info("EmailStorageService initialized with optimized connection pool", "EMAIL_STORAGE");
        }
        else {
            // Use optimized database with caching and performance monitoring
            const databasePath = dbPath || appConfig?.database?.path;
            this.db = getDatabase(databasePath);
            logger.info("EmailStorageService initialized with OptimizedQueryExecutor", "EMAIL_STORAGE");
        }
            ...
            get: (target, prop) => {
                    ...
                    return (sql) => {
                        ...
                        return {
                            run: (...params) => {
                                ...
                                return connection.execute((db) => db.prepare(sql).run(...params));
                            },
                            get: (...params) => {
                                const connection = databaseManager.getConnection('main');
                                return connection.execute((db) => db.prepare(sql).get(...params));
                            },
                            all: (...params) => {
                                const connection = databaseManager.getConnection('main');
                                return connection.execute((db) => db.prepare(sql).all(...params));
                            },
                            iterate: (...params) => {
                                const connection = databaseManager.getConnection('main');
                                return connection.execute((db) => db.prepare(sql).iterate(...params));
                            },
                        };
                    };
                }
                ...
                if (prop === "pragma") {
                    return (pragma, options) => {
                        const connection = databaseManager.getConnection('main');
                        return connection.execute((db) => db.pragma(pragma, options));
                    };
                }
                // For exec method
                if (prop === "exec") {
                    return (sql) => {
                        const connection = databaseManager.getConnection('main');
                        return connection.execute((db) => db.exec(sql));
                    };
                }
                ...
                return (...args) => {
                    ...
                    return connection.execute((db) => db[prop](...args));
                };
            }
        };
     ...
     */
    async initializeMasterOrchestrator() {
        try {
            ...
            logger.warn(`Failed to initialize MasterOrchestrator: ${error instanceof Error ? error.message : 'Unknown error'}. Email processing will use fallback methods.`, "EMAIL_STORAGE");
        }
    }
    /**
    ...
    async processEmailThroughAgents(email) {
        ...
        logger.info(`Processing email through agent system: ${email.subject}`, "EMAIL_STORAGE");
        try {
            const result = await this.masterOrchestrator.processEmail(email);
            logger.info(`Email processed successfully through agents: ${email.id}`, "EMAIL_STORAGE", {
            ...
            logger.error(`Failed to process email through agents: ${email.id}`, "EMAIL_STORAGE", { error });
            throw error;
        }
    }
    /**
     ...
     */
    async processBatchThroughAgents(emailIds, batchSize = 10) {
        let processed = 0;
        let failed = 0;
        const errors = [];
        logger.info(`Starting batch processing of ${emailIds.length} emails through agent system`, "EMAIL_STORAGE");
        // Process in batches to avoid overwhelming the system
        for (let i = 0; i < emailIds.length; i += batchSize) {
            const batch = emailIds.slice(i, i + batchSize);
            await Promise.allSettled(batch.map(async (emailId) => {
                try {
                    // Get email data
                    ...
                    logger.error(`Failed to process email ${emailId} through agents`, "EMAIL_STORAGE", { error });
                }
            }));
        ...
        logger.info(`Batch processing completed: ${processed} processed, ${failed} failed`, "EMAIL_STORAGE");
        return { processed, failed, errors };
    }
    /**
     * Start processing the email backlog through agents
     */
    async startAgentBacklogProcessing() {
        try {
        ...
        LIMIT 1000
      `;
            const unprocessedEmails = this.db.prepare(unprocessedQuery).all();
            const emailIds = unprocessedEmails.map((row) => row.id);
            if (emailIds.length === 0) {
                logger.info("No unprocessed emails found for agent processing", "EMAIL_STORAGE");
                return;
            }
            logger.info(`Starting agent processing for ${emailIds.length} unprocessed emails`, "EMAIL_STORAGE");
            ...
            logger.error("Failed to start agent backlog processing", "EMAIL_STORAGE", { error });
            throw error;
        }
    }
    /**
    ...
    async executeOptimizedQuery(queryDescription, query, params = [], method = "all") {
        ...
        while (retryCount <= maxRetries) {
            try {
                ...
                if (isRetryable && retryCount < maxRetries) {
                    ...
                    logger.warn(`Database query failed (attempt ${retryCount}/${maxRetries}), retrying in ${delay}ms: ${error}`, "EMAIL_STORAGE");
                    await new Promise((resolve) => setTimeout(resolve, delay));
                    continue;
                }
                // Non-retryable error or max retries exceeded
                logger.error(`Database query failed after ${retryCount} retries: ${error}`, "EMAIL_STORAGE", {
                    query: queryDescription,
                    params,
                    error,
                });
                // Wrap in a more descriptive error
                const enhancedError = new Error(`Database operation failed: ${queryDescription}. ${error}`);
        ...
        if (result === undefined) {
            throw new Error(`Query failed: ${queryDescription}. No result obtained after ${maxRetries} retries.`);
        }
        return result;
    }
    /**
    ...
    async storeEmail(email, analysis) {
      ...
      `);
                    ...
                    // Quick analysis
                    analysis?.quick?.workflow.primary, analysis?.quick?.priority, analysis?.quick?.intent, analysis?.quick?.urgency, analysis?.quick?.confidence, analysis?.quick?.suggestedState, analysis?.processingMetadata?.models.stage1, validatedProcessingTimes.stage1Time, 
                    // Deep analysis
                    ...
                    // Metadata
                    analysis?.processingMetadata?.models.stage2, validatedProcessingTimes.stage2Time, validatedProcessingTimes.totalTime, new Date().toISOString());
                });
                transaction();
                ...
                try {
                    ...
                    logger.error(`Failed to broadcast email analysis update: ${broadcastError}`, "EMAIL_STORAGE");
                ...
                if (!isRetryable || attempt >= maxRetries) {
                    logger.error(`Failed to store email after ${attempt} attempts: ${errorMessage}`, "EMAIL_STORAGE", {
                        emailId: email.id,
                        subject: email.subject,
                        error: errorMessage,
                    });
                    // Wrap in a more descriptive error
                    const enhancedError = new Error(`Failed to store email analysis for "${email.subject}": ${errorMessage}`);
                    enhancedError.emailId = email.id;
                    enhancedError.attempts = attempt;
                    throw enhancedError;
                }
                // Wait before retry with exponential backoff
                const delay = Math.min(100 * Math.pow(2, attempt), 2000);
                logger.warn(`Email storage failed (attempt ${attempt}/${maxRetries}), retrying in ${delay}ms: ${errorMessage}`, "EMAIL_STORAGE");
                await new Promise((resolve) => setTimeout(resolve, delay));
            }
        }
    }
     ...
     */
    async storeEmailAnalysis(messageId, analysis) {
        try {
            logger.info(`Storing email analysis for message ID: ${messageId}`, "EMAIL_STORAGE");
            // Validate and sanitize processing times before storage
            const validatedProcessingTimes = this.validateProcessingTimes(analysis.processingMetadata);
            const transaction = this?.db?.transaction(() => {
                // Store enhanced analysis
        ...
        `);
                ...
                analysisStmt.run(analysisId, messageId, // Use messageId as email_id reference
                analysis?.quick?.workflow.primary, analysis?.quick?.priority, analysis?.quick?.intent, analysis?.quick?.urgency, analysis?.quick?.confidence, analysis?.quick?.suggestedState, validatedProcessingTimes?.models?.stage1, validatedProcessingTimes.stage1Time, analysis?.deep?.detailedWorkflow.primary, JSON.stringify(analysis?.deep?.detailedWorkflow.secondary || []), JSON.stringify(analysis?.deep?.detailedWorkflow.relatedCategories || []), analysis?.deep?.detailedWorkflow.confidence, JSON.stringify(analysis?.deep?.entities), JSON.stringify(analysis?.deep?.actionItems), JSON.stringify(analysis?.deep?.workflowState), JSON.stringify(analysis?.deep?.businessImpact), analysis?.deep?.contextualSummary || "", analysis.actionSummary, validatedProcessingTimes.stage1Time, validatedProcessingTimes.stage2Time, validatedProcessingTimes.totalTime, JSON.stringify(validatedProcessingTimes.models), new Date().toISOString(), new Date().toISOString());
            });
            transaction();
            logger.info(`Email analysis stored successfully for message ID: ${messageId}`, "EMAIL_STORAGE");
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger.error(`Failed to store email analysis for message ID: ${messageId}`, "EMAIL_STORAGE", { error: errorMessage });
            throw error;
        }
    }
     ...
     */
    async getRecentEmailsForUser(userId, daysBack = 7) {
        try {
            const stmt = this.db.prepare(`
        SELECT 
          e.id,
      ...
      `);
            ...
            const results = stmt.all(userPattern, userPattern);
    ...
    async getEmailsForTableView(options) {
      ...
      `;
            ...
            if (!Array.isArray(emailsResult)) {
                logger.error(`Expected emails array but received: ${typeof emailsResult}`, "EMAIL_STORAGE", { receivedType: typeof emailsResult, receivedValue: emailsResult });
            }
        ...
        try {
            ...
            return {
                ...
                recommendations: [
                    ...
                    ...(Array.isArray(dbMetrics) ? [] : (dbMetrics.alerts?.map((alert) => alert.message) || [])),
                ],
            };
        }
        catch (error) {
    ...
    async close() {
        ...
        if (this.masterOrchestrator) {
            try {
                ...
                logger.warn(`Failed to cleanup MasterOrchestrator: ${error}`, "EMAIL_STORAGE");
            }
        }
     ...
     */
    validateProcessingTimes(metadata) {
        ...
        if (metadata.totalTime < 0) {
            ...
            validated.totalTime = Math.max(Math.abs(metadata.totalTime), validated.stage1Time + validated.stage2Time);
     ...
     */
    trackProcessingTimeAnomaly(anomaly) {
        try {
      ...
      `);
            if (stmt) {
                stmt.run(uuidv4(), anomaly.type, JSON.stringify(anomaly.issues), JSON.stringify(anomaly.originalValues), JSON.stringify(anomaly.correctedValues), anomaly.timestamp);
            }
        }
        catch (error) {
