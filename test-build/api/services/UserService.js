...
export class UserService {
    ...
    `);
        stmt.run(userId, input?.email?.toLowerCase(), input.username, passwordHash, input.first_name || null, input.last_name || null, input.avatar_url || null, input.role || "user", true, false, now, now);
    ...
    async changePassword(userId, input) {
        ...
        const isCurrentPasswordValid = await passwordManager.verifyPassword(input.currentPassword, user.password_hash);
     ...
     */
    listUsers(query = {}) {
        const { page = 1, limit = 20, search, role, is_active, is_verified, sort_by = "created_at", sort_order = "desc", } = query;
    ...
    `);
        const users = usersStmt.all(...params, limit, offset);
     ...
     * Create user session
     */
    createUserSession(userId, sessionToken, deviceInfo, expiresAt) {
    ...
    `);
        stmt.run(sessionId, userId, sessionToken, deviceInfo?.device_info || null, deviceInfo?.ip_address || null, deviceInfo?.user_agent || null, expires.toISOString(), now, now);
        return {
            id: sessionId,
            user_id: userId,
