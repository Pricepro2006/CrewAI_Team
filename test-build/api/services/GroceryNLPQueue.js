...
export class GroceryNLPQueue extends EventEmitter {
     ...
     */
    async enqueue(operation, priority = "normal", timeout, query, metadata) {
     ...
     */
    async enqueueBatch(operations, priority = "normal", batchOptions) {
        ...
        const maxConcurrency = Math.min(batchOptions?.maxConcurrency || this.maxConcurrent, this.maxConcurrent, operations?.length || 0);
        ...
        try {
            if (failFast) {
                ...
                const batchResults = await Promise.all(operations?.map((op, index) => this.enqueue(op, priority, timeout, `batch-operation-${index}`, { batchId, batchIndex: index }).catch(error => {
                    failedCount++;
                    throw error;
                })));
                ...
                for (const chunk of chunks) {
                    const chunkPromises = chunk?.map((op, index) => this.enqueue(op, priority, timeout, `batch-operation-${results?.length || 0 + index}`, { batchId, batchIndex: results?.length || 0 + index }).then(result => {
                        completedCount++;
                        return result;
                    }).catch(error => {
                        failedCount++;
     ...
     */
    generateFingerprint(query, metadata) {
     ...
     */
    emitRequestStatus(requestId, status, result, error) {
        const event = {
            type: "request_status",
            data: {
                requestId,
