...
export class PerformanceOptimizer {
    ...
    optimizeSelectStatements(query) {
        ...
        if (query.includes("SELECT *") && query.includes("emails")) {
            return query.replace("SELECT *", "SELECT id, internet_message_id, email_alias, requested_by, subject, summary, status, priority, workflow_state, received_date");
        }
        return query;
    }
    /**
    ...
    async withCache(cacheKey, queryFn, ttl = this.CACHE_TTL) {
        ...
        if (this?.queryCache?.size > this.MAX_CACHE_SIZE) {
            const entries = Array.from(this?.queryCache?.entries()).sort((a, b) => a[1].timestamp - b[1].timestamp);
            ...
            cacheHitRate: this?.performanceMetrics?.cacheHits > 0
                ? this?.performanceMetrics?.cacheHits /
                    (this?.performanceMetrics?.cacheHits +
                        this?.performanceMetrics?.cacheMisses)
                : 0,
            avgQueryTime: this?.performanceMetrics?.queryTimes?.length || 0 > 0
                ? this?.performanceMetrics?.queryTimes.reduce((sum, q) => sum + q.executionTime, 0) / this?.performanceMetrics?.queryTimes?.length || 0
                : 0,
            slowQueriesCount: this?.performanceMetrics?.slowQueries?.length || 0,
            totalOptimizationRecommendations: this?.performanceMetrics?.optimizationRecommendations?.length || 0,
        };
    }
    /**
     * Cleanup resources
    ...
    getPerformanceMetrics() {
        const avgQueryTime = this?.performanceMetrics?.queryTimes?.length || 0 > 0
            ? this?.performanceMetrics?.queryTimes.reduce((sum, q) => sum + q.executionTime, 0) / this?.performanceMetrics?.queryTimes?.length || 0
            : 0;
        const cacheHitRatio = this?.performanceMetrics?.cacheHits + this?.performanceMetrics?.cacheMisses >
            0
            ? (this?.performanceMetrics?.cacheHits /
                (this?.performanceMetrics?.cacheHits +
                    this?.performanceMetrics?.cacheMisses)) *
    ...
    generateGlobalRecommendations() {
        ...
        const avgQueryTime = metrics?.queryTimes?.length > 0
            ? metrics?.queryTimes?.reduce((sum, q) => sum + q.executionTime, 0) /
