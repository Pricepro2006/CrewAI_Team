...
export class ProductLookupService {
     ...
     */
    async lookupProducts(query, options = {}) {
        try {
            ...
            if (exactMatches?.length || 0 >= (options.maxResults || 10) &&
     ...
     */
    async matchProductsFromText(text) {
        try {
     ...
     * Find exact matches
     */
    async findExactMatches(query, filters) {
        ...
        try {
            ...
            if (results.ids?.[0]) {
                for (let i = 0; i < results.ids[0].length; i++) {
                    ...
                    if (!productId)
                        continue; // Type guard for possibly undefined
     ...
     * Build query from attributes
     */
    buildQueryFromAttributes(attributes) {
        const parts = [];
        if (attributes.name)
            parts.push(attributes.name);
        if (attributes.brand)
            parts.push(attributes.brand);
        if (attributes.category)
            parts.push(attributes.category);
        if (attributes.flavor)
            parts.push(attributes.flavor);
        if (attributes.size)
     ...
     */
    buildRecommendationQuery(context) {
        const parts = [];
     ...
     * Transform ProductEntity to WalmartProduct
     */
    transformToWalmartProduct(entity) {
        return {
            ...
            category: {
                id: "1",
                name: typeof entity.category_path === "string"
                    ? entity.category_path
                    : "Uncategorized",
                path: typeof entity.category_path === "string"
                    ? [entity.category_path]
                    : ["Uncategorized"],
                level: 1,
            },
            ...
            ingredients: typeof entity.ingredients === "string"
                ? [entity.ingredients]
                : entity.ingredients,
