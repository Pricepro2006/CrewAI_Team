...
const convertRepoGroceryItemToService = (repoItem) => repoItem;
const convertRepoShoppingSessionToService = (repoSession) => repoSession;
export class WalmartGroceryService {
    ...
    async searchProducts(options) {
        try {
            ...
            // Apply filters
            if (options.category) {
                localResults = localResults?.filter(p => p.category_path?.includes(options.category) ||
     ...
     */
    async getProductDetails(productId, includeRealTime = true) {
        try {
            ...
            if (includeRealTime && this.needsPriceUpdate(product)) {
                ...
                if (updatedProduct) {
                    ...
                    if (updatedPrice !== product.current_price) {
     ...
     */
    async createGroceryList(userId, name, description) {
        try {
    ...
    async addItemsToList(listId, items) {
        try {
            ...
            this.updateListTotal(listId).catch(err => logger.error("Failed to update list total", "WALMART_SERVICE", { listId, error: err }));
        }
    }
    /**
     * Find substitutions for unavailable products
     */
    async findSubstitutions(productId, userId, options) {
        try {
            ...
            const scoredSubstitutes = await this.scoreSubstitutions(originalProduct, substitutes, options?.similarityThreshold || 0.7);
     ...
     */
    async startShoppingSession(userId, listId, type = "online") {
        try {
     ...
     */
    async processCheckout(sessionId) {
        try {
    ...
    async getRecommendations(userId, context) {
        try {
            ...
            const recommendations = await this?.lookupService?.getPersonalizedRecommendations(userId, recommendationContext);
     ...
     */
    applyUserPreferences(products, preferences, options) {
     ...
     */
    async scoreSubstitutions(original, substitutes, threshold) {
        ...
        const scored = substitutes?.map(sub => {
            ...
            return { ...sub, similarity_score: score };
        });
     ...
     */
    transformEntityToProduct(entity) {
        return {
            ...
            availability: {
                ...
                stockLevel: entity.stock_quantity > 0 ? "in_stock" : "out_of_stock",
     ...
     */
    transformDatabaseListToType(dbList) {
        return {
            id: dbList.id,
