...
export class DealMatchingService {
     ...
     */
    async findDealsForProduct(productId) {
        try {
            // Check cache first
            ...
            const allMatches = this.combineAndRankMatches(exactMatches, similarMatches, categoryMatches);
            // Cache results
     ...
     */
    async findDealsForProducts(productIds) {
        try {
            ...
            for (let i = 0; i < productIds?.length || 0; i += batchSize) {
                ...
                const batchResults = await Promise.all(batch?.map((id) => this.findDealsForProduct(id)));
     ...
     */
    async analyzeShoppingListDeals(productIds) {
        try {
     ...
     * Get trending deals
     */
    async getTrendingDeals(category, limit = 10) {
        try {
            ...
            for (const deal of recentDeals) {
                ...
                if (category) {
                    const hasCategory = deal?.items?.some((item) => item.product_family?.toLowerCase().includes(category.toLowerCase()));
                    if (!hasCategory)
                        continue;
                }
                // Get the best deal item
                const bestItem = deal?.items?.reduce((best, item) => {
                    const itemDiscount = this.calculateDiscount(item.dealer_net_price, item.msrp || item.dealer_net_price * 1.2);
                    const bestDiscount = this.calculateDiscount(best.dealer_net_price, best.msrp || best.dealer_net_price * 1.2);
                ...
                if (product) {
                    matches.push({
                        ...
                        deal,
                        dealItem: bestItem,
                        savings: (typeof product.price === "object"
                            ? product?.price?.wasPrice || product?.price?.regular || 0
                            : product.price || 0) - bestItem.dealer_net_price,
                        savingsPercent: this.calculateDiscount(bestItem.dealer_net_price, typeof product.price === "object"
                            ? product?.price?.wasPrice || product?.price?.regular || 0
                            : product.price || 0),
                        matchConfidence: 0.8,
                        matchType: "similar",
     ...
     */
    async findExactMatches(product) {
        const matches = [];
        try {
            // Search by SKU
            if (product.upc) {
                ...
                for (const deal of dealsBySku) {
                    const matchingItem = deal.items?.find((item) => item.part_number === product.upc);
                    if (matchingItem) {
                        matches.push(this.createDealMatch(product, deal, matchingItem, "exact"));
                    }
                }
            }
            ...
            if (product.upc) {
                ...
                for (const deal of dealsByUpc) {
                    const matchingItem = deal.items?.find((item) => item.description?.includes(product.upc));
                    if (matchingItem) {
                        matches.push(this.createDealMatch(product, deal, matchingItem, "exact"));
                    }
                }
     ...
     */
    async findSimilarMatches(product) {
        const matches = [];
        try {
            ...
            const nameSearch = recentDeals
                .filter((deal) => deal.items?.some((item) => item.description
                ?.toLowerCase()
            ...
            for (const deal of nameSearch) {
                ...
                const scoredItems = deal?.items?.map((item) => ({
                    item,
                ...
                if (bestMatch) {
                    matches.push(this.createDealMatch(product, deal, bestMatch.item, "similar"));
                }
            }
        }
        catch (error) {
     ...
     */
    async findCategoryMatches(product) {
        const matches = [];
        try {
            if (!product.category)
                return matches;
            ...
            for (const family of families) {
                ...
                const categoryDeals = recentDeals
                    .filter((deal) => deal.items?.some((item) => item.product_family?.toLowerCase().includes(family.toLowerCase())))
                    .slice(0, 5);
                for (const deal of categoryDeals) {
                    if (!deal.items?.length)
                        continue;
                    // Find items that might match
                    const relevantItems = deal?.items?.filter((item) => item.product_family === family);
     ...
     */
    createDealMatch(product, deal, dealItem, matchType) {
        ...
        const savingsPercent = walmartPrice > 0 ? (savings / walmartPrice) * 100 : 0;
     ...
     */
    calculateSimilarity(product, item) {
        let score = 0;
        let factors = 0;
        // Name similarity
        if (product.name && item.description) {
            const nameSimilarity = this.stringSimilarity(product?.name?.toLowerCase(), item?.description?.toLowerCase());
     ...
     */
    calculateDiscount(salePrice, originalPrice) {
        if (originalPrice <= 0)
            return 0;
            ...
            if (item.part_number) {
                ...
                if (products?.length || 0 > 0) {
                    ...
                    return productEntity
                        ? this.convertToTypesProduct(this?.productRepo?.entityToProduct(productEntity))
                        : null;
                }
            }
     ...
     */
    combineAndRankMatches(exact, similar, category) {
     ...
     */
    convertToTypesProduct(repoProduct) {
        return {
            ...
            category: {
                id: "1",
                name: typeof repoProduct.category_path === "string"
                    ? repoProduct.category_path
                    : "Uncategorized",
                path: typeof repoProduct.category_path === "string"
                    ? [repoProduct.category_path]
                    : ["Uncategorized"],
                level: 1,
            },
            ...
            ingredients: typeof repoProduct.ingredients === "string"
                ? [repoProduct.ingredients]
                : repoProduct.ingredients,
            allergens: repoProduct.allergens?.map((allergen) => ({
                type: allergen.toLowerCase(),
                contains: true,
                mayContain: false,
            })),
            metadata: {
