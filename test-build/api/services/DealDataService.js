...
export class DealDataService {
    ...
    async getDealItem(dealId, productNumber) {
      ...
      `);
            ...
            logger.error(`Failed to get deal item ${productNumber} from deal ${dealId}: ${error instanceof Error ? error.message : String(error)}`, "DEAL_DATA");
            throw error;
        }
    }
    /**
     ...
     */
    async analyzeDealForProducts(productIds, customerId) {
        try {
            const deals = [];
            let totalSavings = 0;
      ...
      `;
            ...
            results.forEach((row) => {
                ...
                const discountPrice = this.calculatePrice(row.dealer_net_price, row.product_family);
     ...
     */
    async getRecentDeals(hoursAgo = 24) {
        try {
            // Validate input
      ...
      `);
            const results = stmt.all(`-${hoursAgo}`);
