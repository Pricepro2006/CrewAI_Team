    ...
    const store = redisClient && redisConnected
        ? new RedisStore({
            sendCommand: async (command, ...args) => {
                const cmd = new redisClient.constructor.Command(command, args);
                const result = await redisClient.sendCommand(cmd);
                return result; // Type assertion for RedisReply compatibility
            },
...
export function rateLimitMiddleware(maxRequests = 100, windowMs = 60000, options = {}) {
    const store = new Map();
