...
export function requestTracking(req, res, next) {
    ...
    res.on("finish", () => {
        ...
        metricsCollector.histogram("http_request_memory_delta_bytes", endMemory.heapUsed - startMemory.heapUsed, {
            method: req.method,
            route: req.route?.path || req.path,
        });
        // Log completion
        ...
        if (res.statusCode >= 400) {
            ...
            const errorString = typeof errorMessage === "object"
                ...
                : String(errorMessage);
            errorTracker.trackError(new Error(`HTTP ${res.statusCode}: ${errorString}`), errorContext, severity, true, [`http-${res.statusCode}`, req?.method?.toLowerCase()]);
        }
    });
    next();
}
// Error handling middleware
export function errorTracking(err, req, res, _next) {
    ...
    const errorId = errorTracker.trackError(err, {
        ...
        ip: req.ip,
        userId: req.user?.id,
    }, "high", false, ["unhandled", "middleware"]);
    // Log error
...
export function monitorOperation(operationName) {
    return function (target, propertyKey, descriptor) {
        const originalMethod = descriptor?.value;
        descriptor.value = async function (...args) {
            const fullOperationName = `${operationName}_${propertyKey}`;
            return performanceMonitor.trackAsync(fullOperationName, () => originalMethod.apply(this, args), { class: target?.constructor?.name, method: propertyKey });
        };
        return descriptor;
    };
}
// Request size tracking
export function requestSizeTracking(req, res, next) {
    // Track request size
...
// Rate limit tracking
export function rateLimitTracking(req, res, next) {
    res.on("finish", () => {
        if (res.statusCode === 429) {
            ...
            errorTracker.trackError(new Error("Rate limit exceeded"), {
                endpoint: req.path,
                method: req.method,
                ip: req.ip || "unknown",
                userAgent: req.get("user-agent"),
                userId: req.user?.id,
            }, "low", true, ["rate-limit", "security"]);
        }
    });
    next();
}
// Authentication tracking
export function authTracking(req, res, next) {
    res.on("finish", () => {
        if (req?.path?.includes("/auth") || req?.path?.includes("/login")) {
            ...
            if (!success && res.statusCode === 401) {
                ...
                errorTracker.trackError(new Error("Authentication failed"), {
                    endpoint: req.path,
                    method: req.method,
                    ip: req.ip || "unknown",
                    userAgent: req.get("user-agent"),
                }, "medium", true, ["auth", "security"]);
            }
        }
    });
    next();
...
export function trackDatabaseQuery(queryName, operation) {
...
// WebSocket tracking
export function trackWebSocketConnection(connected, _clientId) {
    if (connected) {
        metricsCollector.increment("websocket_connections_total");
        metricsCollector.gauge("websocket_connections_active", 1, {
            action: "increment",
        });
    }
    else {
...
export function trackWebSocketMessage(direction, event, size) {
...
// Ollama tracking
export function trackOllamaRequest(model, operation) {
