...
export class WebSocketAuthManager {
     ...
     */
    disconnectUser(userId, wsService) {
    ...
    startCleanupInterval() {
        this.cleanupInterval = setInterval(() => {
            ...
            if (expired?.length || 0 > 0) {
                logger.debug(`Cleaned up ${expired?.length || 0} expired WebSocket sessions`, "WS_AUTH");
            }
...
export function createWebSocketAuthMiddleware(authManager) {
    return async (ws, req) => {
