...
import { createSecurityAuditMiddleware, createAuthMiddleware, createAuthorizationMiddleware, createInputValidation, sanitizationSchemas, createCSRFProtection, ensureCSRFToken, } from "../middleware/security/index.js";
...
const secureStringValidation = t.middleware(async ({ ctx, next, input }) => {
    const validationFn = createInputValidation(z.object({
...
const createRateLimitMiddleware = (name, maxRequests, windowMs) => t.middleware(async ({ ctx, next }) => {
    ...
    if (!ctx.rateLimits) {
        ctx.rateLimits = new Map();
    }
    const rateLimits = ctx.rateLimits;
    // Clean old entries
...
export const chatProcedure = protectedProcedure.use(createRateLimitMiddleware("chat", 30, 60000) // 30 requests per minute
);
export const agentProcedure = protectedProcedure.use(createRateLimitMiddleware("agent", 20, 300000) // 20 requests per 5 minutes
);
export const taskProcedure = protectedProcedure.use(createRateLimitMiddleware("task", 25, 600000) // 25 requests per 10 minutes
);
export const ragProcedure = protectedProcedure.use(createRateLimitMiddleware("rag", 15, 120000) // 15 requests per 2 minutes
);
export const strictProcedure = protectedProcedure.use(createRateLimitMiddleware("strict", 5, 1800000) // 5 requests per 30 minutes
);
