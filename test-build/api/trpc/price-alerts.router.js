...
export const priceAlertsRouter = router({
    ...
    getUserAlerts: protectedProcedure
        ...
        try {
            ...
            const filteredAlerts = input?.status ? alerts.filter((alert) => alert.status === input.status) : alerts;
            return {
                success: true,
    ...
    getNotificationHistory: protectedProcedure
        ...
        try {
            const notifications = await service.getNotificationHistory(ctx?.user?.id, input?.limit || 50);
            return {
                success: true,
                notifications,
