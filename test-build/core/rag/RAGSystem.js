...
export class RAGSystem {
    ...
    async initialize() {
        ...
        try {
            ...
            if (storeInfo.fallbackUsed) {
                logger.warn("RAG system initialized with in-memory fallback - advanced RAG features may be limited", "RAG_SYSTEM");
            }
            else {
                logger.info("RAG system initialized successfully with ChromaDB", "RAG_SYSTEM");
            }
            ...
            try {
                ...
                logger.warn(`Embedding service initialization failed: ${error instanceof Error ? error.message : 'Unknown error'}. Some RAG features may be limited.`, "RAG_SYSTEM");
            ...
            logger.error(`RAG system initialization failed: ${error instanceof Error ? error.message : 'Unknown error'}`, "RAG_SYSTEM");
    ...
    async getContextForPrompt(query, options = {}) {
        const { limit = 5, filter, includeMetadata = true, formatForLLM = true, } = options;
     ...
     */
    async indexEmailContent(emailId, emailData) {
        if (!this.isInitialized) {
            await this.initialize();
        }
        ...
        logger.info(`Indexed email content for ID: ${emailId}`, "RAG_SYSTEM");
    }
    /**
     * Batch index multiple emails efficiently
     * Optimized for large datasets like 143,221 emails
     */
    async batchIndexEmails(emails, options = {}) {
        if (!this.isInitialized) {
            await this.initialize();
        }
        ...
        logger.info(`Starting batch email indexing: ${emails.length} emails in ${totalBatches} batches (size: ${batchSize})`, "RAG_SYSTEM");
        for (let i = 0; i < emails.length; i += batchSize) {
            const batch = emails.slice(i, i + batchSize);
            const batchNumber = Math.floor(i / batchSize) + 1;
            logger.info(`Processing batch ${batchNumber}/${totalBatches} (${batch.length} emails)`, "RAG_SYSTEM");
            ...
            try {
                ...
                logger.info(`Batch ${batchNumber}/${totalBatches} completed: ${batch.length} emails indexed (${progress.percentage}% total)`, "RAG_SYSTEM");
        ...
        logger.info(`Email batch indexing completed: ${results.indexed} indexed, ${results.failed} failed in ${timeElapsedSeconds}s (${emailsPerSecond} emails/sec)`, "RAG_SYSTEM");
     ...
     */
    async getEmailContext(query, options = {}) {
     ...
     */
    async indexAgentKnowledge(agentId, documents) {
        if (!this.isInitialized) {
            await this.initialize();
        }
        ...
        logger.info(`Indexed ${documents?.length || 0} documents for agent: ${agentId}`, "RAG_SYSTEM");
    }
    /**
     * Get knowledge specific to an agent
     */
    async getAgentKnowledge(agentId, query, limit = 5) {
        return await this.searchWithFilter(query, { agentId }, limit);
    }
