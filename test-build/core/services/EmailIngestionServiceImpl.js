...
import { IngestionMode, IngestionSource, IngestionError, IngestionErrorCodes } from './EmailIngestionService.js';
import { logger } from '../../utils/logger.js';
import { metrics } from '../../api/monitoring/metrics.js';
import { io } from '../../api/websocket/index.js';
export class EmailIngestionServiceImpl {
    config;
    emailRepository;
    unifiedEmailService;
    redis;
    ...
    constructor(config, emailRepository, unifiedEmailService) {
        this.config = config;
    ...
    async initializeQueues() {
        ...
        this.deadLetterQueue = new Queue('email-ingestion-dlq', {
            connection: redisConfig,
            defaultJobOptions: {
        ...
        this.worker = new Worker('email-ingestion', async (job) => this.processIngestionJob(job), {
            connection: redisConfig,
            concurrency: this?.config?.processing.concurrency,
            maxStalledCount: 3,
    ...
    // =====================================================
    async ingestEmail(email, source) {
        try {
    ...
    async ingestBatch(emails, source) {
    ...
    async ingestFromDatabase(query, limit = 1000) {
        try {
    ...
    async ingestFromMicrosoftGraph(folderId) {
        try {
    ...
    async ingestFromGmailApi(labelId) {
        try {
    ...
    async retryFailedJobs(limit = 100) {
    ...
    async processIngestionJob(job) {
        ...
        try {
            ...
            throw new IngestionError(`Failed to process email: ${error instanceof Error ? error.message : String(error)}`, IngestionErrorCodes.PROCESSING_ERROR, source, true, error instanceof Error ? error : undefined);
        }
    }
    async moveToDeadLetterQueue(jobData, error) {
        try {
    ...
    recordError(source, error) {
        if (!this.processingMetrics)
            return;
        const existingError = this.processingMetrics.errors.find(e => e.source === source && e.error === error);
        if (existingError) {
            existingError.count++;
    ...
    // =====================================================
    async getMetrics() {
        const avgProcessingTime = this?.processingMetrics?.processingTimes?.length || 0 > 0
            ? this?.processingMetrics?.processingTimes.reduce((a, b) => a + b, 0) / this?.processingMetrics?.processingTimes?.length || 0
            : 0;
