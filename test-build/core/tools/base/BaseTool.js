        ...
        for (const param of this.parameters) {
            ...
            if (param.name in params) {
                ...
                if (!this.validateType(value, param.type)) {
                    errors.push(`Invalid type for ${param.name}: expected ${param.type}, got ${typeof value}`);
                }
                // Additional validations
                if (param.type === "string" && param.pattern) {
                    const regex = new RegExp(param.pattern);
                    if (!regex.test(value)) {
                        errors.push(`Invalid format for ${param.name}: must match pattern ${param.pattern}`);
                    }
                }
