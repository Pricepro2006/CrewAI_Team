    ...
    async fallbackToInstantAnswerAPI(query, limit) {
        try {
            ...
            if (data.RelatedTopics && Array.isArray(data.RelatedTopics)) {
                for (const topic of data?.RelatedTopics?.slice(0, limit - results?.length || 0)) {
...
class SearxEngine extends SearchEngine {
    name = "searx";
    baseUrl = process.env.SEARX_URL || "http://localhost:8888/search"; // Local instance
    async search(query, limit) {
        try {
