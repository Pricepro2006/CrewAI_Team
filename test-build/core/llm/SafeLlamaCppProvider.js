...
export class SafeLlamaCppProvider extends EventEmitter {
    ...
    modelLoaded = false;
    llamaCppPath;
    static processPool = new ProcessPool(2); // Max 2 concurrent processes
    processStartTime;
    ...
    validateConfig() {
        ...
        if (this.config.allowedModelPaths && this.config.allowedModelPaths.length > 0) {
            const isAllowed = this.config.allowedModelPaths.some(allowed => modelPath.startsWith(path.resolve(allowed)));
            if (!isAllowed) {
