...
import { PRODUCTION_EMAIL_CONFIG, enhancePriorityDetection, } from "./EmailAnalysisConfig.js";
export class EmailAnalysisAgent extends BaseAgent {
    ...
    constructor() {
        try {
            super("EmailAnalysisAgent", "Specializes in analyzing and categorizing TD SYNNEX email communications", PRODUCTION_EMAIL_CONFIG.primaryModel);
        }
        catch (error) {
            ...
            super("EmailAnalysisAgent", "Specializes in analyzing and categorizing TD SYNNEX email communications", "llama3.2:3b");
        }
...
5. Confidence score (0.0 to 1.0)`;
    ...
    parseCategorizeResponse(response) {
...
Provide detailed categorization with high confidence.`;
    ...
    async extractNEREntities(text) {
        ...
        customerPatterns.forEach((pattern) => {
            ...
            customers.push(...matches?.map((m) => m
...
Summary:`;
    ...
    fallbackCategorization(email) {
        ...
        else if (content.includes("action required") ||
