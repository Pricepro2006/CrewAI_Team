...
export class WriterAgent extends BaseAgent {
    ...
    async execute(task, context) {
        try {
            ...
            if (this.ragSystem && this.ragEnabled) {
                ...
                writingExamples = searchResults.filter(r => r.metadata?.type === 'writing' || r.metadata?.type === 'content');
            }
    ...
    async analyzeWritingTask(task, context, ragContext = "") {
        const prompt = `
    ...
    parseWritingTaskAnalysis(response) {
        ...
        if (lowerResponse.includes("formal"))
            style = "formal";
        else if (lowerResponse.includes("casual"))
            style = "casual";
        else if (lowerResponse.includes("persuasive"))
            style = "persuasive";
        else if (lowerResponse.includes("narrative"))
            style = "narrative";
        ...
        // Determine length
        let length = "medium";
        if (lowerResponse.includes("short") || lowerResponse.includes("brief"))
            length = "short";
        else if (lowerResponse.includes("long") || lowerResponse.includes("detailed"))
            length = "long";
        return {
            contentType,
            style,
    ...
    async writeArticle(analysis, context, writingExamples = []) {
        const prompt = `
    ...
    async writeReport(analysis, context, writingExamples = []) {
        const prompt = `
    ...
    async writeEmail(analysis, context, writingExamples = []) {
        const prompt = `
    ...
    async writeCreative(analysis, context, writingExamples = []) {
        const prompt = `
    ...
    async writeTechnical(analysis, context, writingExamples = []) {
        const prompt = `
