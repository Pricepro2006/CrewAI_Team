...
export class DataAnalysisAgent extends BaseAgent {
    constructor() {
        super("DataAnalysisAgent", "Specializes in data processing, analysis, visualization, and insights extraction");
    }
    async execute(task, context) {
        try {
            ...
            if (this.ragSystem && this.ragEnabled) {
                ...
                historicalPatterns = searchResults.filter(r => r.metadata?.type === 'analysis' || r.metadata?.type === 'pattern');
            }
    ...
    async analyzeDataTask(task, context, ragContext) {
        const prompt = `
    ...
    parseDataTaskAnalysis(response) {
        try {
    ...
    async performStatisticalAnalysis(analysis, context, historicalPatterns = []) {
        const prompt = `
    ...
    async createVisualization(analysis, context) {
        const prompt = `
    ...
    async transformData(analysis, context) {
        const prompt = `
    ...
    async exploreData(analysis, context, historicalPatterns = []) {
        const prompt = `
    ...
    async generalDataAnalysis(task, context) {
        const prompt = `
    ...
    parseVisualizationConfig(config) {
        try {
    ...
    extractValuableInsights(result) {
        const insights = [];
