...
export function getModelTimeout(modelType) {
    return (MODEL_CONFIG.timeouts[modelType] ||
        MODEL_CONFIG?.timeouts?.primary);
}
// Helper function to get batch size for a model
export function getModelBatchSize(modelType) {
    return (MODEL_CONFIG.batchSizes[modelType] || MODEL_CONFIG?.batchSizes?.primary);
}
// Helper function to check if we have enough memory for a model
export function canRunModel(modelName) {
