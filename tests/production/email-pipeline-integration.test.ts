...
describe('Production Email Pipeline Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    logger.info('Starting Production Email Pipeline Integration Tests', 'PROD_TEST');
  ...
  describe('End-to-End Pipeline Flow', () => {
    it('should execute complete three-stage pipeline successfully', async () => {
      ...
      // Execute pipeline
      const results: PipelineResults = await pipelineOrchestrator.runThreeStagePipeline();
      ...
      expect(results.stage2Count).toBeLessThanOrEqual(Math.min(5000, results.totalEmails));
      expect(results.stage3Count).toBeLessThanOrEqual(Math.min(500, results.stage2Count));
    ...
    it('should persist pipeline execution records correctly', async () => {
      ...
      expect(stageResults.every(sr => sr.email_id && sr.stage && sr.priority_score !== null)).toBe(true);
    });
    ...
    it('should update email analysis records correctly', async () => {
      ...
      const updatedEmails = db.prepare(`
      ...
      const stage1Count = updatedEmails.filter(e => e.pipeline_stage === 1).length;
      const stage2Count = updatedEmails.filter(e => e.pipeline_stage === 2).length;
      const stage3Count = updatedEmails.filter(e => e.pipeline_stage === 3).length;
      ...
      const stage2Emails = updatedEmails.filter(e => e.pipeline_stage >= 2 && e.llama_analysis);
      const stage3Emails = updatedEmails.filter(e => e.pipeline_stage === 3 && e.phi4_analysis);
  ...
  describe('Duplicate Handling and Error Recovery', () => {
    it('should handle duplicate pipeline executions gracefully', async () => {
      ...
      const successfulResults = [firstResult, secondResult].filter(result => result.status === 'fulfilled');
    ...
    it('should handle database transaction failures and rollback', async () => {
      ...
      const initialExecutionCount = db.prepare('SELECT COUNT(*) as count FROM pipeline_executions').get().count;
      ...
      const finalExecutionCount = db.prepare('SELECT COUNT(*) as count FROM pipeline_executions').get().count;
      ...
      if (transactionFailed) {
        expect(finalExecutionCount).toBeLessThanOrEqual(initialExecutionCount + 1);
        
        // If execution record exists, it should be marked as failed
        const failedExecution = db.prepare(`
    ...
    it('should recover from individual stage failures', async () => {
      ...
      try {
        ...
        logger.error('Pipeline failed during recovery test', 'PROD_TEST', { error: error.message });
      }
    });
  });
  ...
  describe('Database Transactions and Concurrent Processing', () => {
    it('should maintain ACID properties during concurrent operations', async () => {
      logger.info('Testing ACID properties with concurrent operations', 'PROD_TEST');
    ...
    it('should handle workflow chain creation and updates transactionally', async () => {
      ...
      // Verify chain state
      const chain = db.prepare('SELECT * FROM workflow_chains WHERE id = ?').get(chainId1);
      expect(chain).toBeDefined();
      expect(chain.email_count).toBe(2);
      expect(chain.current_state).toBe('in_progress');
      
      // Verify chain links
      const chainLinks = db.prepare('SELECT * FROM workflow_chain_emails WHERE chain_id = ?').all(chainId1);
      expect(chainLinks.length).toBe(2);
      expect(chainLinks.map(l => l.email_id).sort()).toEqual([email1Id, email2Id].sort());
    });
  });
  ...
  async function verifyDatabaseConnectivity(): Promise<void> {
    try {
      ...
      logger.error('Database connectivity failed', 'PROD_TEST', { error: error.message });
  ...
  async function setupTestEmails(): Promise<void> {
    ...
    const testEmails = [
      ...
      generateTestEmailParams('test-with-attachments', 'conv-4', 'medium', true),
  ...
  async function cleanupTestData(): Promise<void> {
    try {
      ...
      db.prepare(`DELETE FROM emails_enhanced WHERE message_id LIKE 'test-%'`).run();
      ...
      db.prepare(`DELETE FROM stage_results WHERE email_id IN (
        SELECT id FROM emails_enhanced WHERE message_id LIKE 'test-%'
      )`).run();
      
      logger.info('Test data cleanup completed', 'PROD_TEST');
    } catch (error) {
      logger.error('Test data cleanup failed', 'PROD_TEST', { error: error.message });
    }
  }
  ...
  async function createProblematicTestEmails(): Promise<string[]> {
    const problematicEmails = [
      generateTestEmailParams('problematic-empty-body', 'conv-prob-1', 'medium', false, ''),
      generateTestEmailParams('problematic-very-long', 'conv-prob-2', 'high', false, 'A'.repeat(10000)),
      generateTestEmailParams('problematic-special-chars', 'conv-prob-3', 'low', false, 'Test with 特殊字符 and émojis 🚀'),
    ];
  ...
  async function captureInitialDatabaseState(): Promise<any> {
    return {
      ...
      executionCount: db.prepare('SELECT COUNT(*) as count FROM pipeline_executions').get().count,
      analysisCount: db.prepare('SELECT COUNT(*) as count FROM email_analysis').get().count
    };
  }
  ...
  async function verifyDatabaseConsistency(initialState: any, finalState: any): Promise<void> {
    // Verify counts are consistent with operations performed
    expect(finalState.emailCount).toBeGreaterThanOrEqual(initialState.emailCount);
    expect(finalState.executionCount).toBeGreaterThanOrEqual(initialState.executionCount);
    expect(finalState.analysisCount).toBeGreaterThanOrEqual(initialState.analysisCount);
    
    // Verify referential integrity
    const orphanedAnalysis = db.prepare(`
