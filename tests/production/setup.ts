...
afterAll(async () => {
  ...
  try {
    ...
    logger.info('Production test environment cleanup completed', 'PROD_TEST_SETUP');
  } catch (error) {
...
async function setupTestDatabase(): Promise<void> {
  try {
    ...
    const tables = ['emails_enhanced', 'pipeline_executions', 'stage_results', 'email_analysis'];
    ...
    db.prepare(`DELETE FROM emails_enhanced WHERE message_id LIKE 'test-%' OR message_id LIKE 'load-test-%'`).run();
...
async function verifyServiceDependencies(): Promise<void> {
  const services = [
    ...
    },
    {
      ...
      check: async () => {
        try {
          const response = await fetch(`${process.env.OLLAMA_HOST}/api/version`);
          return response.ok;
        } catch {
          return false;
        }
    ...
    },
    {
      ...
      check: async () => {
        try {
          const response = await fetch(`${process.env.CHROMADB_URL}/api/v1/version`);
          return response.ok;
        } catch {
          return false;
        }
...
async function cleanupTestDatabase(): Promise<void> {
  try {
    ...
    for (const query of cleanupQueries) {
      ...
      logger.debug(`Cleanup query executed: ${result.changes} rows affected`, 'PROD_TEST_SETUP');
    }
...
async function generateTestSummaryReport(): Promise<void> {
  try {
    ...
    const memorySnapshots = (global as any).__PROD_TEST_MEMORY_SNAPSHOTS__ || [];
