...
class PerformanceMonitor extends EventEmitter {
  ...
  getMemoryStats(): {
    ...
    const average = {
      rss:
        snapshots.reduce((sum, s) => sum + s.usage.rss, 0) / snapshots.length,
      heapTotal:
        snapshots.reduce((sum, s) => sum + s.usage.heapTotal, 0) /
        snapshots.length,
      heapUsed:
        snapshots.reduce((sum, s) => sum + s.usage.heapUsed, 0) /
        snapshots.length,
      external:
        snapshots.reduce((sum, s) => sum + s.usage.external, 0) /
        snapshots.length,
      arrayBuffers:
        snapshots.reduce((sum, s) => sum + s.usage.arrayBuffers, 0) /
        snapshots.length,
    };
...
class LoadTestDataGenerator {
  ...
  private generateEmailBody(priority: string, department: string): string {
    ...
    const departmentTemplates =
      templates[department as keyof typeof templates] || templates.general;
    const template =
      departmentTemplates[
        Math.floor(Math.random() * departmentTemplates.length)
      ];
...
describe("Email Pipeline Load Testing Suite", () => {
  ...
  describe("High-Volume Email Processing (100+ emails/sec)", () => {
    it("should process 1000 emails within 10 seconds", async () => {
      ...
      logger.info(
        `Starting high-volume test: ${emailCount} emails`,
        "LOAD_TEST",
      );
      ...
      for (let i = 0; i < batches; i++) {
        ...
        performanceMonitor.recordProcessingMetric(
          processedCount,
          avgResponseTime,
        );

        logger.debug(
          `Processed batch ${i + 1}/${batches} (${batch.length} emails) in ${batchProcessingTime.toFixed(2)}ms`,
          "LOAD_TEST",
        );
      }
    ...
    it("should handle concurrent pipeline executions", async () => {
      ...
      logger.info(
        `Starting concurrent pipeline test: ${concurrentPipelines} pipelines, ${emailsPerPipeline} emails each`,
        "LOAD_TEST",
      );
      ...
      const pipelinePromises = Array.from(
        { length: concurrentPipelines },
        async (_, index) => {
      ...
      const maxProcessingTime = Math.max(
        ...results.map((r) => r.processingTime),
      );
  ...
  describe("Memory Usage and Resource Monitoring", () => {
    it("should maintain memory usage below 500MB during high-volume processing", async () => {
      ...
      logger.info(
        `Starting memory monitoring test: ${emailCount} emails`,
        "LOAD_TEST",
      );
      ...
      for (let i = 0; i < batches; i++) {
        ...
        logger.debug(
          `Batch ${i + 1}/${batches} processed, memory: ${Math.round(currentMemory.heapUsed / 1024 / 1024)}MB`,
          "LOAD_TEST",
        );
      }
    ...
    it("should efficiently handle memory cleanup after processing", async () => {
      ...
      logger.info(
        `Starting memory cleanup test: ${emailCount} emails`,
        "LOAD_TEST",
      );
      ...
      const memoryReduction =
        memoryAfterProcessing.heapUsed - memoryAfterCleanup.heapUsed;
  ...
  describe("Database Performance Under Load", () => {
    it("should maintain database query performance under high load", async () => {
      ...
      logger.info(
        `Starting database performance test: ${emailCount} emails`,
        "LOAD_TEST",
      );
      ...
      for (let round = 0; round < 5; round++) {
        ...
        logger.debug(
          `Query round ${round + 1} completed, max time: ${maxRoundTime.toFixed(2)}ms`,
          "LOAD_TEST",
        );
      }
    ...
    it("should handle database transaction failures gracefully under load", async () => {
      ...
      logger.info(
        `Starting database resilience test: ${emailCount} emails, ${concurrentOperations} concurrent ops`,
        "LOAD_TEST",
      );
          ...
          for (const email of batch) {
            try {
              await emailRepository.createEmail(
                transformEmailForRepository(email),
              );
              results.successful++;
            } catch (error) {
              results.failed++;
      ...
      // Aggregate results
      const totalSuccessful = operationResults.reduce(
        (sum, r) => sum + r.successful,
        0,
      );
      const totalFailed = operationResults.reduce(
        (sum, r) => sum + r.failed,
        0,
      );
  ...
  describe("Pipeline Performance Optimization", () => {
    it("should optimize processing for different email priorities", async () => {
      ...
      const processingResults: {
        [key: string]: { emails: Email[]; processingTimes: number[] };
      } = {};
      ...
      const avgProcessingTimes = Object.entries(processingResults).map(
        ([priority, data]) => ({
          priority,
          avgTime:
      ...
      const criticalAvg =
        avgProcessingTimes.find((p) => p.priority === "critical")?.avgTime || 0;
      const lowAvg =
        avgProcessingTimes.find((p) => p.priority === "low")?.avgTime || 0;

      // This might not always be true due to processing variations, but log for analysis
      logger.info(
        "Priority processing times",
        "LOAD_TEST",
        avgProcessingTimes.reduce(
          (obj, p) => ({
            ...obj,
    ...
    it("should scale processing with available system resources", async () => {
      ...
      logger.info(
        `Starting resource scaling test: ${emailCount} emails, ${cpuCount} CPUs`,
        "LOAD_TEST",
      );
      ...
      const batchSizes = [
        ...
        5,
        10,
        Math.min(25, cpuCount),
        Math.min(50, cpuCount * 2),
      ];
      const batchResults: {
        batchSize: number;
        totalTime: number;
      ...
      for (const batchSize of batchSizes) {
        ...
        logger.debug(
          `Batch size ${batchSize}: ${totalTime.toFixed(2)}ms, ${throughput.toFixed(2)} emails/sec`,
          "LOAD_TEST",
        );
      }
  ...
  async function cleanupLoadTestData(): Promise<void> {
    try {
      ...
      db.prepare(
        `DELETE FROM emails_enhanced WHERE message_id LIKE 'load-test-%'`,
      ).run();
